...
In practice, opcodes are lower-cased with under_scores joining each word in the name. For instance, [Nonce Proof](#nonce-proof) would be `nonce_proof`.
...
| [Nonce Proof](#nonce-proof-structure) | Submit a cryprographic proof of the handshake |
...
| [Nonce Proof](#nonce-proof-structure)                 | Requests a cryptographic proof of the handshake                                                               |
...
| ---- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1000 | Normal closure    | The remote auth session was [finished](/remote-authentication/mobile#finish-remote-auth) or [canceled](/remote-authentication/mobile#cancel-remote-auth) successfully |
| 4000 | Unknown error     | We're not sure what went wrong; try reconnecting?                                                                                                                     |
| 4001 | Handshake failure | The [initial handshake](#handshaking) failed; maybe reconnect and try again?                                                                                          |
...
Once you open a connection to the Gateway, you will receive an [Opcode Hello](#gateway-events) payload, which contains the heartbeat interval and timeout duration. See the [heartbeating](#heartbeating) section for more information.
...
Upon receiving the [Opcode Hello](#gateway-events) payload, you should immediately send an [Opcode Init](#gateway-commands) containing the public key that all future communication will be encrypted with.
...
If the Gateway accepts the public key, it will respond with an [Opcode Nonce Proof](#gateway-events) payload, which contains a nonce that you must decrypt with your private key.
...
You should then send an [Opcode Nonce Proof](#gateway-commands) payload containing a base64URL-encoded representation of the decrypted nonce's SHA-256 digest.
...
If the Gateway accepts the nonce proof, it will respond with an [Opcode Pending Remote Init](#gateway-events) payload, which indicates that the key exchange was successful and the remote auth session is ready.
...
In order to maintain your WebSocket connection, you need to continuously send heartbeats at the interval determined in [Opcode Hello](#gateway-events):
...
After receiving [Opcode Hello](#gateway-events), you should send [Opcode Heartbeat](#gateway-events) every elapsed interval:
...
In return, you will be sent back an [Opcode Heartbeat ACK](#gateway-events).
...
Once the mobile client [creates a remote auth session](/remote-authentication/mobile#create-remote-auth-session) (scans the QR code), the Gateway will send an [Opcode Pending Ticket](#gateway-events) payload, which contains the information of the user that is attempting to authenticate.
...
At this point, the desktop client will either receive an [Opcode Pending Login](#gateway-events) or [Opcode Cancel](#gateway-events) event.
...
If the mobile client [finishes remote auth](/remote-authentication/mobile#finish-remote-auth), the Gateway will send an [Opcode Pending Login](#gateway-events) payload, which indicates that the authentication was successful.
...
If the mobile client [cancels remote auth](/remote-authentication/mobile#cancel-remote-auth), the Gateway will send an [Opcode Cancel](#gateway-events) payload, which indicates that the authentication was canceled.
...
<RouteHeader method="POST" url="/users/@me/remote-auth/login">
