...
| user_application_profiles? ^11^           | map[snowflake, array[[user application profile](#user-application-profile-structure) object]]                              | A mapping of user IDs to their application profiles of connected OAuth2 application encountered across all objects in the event                                                                                                                                               |
...
| external_id? ^1^ | [user application profile external ID](#user-application-profile-external-id-structure) object      | The external ID of the provisional account           |
| avatar_hash      | ?string                                                                                             | The user's [avatar hash](/reference#cdn-formatting)  |

^1^ Only present in [Ready](#ready) Gateway event.
