...
| regional_feature_config ^1^               | [regional feature config](#regional-feature-config-object) object                                                          | Configuration of features that can be restricted by the region you are connecting from                                                                                                                                                                                        |
...
| flags_new      | string    | The [application's flags](/resources/application#application-flags) as a string |
...
| data?         | ?[user application profile data](/resources/user#user-application-profile-data-structure) object | The user application data                            |
...
| Field                    | Type   | Description                                                      |
| ------------------------ | ------ | ---------------------------------------------------------------- |
| age_gated_features       | number | Bitflag of [age gated features](#age-gated-features)             |
| teen_by_default_settings | number | Bitflag of [teen by default settings](#teen-by-default-settings) |

###### Age Gated Features

| Value  | Name                                | Description                                         |
| ------ | ----------------------------------- | --------------------------------------------------- |
| 1\<\<0 | AGE_GATED_SPACES                    | Restricted from guilds marked as NSFW               |
| 1\<\<1 | SENSITIVE_CONTENT_SHOW_SETTING      | Restricted from changing sensitive content settings |
| 1\<\<2 | DM_PRIVACY_SETTINGS                 | Restricted from changing DM privacy settings        |
| 1\<\<3 | MESSAGE_REQUEST_RESTRICTIONS_TOGGLE | Restricted from receiving message requests          |
| 1\<\<4 | LARGE_SERVER_ACCESS                 | Restricted from large guilds                        |
| 1\<\<5 | COMMANDS_TOGGLE                     | Restricted from using commands marked as NSFW       |
| 1\<\<6 | REACTIVE_CHECK                      | Generic age verification required flag              |
| 1\<\<7 | STAGE_SPEAKING                      | Restricted from speaking in stages                  |

###### Teen By Default Settings

| Value  | Name                                 | Description                                                   |
| ------ | ------------------------------------ | ------------------------------------------------------------- |
| 1\<\<0 | SENSITIVE_CONTENT                    | Restricts sensitive content settings                          |
| 1\<\<1 | FRIEND_REQUEST_STRANGER_CONFIRMATION | Restricts "everyone" option from friend request settings      |
| 1\<\<2 | MESSAGE_REQUEST_RESTRICTIONS         | Restricts toggling message requests                           |
| 1\<\<3 | GUILD_ACTIVITY_STATUS                | Restricts large server options from activity privacy settings |
| 1\<\<4 | SPAM_FILTERS                         | Restricts NSFW content options from spam filter settings      |
