...
| [Bulk Update Guild Subscriptions](#bulk-update-guild-subscriptions)                   | ...                                                               |
...
Used to update subscriptions for a guild. Accepts a partial [update guild subscription](#partial-update-guild-subscription-structure) object, with extra fields below.

###### Update Guild Subscriptions Extra Fields Structure

| Field    | Type      | Description         |
| -------- | --------- | ------------------- |
| guild_id | snowflake | The ID of the guild |

###### Partial Update Guild Subscription Structure

| Field                | Type                                           | Description                                                                                                                                                               |
| -------------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| typing?              | boolean                                        | Whether the current user is subscribed to the guild                                                                                                                       |
| threads?             | boolean                                        | Whether the current user is subscribed to the guild threads                                                                                                               |
| activities?          | boolean                                        | Whether the current user can receive embedded activity events                                                                                                             |
| members?             | array[snowflake]                               | Te IDs of the users whose member events should be sent for the current user                                                                                               |
| member_updates?      | boolean                                        | Whether the member events are sent                                                                                                                                        |
| channels? ^1^        | map[snowflake, array[array[integer, integer]]] | A mapping of channel IDs to ranges of member list items, containing the `(start, end)` arrays representing the indexes of chunk (max 5 channel subscriptions, 1-3 ranges) |
| thread_member_lists? | array[snowflake]                               | The IDs of the threads to receive member list for                                                                                                                         |

^1^ See [Range Parsing](/topics/guild-subscriptions#range-parsing) for details.

#### Request Forum Unreads

Used to request thread-only channel unread counts. Fires a [Forum Unreads](#forum-unreads) Gateway event.

###### Request Forum Unreads Structure

| Field      | Type                                                           | Description                        |
| ---------- | -------------------------------------------------------------- | ---------------------------------- |
| guild_id   | snowflake                                                      | The ID of the guild                |
| channel_id | snowflake                                                      | The ID of the channel              |
| threads    | [forum unread request](#forum-unread-request-structure) object | The threads to request unreads for |

#### Forum Unread Request Structure

| Field          | Type      | Description                             |
| -------------- | --------- | --------------------------------------- |
| thread_id      | snowflake | The ID of the thread                    |
| ack_message_id | snowflake | The ID of the last acknowledged message |
...
#### Bulk Update Guild Subscriptions

Used to update subscriptions for multiple guilds.

<Alert type="warn">

Due to 15 KiB limitation, this command must be fragmented in a specific way using the following pseudocode:

```py
import json

def fragment_guild_subscriptions(subscriptions):
  current_subscriptions = {}
  current_payload_length = 0

  for guild_id, subscriptions_ in subscriptions.items():
    payload_length = len(json.dumps([str(guild_id), subscriptions_], separators=(',', ':')))
    if (current_payload_length + payload_length) > 15360:
      yield current_subscriptions
      current_subscriptions.clear()
      current_payload_length = 0

    current_subscriptions[guild_id] = subscriptions_
...
| Field         | Type                                                                                                     | Description                                                 |
| ------------- | -------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| subscriptions | map[snowflake, partial [update guild subscription](#partial-update-guild-subscription-structure) object] | A mapping of guild IDs to their subscription update request |
...
| [Forum Unreads](#forum-unreads)                                                                   | Response to [Request Forum Unreads](#request-forum-unreads)                                       |
...
| [Thread Member List Update](#thread-member-list-update)                                           | Thread member list was updated                                                                    |
...
| [Guild Member List Update](#guild-member-list-update)                                             | Guild member list was updated                                                                     |
...
Sent in response to [Request Forum Unreads](#request-forum-unreads). Contains the counts of unread messages in threads.

###### Forum Unreads Structure
...
| channel_id        | snowflake                                                      | The ID of the channel                                                               |
| threads?          | [forum unread request](#forum-unread-request-structure) object | The threads to request unreads for                                                  |
| permission_denied | boolean                                                        | Whether the current user does not have permissions to view the threads-only channel |

#### Forum Unread Structure

| Field     | Type      | Description                  |
| --------- | --------- | ---------------------------- |
| thread_id | snowflake | The ID of the thread         |
| missing?  | integer   | Unknown                      |
| count?    | integer   | The count of unread messages |
...
Sent when the thread member list is updated. See [Guild Subscriptions](/topics/guild-subscriptions#thread-subscriptions) for more details.

###### Thread Member List Update Structure

| Field     | Type                                                                        | Description               |
| --------- | --------------------------------------------------------------------------- | ------------------------- |
| guild_id  | snowflake                                                                   | The ID of the guild       |
| thread_id | snowflake                                                                   | The ID of the thread      |
| members   | array[[thread member list item](#thread-member-list-item-structure) object] | The members in the thread |

###### Thread Member List Item Structure

| user_id | snowflake |
| member | [guild member](/resources/guild#guild-member-object) object |
| presence | [presence](/resources/presence#presence-object) object |
...
For user accounts, this event is only sent for themselves, members they are subscribed to, and in response to actions committed by the user. [Users are automatically subscribed](/topics/guild-subscriptions) to [friends and implicit relationships](/resources/relationships#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/gateway/using-gateway#gateway-capabilities)), as well as users they have an open DM with.
...
| -------- | ------------------------------------------ | ------------------- |
| guild_id | snowflake                                  | The ID of the guild |
| user ^1^ | [user](/resources/user#user-object) object | The removed user    |

^1^ If the object contains only `id`, then the event is a signal that the user is not a member of the guild. See [Guild Subscriptions](/topics/guild-subscriptions) for more details.
...
Sent when the guild member list is updated. See [Guild Subscriptions](/topics/guild-subscriptions) for more details.

###### Guild Member List Update Structure

| Field        | Type                                                                                | Description                                                             |
| ------------ | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| guild_id     | snowflake                                                                           | The ID of the guild                                                     |
| id ^1^       | string                                                                              | The [ID of the member list](/topics/guild-subscriptions#member-list-id) |
| member_count | integer                                                                             | Number of total members in the guild                                    |
| online_count | integer                                                                             | Number of total non-offline members in the guild                        |
| groups       | array[[guild member list group](#guild-member-list-group-structure) object]         | The groups                                                              |
| ops          | array[[guild member list operation](#guild-member-list-operation-structure) object] | The operations on the member list                                       |

###### Guild Member List Group Structure

| Field | Type               | Description                                                                                                                |
| ----- | ------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| id    | snowflake \|string | The ID of the role, or `online` / `offline` representing the online members without roles and offline members respectively |
| count | integer            | Number of total members with the role                                                                                      |

###### Guild Member List Operation Structure

| Field  | Type                                                                      | Description                                                | Operation Type               |
| ------ | ------------------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------- |
| op     | string                                                                    | The [type of operation](#guild-member-list-operation-type) | All                          |
| items? | array[[guild member list item](#guild-member-list-item-structure) object] | The items in the requested range                           | `SYNC`                       |
| range? | array[integer, integer]                                                   | The range that the operation is syncing or invalidating    | `SYNC`, `INVALIDATE`         |
| item?  | [guild member list item](#guild-member-list-item-structure) object        | The item that was added or updated                         | `INSERT`, `UPDATE`           |
| index? | integer                                                                   | The index of the item that was updated or removed          | `INSERT`, `UPDATE`, `DELETE` |

###### Guild Member List Item Structure

Only either `group` or `member` fields will be present.

| Field   | Type                                                                 | Description      |
| ------- | -------------------------------------------------------------------- | ---------------- |
| group?  | [guild member list group](#guild-member-list-group-structure) object | Item as a group  |
| member? | [guild member](/resources/guild#guild-member-object) object          | Item as a member |

###### Guild Member List Operation Type

| Value      | Description                     |
| ---------- | ------------------------------- |
| SYNC       | Member list is syncing          |
| INVALIDATE | A specific range is invalidated |
| INSERT     | Member list item is added       |
| UPDATE     | Member list item is updated     |
| DELETE     | Member list item is removed     |
