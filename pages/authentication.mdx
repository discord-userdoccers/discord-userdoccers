...
If the user's account is suspended by Discord, the login request will fail with a 403 forbidden with a special error response body, similar to a success response:
...
<RouteHeader method="POST" url="/auth/password/validate" unauthenticated>
