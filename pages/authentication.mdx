...
A verification token should be retrieved by [verifying the phone number](/topics/phone-verification#verify-phone).
...
If the user's account is suspended by Discord, the login request will fail with a special error response body, similar to a success response:
...
| theme  | string | The [client theme](/resources/user-settings#theme) selected by the user (only `dark` or `light` are returned) |
...
Then, the client must [verify the phone number](/topics/phone-verification#verify-phone) using the received code before completing the registration using the [Register Account](#register-account) endpoint as usual. {/* todo: link this here */}
...
Sends a verification code to the user's phone number to register an account. Returns a 204 empty response on success. The verification code should be first used to [verify the phone number](/topics/phone-verification#verify-phone) before [completing the registration](#register-account).
