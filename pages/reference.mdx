...
| Edge              | Microsoft Edge desktop       | [`web`](/resources/presence#client-status-object)      |
...
| Quest Asset ^5^                  | /assets/quests/[{quest_id}](/resources/quests#quest-object)/[{asset_name}](/resources/quests#quest-assets-structure).png, /assets/quests/[{quest_id}](/resources/quests#quest-object)/[{theme}](/resources/user/#client-theme)/[{asset_name}](/resources/quests#quest-assets-structure).png | PNG, JPEG, GIF, SVG, WebM |
...
^5^ The [theme](/resources/user/#client-theme) path is only used for fetching the [`game_tile` and `logotype` assets](/resources/quests#quest-assets-structure).
...
This endpoint has a high risk of abuse and should be used with caution. It may be subject to rate limiting, captchas, flagged accounts, which may result in [additional verification steps](/resources/user#required-action-type) or account termination.

You should **not** use this endpoint in a loop or in a way that will send more than 10 requests per 10 minutes.

### Known Detection

Discord uses many detections on this endpoint, such as:

#### Headers

You will need to include all the default headers that are sent by the Discord client or browser. These headers are used for analytics and client detection.

Some more headers may be required for specific endpoints:
- `X-Super-Properties` - This header is used to track client properties and is used for analytics and client detection.
- `X-Context-Properties` - This header is used to got the context of the request.

The `Referer` and `Origin` are also important headers that should be sent with the request as a context like the `X-Context-Properties` header.

#### Cookies

You will need all the cookies that are sent by Discord or Cloudflare on some endpoints such as [fingerprints](/authentication#fingerprints) and [location metadata](authentication#get-location-metadata).

You should have these 4 cookies before using this endpoint:
- `__dcfduid`
- `__sdcfduid`
- `_cfuvid`
- `__cfduid`

Some endpoints may also require the `__cf_bm` cookie or the `__stripe_mid` cookie.

#### TLS/SSL/HTTP

You should use the identical TLS/SSL fingerprints *(like ja3)* that are used by the Discord client or browser by sending a valid [client hello](https://www.cloudflare.com/learning/ssl/what-happens-in-a-tls-handshake).

This means that you need the same TLS/SSL version, cipher suites, elliptic curves, and extensions that are used by the Discord client or browser.

Lastly, you should use the same HTTP version that is used by the Discord client or browser.