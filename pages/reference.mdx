...
| Phone Number ^3^        | `<+PHONE_NUMBER>`              | `<+1 (555) 123 4567>`           |
...
^3^ Email and phone number markdown uses `mailto:` and `tel:` URI schemes respectively that can optionally be prefixed (e.g. `<mailto:nelly@discordapp.com>`). Email markdown supports headers, values must be [URL Encoded](https://en.wikipedia.org/wiki/Percent-encoding) (e.g. `<nelly@discordapp.com?subject=Message%20Title&body=Message%20Content>`).
