...
^5^ The [theme](/resources/user-settings#theme) path is only used for fetching the [`game_tile` and `logotype` assets](/resources/quests#quest-assets-structure).

^6^ This field is a CDN path that can be used to access the nameplate's assets. Valid asset names are limited to: `img.png`, `asset.webm`.
...
A file upload size limit applies to _all_ files in a request (rather than each individual file). While the limit depends on the [**premium tier (boost level)**](https://support.discord.com/hc/en-us/articles/360028038352-Server-Boosting-FAQ-#h_419c3bd5-addd-4989-b7cf-c7957ef92583) of a guild, it is 10 MiB by default.
