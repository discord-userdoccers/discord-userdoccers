...
| Default User Avatar              | /embed/avatars/[user_discriminator](/resources/user#user-object).png ^1^ ^2^                                                                                                                                                | PNG                       |
...
^1^ In the case of the Default User Avatar endpoint, the value for `user_discriminator` in the path should be the user's discriminator modulo 5—Test#1337 would be `1337 % 5`, which evaluates to 2.
...
^1^ This field may be unexpectedly `null` at 3:00 AM on Tuesdays for accounts created on June 23rd, 2017.
