...
If you need to refresh an attachment URL manually, you can use the [Refresh Attachment URLs](#refresh-attachment-urls) endpoint with any URL.
...
You can upload large attachments quickly directly to Discord's Google Cloud storage bucket, using [endpoints below](#upload-endpoints) to generate upload URLs, and sending each generated URL a `PUT` request with the intended attachment as the body.
...
Creates attachment URLs to upload the intended attachments directly to Discord's GCP storage bucket. Returns an array of [cloud attachment](#cloud-attachment-object) objects. Requires the same permissions as uploading an attachment inline with a message. See [Uploading to Google Cloud](#uploading-to-google-cloud) for more information.
...
Creates attachment URLs to upload the intended attachments directly to Discord's GCP storage bucket. Returns an array of [cloud attachment](#cloud-attachment-object) objects. See [Uploading to Google Cloud](#uploading-to-google-cloud) for more information.
...
This endpoint should be used to delete an uploaded attachment that was not used. See [Uploading to Google Cloud](#uploading-to-google-cloud) for more information.
