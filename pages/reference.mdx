...
| client_app_state? ^3^                           | string   | The [focus state](#client-app-state) of the client when the Gateway session was instantiated                                                                                    |
| client_launch_id?                               | string   | A client-generated UUID used to identify the client launch                                                                                                                      |
| client_heartbeat_session_id?                    | ?string  | A client-generated UUID representing the current persisted analytics heartbeat                                                                                                  |
...
| is_fast_connect? ^3^                            | boolean  | Whether the Gateway session sent the [identified](/topics/gateway#identifying) using fast connect (a preload script ran before the client was loaded)                           |
...
^3^ These properties are only sent when [identifying](/topics/gateway#identifying) with the Gateway and are not included in the `X-Super-Properties` header.
...
^3^ Can be optionally prefixed with `mailto:`, following the same format as the [`mailto:`](https://en.wikipedia.org/wiki/Mailto) URI scheme. This doesn't support multiple comma separated addresses.

^4^ Can be optionally prefixed with `tel:`/`sms:`, the `+` is optional when specifying a scheme and it doesn't adhere to the `tel:` URI scheme specification. Specifying headers with `sms:` is not supported
...
| Type                             | Path                                                                                                                                                                                                                                                                                 | Supports                                             |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| Activity Link Asset              | /attachments-quick-links/[{asset_path}](/resources/application#activity-link-object)                                                                                                                                                                                                 | Uploaded format ^4^                                  |
...
| Clan Badge                       | /clan-badges/[{guild_id}](/resources/guild#guild-object)/[{badge_hash}](/resources/discovery#guild-profile-object).png                                                                                                                                                               | PNG                                                  |
