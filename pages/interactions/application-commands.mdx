...
| file_types?                | array[string] ^2^                                                                               | The dotted file extensions permitted to be uploaded (max 10, only applicable for [`ATTACHMENT`](#application-command-option-type) options)                  |

^1^ This field is received only and cannot be set.

^2^ Supported values are the file group keywords `image`, `audio`, and `video`, or any dot-prefixed file extension such as `.pdf`.
It's recommended to use the provided file group keywords. If you specify individual extensions instead, you must include `.jpg` for
image uploads and both `.mp4` and `.mov` for video uploads, as mobile clients may otherwise reject valid files.
