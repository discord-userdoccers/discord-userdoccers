...
`CHAT_INPUT` command names and command option names must match the regex `^[-_'\p{L}\p{N}\p{sc=Deva}\p{sc=Thai}]{1,32}$` with the unicode flag set. If there is a lowercase variant of any letters used, you must use those.
...
<Collapsible title="Making a Global Command" description="Simple examples of making a global command" icon="code">
...
<Collapsible title="Making a Guild Command" description="Simple examples of making a guild command" icon="code">
...
The value for `default_member_permissions` is a bitwise OR-ed set of [permissions](/docs/topics/permissions#permissions-bitwise-permission-flags), serialized as a string.
...
<RouteHeader method="GET" url="/applications/{application.id}/application-command-index">
  Get Bot Application Command Index
</RouteHeader>

Returns an [application command index](#application-command-index-object) object for the given application ID, containing the commands available to use in the application's bot DMs.

<RouteHeader method="GET" url="/channels/{channel.id}/application-command-index">
  Get Channel Application Command Index
</RouteHeader>

Returns an [application command index](#application-command-index-object) object for the given group DM ID.
...
Returns an [application command index](#application-command-index-object) object for the current user, containing the commands available from applications installed to the user's account.
...
Modifies permissions for a specific application command in a guild. Returns a [guild application command permissions](#guild-application-command-permissions-structure) object on success. You can only add up to 10 permission overwrites for a command.
