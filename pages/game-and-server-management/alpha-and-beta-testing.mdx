...
Role-based entitlement will only work if you use Discord's DRM, which is a simple entitlement check. You can do that with the Dispatch command [`dispatch build drm-wrap`](/dispatch/list-of-commands#build-drmwrap) or by calling [`ApplicationManager.ValidateOrExit()`](/game-sdk/applications#validateorexit). Otherwise, Discord will not check the user's entitlement when the game starts.
