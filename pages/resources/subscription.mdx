...
<RouteHeader method="GET" url="/users/@me/billing/subscriptions/${e}/eligible-users">
  Get Subscription Eligible Users
</RouteHeader>

Returns the users eligible to participate in a premium group.

###### Query String Parameters

| Field               | Type    | Description                                          |
| ------------------- | ------- | ---------------------------------------------------- |
| index?              | integer | The current search index (0-2000)                    |
| limit?              | integer | Max numbers of users to return (max 50, default 10)  |
| search_query?       | string  | Query to match usernames against (max 32 characters) |
| include_ineligible? | boolean | Whether to include ineligible users                  |
...
<RouteHeader method="POST" url="/users/@me/billing/subscriptions/{subscription.id}/invites">
  Create Subscription Invites
</RouteHeader>

Creates multiple subscription invites for the specified users.

###### JSON Params

| Field    | Type             | Description                          |
| -------- | ---------------- | ------------------------------------ |
| user_ids | array[snowflake] | The IDs of the users to invite (1-5) |

###### Response Body

| Field            | Type             | Description                                        |
| ---------------- | ---------------- | -------------------------------------------------- |
| invited_users    | array[snowflake] | The IDs of the invited users                       |
| ineligible_users | array[snowflake] | The IDs of the users ineligible for the experiment |

<RouteHeader method="PATCH" url="/users/@me/billing/subscriptions/{subscription.id}/members/{user.id}/invite">
  Accept Subscription Invite
</RouteHeader>

Accepts a subscription invite. Returns a 204 empty response. The current user must belong to the subscription.

<RouteHeader method="DELETE" url="/users/@me/billing/subscriptions/{subscription.id}/members/{user.id}/invite">
  Reject Subscription Invite
</RouteHeader>

Rejects a subscription invite. Returns a 204 empty response. The current user must belong to the subscription.

<RouteHeader method="DELETE" url="/users/@me/billing/subscriptions/{subscription.id}/members/{user.id}">
  Remove Subscription Member
</RouteHeader>

Removes a subscription member. Returns a 204 empty response. The current user must belong to the subscription.

<RouteHeader metohd="GET" url="/users/@me/billing/subscriptions/{subscription.id}/members">
  Get Subscription Members
</RouteHeader>

Returns a list of [subscription member](#subscription-member-structure) objects for the given subscription ID.

###### Subscription Member Structure

| Field       | Type                                               | Description                                                                        |
| ----------- | -------------------------------------------------- | ---------------------------------------------------------------------------------- |
| user        | partial [user](/resources/user#user-object) object | The user this subscription member represents                                       |
| member_type | integer                                            | The [type](/resources/user#premium-subscription-group-role) of subscription member |
| accepted_at | ?ISO8601 timestamp                                 | When the user was accepted                                                         |
