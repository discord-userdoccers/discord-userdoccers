...
^3^ Not present when the channel is fetched from an [invite](/resources/invite#invite-object), as it can be inferred from the `guild_id` field.
...
For user accounts, the channel list will only include channels the user has the `VIEW_CHANNEL` permission for.
