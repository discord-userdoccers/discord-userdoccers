...
Only channels to be modified are required. The following logic is recommended when modifying channels, but it is not required or enforced.

- When moving categories, you should include every category in the guild.
- When moving uncategorized channels, you should include every uncategorized channel in the guild.
- When moving channels within the same category, you should include every channel in that category.
- When moving a channel out of a category, you should include every uncategorized channel in the guild. `parent_id` should be set to `null`.
- When moving a channel into a category or to a different category, you should include every channel in the target category. `parent_id` should be set to the target category's ID.

</Alert>

<Alert type="info">

In the Discord client, voice and stage channels are always displayed below other channel types. This constraint is not enforced by the API, meaning special care must be taken in order to prevent any unexpected behavior.

</Alert>

<Alert type="info">

Channels with the same position are sorted by their ID in ascending order.
...
<RouteHeader method="PUT" url="/channels/{channel.id}/tags/{forum_tag.id}" supportsAuditReason supportsBot>
  Modify Channel Tag
</RouteHeader>
...
<RouteHeader method="DELETE" url="/channels/{channel.id}/tags/{forum_tag.id}" supportsAuditReason supportsBot>
  Delete Channel Tag
</RouteHeader>
