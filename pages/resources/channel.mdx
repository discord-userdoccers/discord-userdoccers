...
Only channels to be modified are required. The following conventions are recommended when modifying channels, but they are not required or enforced by the API.

- When moving a category, you should include every category in the guild.
- When moving an uncategorized channel, you should include every uncategorized channel in the guild.
- When moving a channel within the same category, you should include every channel in that category.
- When moving a channel out of a category, you should include every uncategorized channel in the guild. The `parent_id` field should be set to `null`.
- When moving a channel into a category or to a different category, you should include every channel in the target category. The `parent_id` field should be set to the target category's ID.
