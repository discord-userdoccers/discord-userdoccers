...
Update a channel's settings. Returns a [channel](#channel-object) on success. Fires a [Channel Update](/topics/gateway#channel-update) or [Thread Update](/topics/gateway#channel-update) Gateway event.
...
| flags?                              | integer                                              | The [channel's flags](#channel-flags)                                                                                                                                                | Text, News, Voice, Stage, Forum                             |
...
Delete a channel's read state for the current user. Returns a 204 empty response on success.
...
Create a new [invite](/resources/invite#invite-object) object for the channel. Only usable for guild channels and group DMs. Requires the `CREATE_INSTANT_INVITE` permission if the channel is in a guild. Returns an [invite](/resources/invite#invite-object) object (with [invite metadata](/resources/invite#invite-metadata-object)). Fires an [Invite Create](/topics/gateway#invite-create) Gateway event if the channel is in a guild.
...
Follow a News Channel to send messages to a target channel. Requires the `MANAGE_WEBHOOKS` permission in the target channel. Returns a [followed channel](/resources/message#followed-channel-object) object. Fires a [Webhooks Update](/topics/gateway#webhooks-update) Gateway event for the target channel.
...
Post a typing indicator for the specified channel. Returns a 204 empty response on success. Fires a [Typing Start](/topics/gateway#typing-start) Gateway event.
...
Modify the active call in a channel. Returns a 204 empty response on success. Fires a [Call Update](/topics/gateway#call-update) Gateway event.
...
| Field        | Type                                                       | Description                                                                               |
| ------------ | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| flags?       | integer                                                    | The user's [thread flags](#thread-member-object) flags; all except the first are settable |
| muted?       | boolean                                                    | Whether the user has muted the thread                                                     |
| mute_config? | ?[mute config](/resources/guild#mute-config-object) object | The mute metadata for the thread                                                          |
