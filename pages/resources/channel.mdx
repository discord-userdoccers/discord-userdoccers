...
| is_message_request?                 | boolean                                                            | Whether the channel is a message request                                                                                                                                                     |
| is_message_request_timestamp?       | ?ISO8601 timestamp                                                 | The message request creation timestamp                                                                                                                                                       |
| is_spam?                            | boolean                                                            | Whether the channel is a spam message request                                                                                                                                                |
...
^2^ Only returned when part of the `resolved` data received in a slash command interaction or when `permissions` is set to `true` when fetching [Get Guild Channels](#get-guild-channels).
...
^4^ Only included in the [Gateway Guild](/topics/gateway-events#gateway-guild-object) object.
...
| ~~1 << 10~~ | ~~MEDIA_CHANNEL~~ ^1^                      | ~~This forum channel is a media channel~~                                                                                                                       |
...
| 1 << 16     | JOIN_REQUEST_INTERVIEW_CHANNEL             | This group DM is used for [guild join request interviews](https://support.discord.com/hc/en-us/articles/23187611406999-Guilds-FAQ#h_01HXW2MCD0BT70FB9SRV1YRPBV) |
...
<RouteHeader method="GET" url="/users/@me/dms/{user.id}">
  Get DM Channel
</RouteHeader>
...
You should not use this endpoint to create multiple new DMs in a short period of time. A DM is only counted as created if the user sends the first message in the DM and the channel did not have existing message history. Users may only create 10 new DMs to non-bot users in a 10-minute window.
...
<RouteHeader method="GET" url="/guilds/{guild.id}/top-read-channels">
...
<RouteHeader method="POST" url="/channels/{channel.id}/call/stop-ringing">
...
Returns a [DM channel](#channel-object) on success. Fires a [Channel Update](/topics/gateway-event#channel-update) Gateway event.

<Alert type="warn">

All consent statuses other than `ACCEPTED` are only usable by [Discord staff](/resources/user#user-flags).
...
Returns a [DM channel](#channel-object) on success. Fires [Channel Update](/topics/gateway-events#channel-update), [Message ACK](/topics/gateway-events#message-ack), [Channel Delete](/topics/gateway-events#channel-delete), and has a chance to send [DM Settings Upsell Show](/topics/gateway-events#dm-settings-upsell-show) Gateway event.
