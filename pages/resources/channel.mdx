...
\* These fields are omitted on the member sent within each thread in the [Guild Create](/topics/gateway-events#guild-create) event.
...
Creates a new [invite](/resources/invite#invite-object) object for the channel. Only usable for guild channels and group DMs. Requires the `CREATE_INSTANT_INVITE` permission if the channel is in a guild. Returns an [invite](/resources/invite#invite-object) object (with [invite metadata](/resources/invite#invite-metadata-object)). Fires an [Invite Create](/topics/gateway-events#invite-create) Gateway event if the channel is in a guild.
...
Modifies the active call in a channel. Returns a 204 empty response on success. Fires a [Call Update](/topics/gateway-events#call-update) Gateway event.
...
Rings the recipients of a channel to notify them of an active call. Returns a 204 empty response on success. Fires a [Call Update](/topics/gateway-events#call-update) Gateway event.
...
Stops ringing the recipients of a channel about an active call. Returns a 204 empty response on success. Fires a [Call Update](/topics/gateway-events#call-update) Gateway event.
...
Creates a new tag in the forum channel. Requires the `MANAGE_CHANNELS` permission. Returns a [channel](#channel-object) object on success. Fires a [Channel Update](/topics/gateway-events#channel-update) Gateway event.
...
Replaces a tag in the forum channel. Requires the `MANAGE_CHANNELS` permission. Returns a [channel](#channel-object) object on success. Fires a [Channel Update](/topics/gateway-events#channel-update) Gateway event.
...
Deletes a tag in the forum channel. Requires the `MANAGE_CHANNELS` permission. Returns a [channel](#channel-object) object on success. Fires a [Channel Update](/topics/gateway-events#channel-update) Gateway event.
