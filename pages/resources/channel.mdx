...
^4^ Only returned when part of the `resolved` data received in a slash command interaction or when `permissions` is set to `true` when fetching [Get Guild Channels](#get-guild-channels).

^5^ Only included in the [Gateway Guild](/topics/gateway-events#gateway-guild-object) object.
...
| available_tags? ^3^                 | ?array[partial [tag](#forum-tag-object) object]                     | The tags that can be used in a thread-only channel (max 20)                                                                                                                          | Forum, Media                           |
