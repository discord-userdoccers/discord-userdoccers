...
| 1 << 17     | OBFUSCATED ^2^                             | This channel is unavailable for the current user                                                                                                                |

^1^ Media channels are now represented by the [`GUILD_MEDIA` channel type](#channel-type).

^2^ The channel's name is always returned as `___hidden___`.
...
| mute_config? ^2^ | ?[mute config](#mute-config-object) object                  | The mute metadata for the thread                 |
...
Follows a News Channel to send messages to a target channel. Requires the `MANAGE_WEBHOOKS` permission in the target channel. Returns a [followed channel](/resources/message#followed-channel-object) object on success. Fires a [Webhooks Update](/topics/gateway-events#webhooks-update) Gateway event for the target channel.
...
Modifies a message request's status. Returns a [DM channel](#channel-object) object on success. Fires a [Channel Update](/topics/gateway-event#channel-update) Gateway event.
...
Rejects and deletes a pending message request. Returns a [DM channel](#channel-object) object on success. Fires [Channel Update](/topics/gateway-events#channel-update), [Message ACK](/topics/gateway-events#message-ack), [Channel Delete](/topics/gateway-events#channel-delete), and optionally [DM Settings Upsell Show](/topics/gateway-events#dm-settings-upsell-show) Gateway events.

<RouteHeader method="PUT" url="/channels/recipients/@me/batch-reject">
  Batch Reject Message Requests
</RouteHeader>

Rejects and deletes multiple pending message requests. Returns a list of [DM channel](#channel-object) objects on success. Fires multiple [Channel Update](/topics/gateway-events#channel-update), [Message ACK](/topics/gateway-events#message-ack), [Channel Delete](/topics/gateway-events#channel-delete), and optionally [DM Settings Upsell Show](/topics/gateway-events#dm-settings-upsell-show) Gateway events.
...
| mute_config? | ?[mute config](#mute-config-object) object | The mute metadata for the thread                                                         |
