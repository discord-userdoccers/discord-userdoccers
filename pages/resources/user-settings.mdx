...
| volume           | float   | Volume level of the user or stream (min 0, max 200) |
...
<RouteHeader method="PATCH" url="/users/@me/audio-settings/{audio_context}/{user.id}" supportsOAuth2="voice">
  Modify Audio Settings
</RouteHeader>

Modifies user's audio settings. Returns a 204 empty response on success. Fires a [Audio Settings Update](/topics/gateway-events#audio-settings-update) Gateway event.
