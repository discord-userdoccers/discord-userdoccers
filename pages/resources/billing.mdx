...
| email?       | string  | The email address for the billing address |
...
| 1 \<\< 4 | FAILED_ADDRESS     | Billing address validation failed for the payment source |
...
| default?               | boolean                                              | Whether the payment source should be the default payment source                                       |
...
<RouteHeader method="POST" url="/users/@me/billing/stripe/payment-elements/setup-intents">
  Create Stripe Payment Elements Setup Intent
</RouteHeader>

Creates a Stripe SetupIntent for Stripe Payment Elements and returns the client secret and custom payment methods.
...
| ---------------------- | ------------------------------------------------------------------------------------- | ------------------------------------------ |
| client_secret          | string                                                                                | The client secret of the SetupIntent       |
| custom_payment_methods | array[[Stripe custom payment method](#stripe-custom-payment-method-structure) object] | Custom payment methods for Stripe Elements |

###### Stripe Custom Payment Method Structure

| Field                    | Type    | Description                                                                   |
| ------------------------ | ------- | ----------------------------------------------------------------------------- |
| custom_payment_method_id | string  | The ID of the custom payment method                                           |
| payment_source_type      | integer | The [payment source type](#payment-source-type) for the custom payment method |
