...
^1^ Providing a valid [friend token](/resources/user#get-friend-token) will bypass the user's friend request settings.
...
Sends a game friend request to another user, which can be accepted in-game by creating a new game relationship of type `FRIEND`, or by the user using [Accept Game Relationship](#accept-game-relationship).
...
Creates a game relationship with another user. Returns a 204 empty response on success.
...
Accepts a game relationship from another user on a specific application. Returns a 204 empty response on success.
...
Removes a game relationship with another user. Returns a 204 empty response on success.
