...
| ----- | ---------------- | --------------------------------------------------------------------------------- |
| 1     | KEYWORD          | When content contains words from a user defined list of keywords (max 6)          |
| ~~2~~ | ~~HARMFUL_LINK~~ | ~~When content contains any harmful links (max 1)~~                               |
| 3     | SPAM             | When content represents generic spam (max 1)                                      |
| 4     | KEYWORD_PRESET   | When content contains words from internal predefined wordsets (max 1)             |
| 5     | MENTION_SPAM     | When content contains more unique mentions than allowed (max 1)                   |
...
| 4     | QUARANTINE_USER ^3^    | Quarantine user indefinitely; quarantining a user, similar to timing out, prevents them from interacting with the guild in any way                      |
