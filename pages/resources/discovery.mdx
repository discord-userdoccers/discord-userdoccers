...
| primary_category_id        | snowflake                                                     | The ID of the primary [discovery category](#discovery-category-object) set for the guild         |
...
| social_links? ^2^          | ?array[string]                                                | The guild's social media links shown in the discovery web page (max 9)                           |
| about? ^2^                 | ?string                                                       | The guild's long description shown in the discovery web page (max 120 characters)                |
...
Represents a guild's path towards meeting the requirements of joining discovery.
...
^1^ Certain guilds, such as those that are [verified](https://discord.com/verification), are exempt from discovery requirements. These guilds will not have a fully populated discovery requirements object (guaranteed to receive only `sufficient` and `sufficient_without_grace_period`).
...
| channel_banned_keywords?     | map[snowflake, array[string]] | The disallowed terms found in the channel names                |
...
| partner_actioned_timestamp    | ?ISO8601 timestamp                                            | When the guild's partner application was actioned by staff                                 |
...
Returns an array of [discoverable guild](#discoverable-guild-object) objects representing the guilds that are available for the current user to discover.
...
Returns an array of [discoverable guild](#discoverable-guild-object) objects representing the guilds that match the query. This endpoint is a proxy for [searching using the Algolia API](#searching-discovery). See the [Algolia Search documentation](https://www.algolia.com/doc/rest-api/search/) for more information.
...
Returns information about a guild's landing web page or monetization store page for the given guild ID. This endpoint requires the guild to be indexable (be discoverable and [published](#discovery-metadata-object) OR have the `CREATOR_STORE_PAGE` [guild feature](/resources/guilds#guild-features)).
...
| store_page? | [monetization store page](#monetization-promo-page-structure) object | The guild's monetization store page, if enabled                                                                 |
...
| approximate_member_count   | integer                                                  | Approximate number of members in the guild                                            |
| approximate_presence_count | integer                                                  | Approximate number of online members in the guild                                     |
| locked_server              | boolean                                                  | Whether the guild is not publicly accessible                                          |
...
Returns an array of [discovery category](#discovery-category-object) objects representing the available discovery categories.
