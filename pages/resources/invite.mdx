...
| is_nickname_changeable?         | boolean                                                                                       | Whether the @everyone role has the `CHANGE_NICKNAME` permission in the guild this invite is for                              |

^1^ Only included when fetched from the [Get Invite](#get-invite) endpoint with `with_counts` set to `true`.

^2^ Only included when fetched from the [Accept Invite](#accept-invite) endpoint. Note that this is erroneously set to `true` for non-guild invites.
