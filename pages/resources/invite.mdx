...
Returns the IDs of the users allowed to see and accept the given invite. Invite must have a target users file associated with it. Requires the `MANAGE_GUILD` permission if the requestor is not the inviter. Returns the CSV file contents as uploaded.
...
Returns the status of the target users file processing job for the given invite. Invite must have a target users file associated with it. Requires the `MANAGE_GUILD` permission if the requestor is not the inviter.
