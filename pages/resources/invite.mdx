...
| roles?                          | array[[partial role](#invite-role-object) object]                                             | The roles to grant to the invitee upon acceptance                                                                    |
...
^4^ Only included when fetched from the [Get Guild Invites](#get-guild-invites) endpoint.
...
The role from an invite.

###### Invite Role Structure
...
| colors                 | [role colors](/resources/guild#role-colors-structure) object | The colors of the role encoded as an integer representation of hexadecimal color codes     |
...
Returns the IDs of the users allowed to see and accept the given invite. Invite must have a target users file associated with it. Requires the `MANAGE_GUILD` or `VIEW_AUDIT_LOG` permission if the requestor is not the inviter. Returns the CSV file contents as uploaded.
