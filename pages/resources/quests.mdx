...
| reward_redemption_instructions_by_platform? | map[int, string] | The instrutions on redeeming the reward [per-platform](#quest-platform-type) |
...
Represents the version of the quest configuration.
...
Represents areas where the quest can be shown in the Discord client.
...
Represents a behavioral variant for a quest.
...
Represents dismissed [quest content areas](#quest-content-type).
...
Represents an object that holds the quest's reward code.
...
Quests may only be completed from desktop clients. If the requesting user-agent does not include an `Electron/` version, the request will fail with a 401 unauthorized error.
...
^1^ The stream must have at least 1 viewer to be eligible for quest progress.
