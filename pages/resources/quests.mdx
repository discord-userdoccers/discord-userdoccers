...
| entitlement_expiration_metadata | map[snowflake, [entitlement expiration metadata](#entitlement-expiration-metadata-structure) object] | The expiration metadata for the entitlements      |
...
| excluded_quests | array[[quest](#quest-object) object] | The quests that the user cannot participate in |
...
Claims the quest's rewards, setting the `completed_at` and `claimed_at` fields of the [quest user status](#quest-user-status-object) to the current timestamp. Returns a [quest claimed reward](#quest-claimed-reward-object) object on success.
...
Returns a [quest user status](#quest-user-status-object) object on success.
...
Starts completing a quest on console.
...
| quest_user_status | ?[quest user status](#quest-user-status-object) object         | The user's quest status                           |
...
Stops completing a quest on console. Returns a 204 empty response on success.
