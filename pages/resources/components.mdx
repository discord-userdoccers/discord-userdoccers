...
To use these components, you need to send the [message flag](/resources/message#message-flags) `1 << 15` (`IS_COMPONENTS_V2`) which can be sent on a per-message basis. This enables the new components system with the following changes:
...
[Legacy component behavior](#legacy-message-component-behavior) will continue to work but provide less flexibility and control over the message layout.
...
Components are a field on the [message object](/resources/message#message-object) and [modal](/interactions/receiving-and-responding#interaction-response-object-modal). You can use them when creating messages or responding to an interaction, like an [application command](/interactions/application-commands).
...
| ------ | --------------------------------------------------- | ---------------------------------------------------------- | ----------- | -------------- |
| 1      | [Action Row](#action-rows)                          | A container for other components                           | Layout      | Message, Modal |
| 2      | [Button](#buttons)                                  | A button object                                            | Interactive | Message        |
| 3      | [String Select](#string-select)                     | Select menu for picking from defined text options          | Interactive | Message        |
| 4      | [Text Input](#text-inputs)                          | Text input object                                          | Interactive | Modal          |
| 5      | [User Select](#select-menus)                        | Select menu for users                                      | Interactive | Message        |
| 6      | [Role Select](#select-menus)                        | Select menu for roles                                      | Interactive | Message        |
| 7      | [Mentionable Select](#select-menus)                 | Select menu for mentionables (users *and* roles)           | Interactive | Message        |
| 8      | [Channel Select](#select-menus)                     | Select menu for channels                                   | Interactive | Message        |
| 9 ^1^  | [Section](#section)                                 | Container to display text alongside an accessory component | Layout      | Message        |
| 10 ^1^ | [Text Display](#text-display)                       | Markdown text                                              | Content     | Message        |
| 11 ^1^ | [Thumbnail](#thumbnail)                             | Small image that can be used as an accessory               | Content     | Message        |
| 12 ^1^ | [Media Gallery](#media-gallery)                     | Display images and other media                             | Content     | Message        |
| 13 ^1^ | [File](#file)                                       | Displays an attached file                                  | Content     | Message        |
| 14 ^1^ | [Separator](#separator)                             | Component to add vertical padding between other components | Layout      | Message        |
| 16 ^2^ | [Content Inventory Entry](#content-inventory-entry) | Displays an activity feed entry                            | Content     | Message        |
| 17 ^1^ | [Container](#container)                             | Container that visually groups a set of components         | Layout      | Message        |

^1^ Requires [message flag](/resources/message#message-flags) `1 << 15` (`IS_COMPONENTS_V2`).
...
| components | array of [component objects](#component-types) | Up to 5 interactive [button](#button) components or a single [select](#string-select) component |
...
String Selects can be configured for both single-select and multi-select behavior. When a user finishes making their choice(s) your app receives an [interaction](/interactions/receiving-and-responding#interaction-object-interaction-structure).
...
| options      | array of [select options](#select-option-structure) | Specified choices in a select menu; max 25                                 |
...
When defining a text input component, you can set attributes to customize the behavior and appearance of it. However, not all attributes will be returned in the [text input interaction payload](#text-input-text-input-interaction).
...
User Selects can be configured for both single-select and multi-select behavior. When a user finishes making their choice(s) your app receives an [interaction](/interactions/receiving-and-responding#interaction-object-interaction-structure).
...
Role Selects can be configured for both single-select and multi-select behavior. When a user finishes making their choice(s) your app receives an [interaction](/interactions/receiving-and-responding#interaction-object-interaction-structure).
...
Mentionable Selects can be configured for both single-select and multi-select behavior. When a user finishes making their choice(s), your app receives an [interaction](/interactions/receiving-and-responding#interaction-object-interaction-structure).
...
Channel Selects can be configured for both single-select and multi-select behavior. When a user finishes making their choice(s) your app receives an [interaction](/interactions/receiving-and-responding#interaction-object-interaction-structure).
...
| id?             | integer                                                           | Optional identifier for component                                                                                                                        |
| custom_id       | string                                                            | ID for the select menu; max 100 characters                                                                                                               |
| channel_types?  | array of [channel types](/resources/channel#channel-type)         | List of channel types to include in the channel select component                                                                                         |
| placeholder?    | string                                                            | Placeholder text if nothing is selected; max 150 characters                                                                                              |
| default_values? | array of [default value objects](#select-default-value-structure) | List of default values for auto-populated select menu components; number of default values must be in the range defined by `min_values` and `max_values` |
| min_values?     | integer                                                           | Minimum number of items that must be chosen (defaults to 1); min 0, max 25                                                                               |
| max_values?     | integer                                                           | Maximum number of items that can be chosen (defaults to 1); max 25                                                                                       |
...
| id?        | integer                                                           | Optional identifier for component                                                                 |
| components | array of [text display components](#text-display)                 | One to three text components                                                                      |
| accessory  | [thumbnail component](#thumbnail) or [button components](#button) | A thumbnail or a button component, with a future possibility of adding more compatible components |

<Alert type="info">

Don't hardcode `components` to contain only text components. Other components may be added in the future. Similarly, `accessory` may be expanded to include other components in the future.
...
| id?          | integer                                               | Optional identifier for component                                               |
| media        | [unfurled media item](#unfurled-media-item-structure) | A url or attachment                                                             |
...
Check out the [Section example](#section-example) to see the thumbnail in use as an accessory.
...
| id?   | integer                                                       | Optional identifier for component |
| items | array of [media gallery items](#media-gallery-item-structure) | Maximum of 10 items               |
...
A File is a top-level component that allows you to display an [uploaded file](#uploading-a-file) as an attachment to the message and reference it in the component. Each file component can only display 1 attached file, but you can upload multiple files and add them to different file components within your payload. This is similar to the `embeds` field of a message but allows you to control the layout of your message by using this anywhere as a component. Files are only available in messages.
...
| id?      | integer                                               | Optional identifier for component                                                                                                |
| file     | [unfurled media item](#unfurled-media-item-structure) | This unfurled media item is unique in that it **only** supports attachment references using the `attachment://<filename>` syntax |
| spoiler? | boolean                                               | Whether the media should be a spoiler (or blurred out). Defaults to `false`                                                      |
...
| author_type   | integer                                                                   |                                                                                |
| content_type  | integer                                                                   | [Content type](#content-inventory-content-types)                               |
...
| extra         | [Content Inventory metadata](#content-inventory-metadata-object) object   | Metadata, such as game or song                                                 |
...
###### Content Inventory Content Types
...
| Field                    | Type              | Description                                                               |
| ------------------------ | ----------------- | ------------------------------------------------------------------------- |
| type                     | integer           | [Trait type](#content-inventory-trait-types)                              |
| first_time?              | boolean           | Shows the "New player" text (only type `1`)                               |
| duration_seconds?        | integer           | Total time elapsed during this entry (only type `2`)                      |
| is_live?                 | boolean           | Entry is still ongoing (only type `3`)                                    |
| range?                   | integer           | Time range (`0` unspecified, `1` week) (only type `4`)                    |
| resurrected_last_played? | ISO8601 timestamp | Timestamp of when the game for this entry was last played (only type `5`) |
| marathon?                | boolean           | Shows the "#h marathon" text (only type `6`)                              |
| streak_count_days?       | integer           | Number of days for the streak text (only type `8`)                        |
| trending?                | integer           | Trending type (`0` unspecified, `1` global) (only type `9`)               |
| count?                   | integer           | Total Count (only types `10`, `11`, `12`)                                 |

###### Content Inventory Trait Types

| Value | Description                     |
| ----- | ------------------------------- |
| `0`   | Unspecificed                    |
| `1`   | First time ("New Player")       |
| `2`   | Duration                        |
| `3`   | Is Live                         |
| `4`   | Aggregate Range                 |
| `5`   | Resurrected ("Returning after") |
| `6`   | Marathon                        |
| `7`   | New Release                     |
| `8`   | Streak                          |
| `9`   | Trending                        |
| `10`  | Top Item Total Count            |
| `11`  | Top Parent Item Total Count     |
| `12`  | Aggregate Count                 |
...
| Field                     | Type                                                                     | Description                                                                                        |
| ------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| type                      | string                                                                   | [Metadata type](#content-inventory-metadata-types)                                                 |
| game_name?                | string                                                                   | Name of the game (`played_game_extra` only)                                                        |
| application_id?           | snowflake                                                                | ID of the associated application                                                                   |
| platform?                 | integer                                                                  | [Platform type](#content-inventory-platform-types) (`played_game_extra` only)                      |
...
| provider?                 | integer                                                                  | (`0` unspecified, `1` spotify) (`listened_media_extra` only)                                       |
| media_type?               | integer                                                                  | Type of this media (`0` top artist, `1` track) (`listened_media_extra` only)                       |
| parent_title?             | string                                                                   | Title of this media's parent (album title when `media_type == 1`) (`listened_media_extra` only)    |
| title?                    | string                                                                   | Title of this media (`listened_media_extra` only)                                                  |
| image_url?                | string                                                                   | Image of this media (e.g. album art) (`listened_media_extra` only)                                 |
| artist?                   | [artist](#content-inventory-artist-object) object                        | Top listened artist (`top_artist_extra` only)                                                      |
| artists?                  | array[[artist](#content-inventory-artist-object) object]                 | Artists for this media (empty when `top_artist_extra` media) (`listened_media_extra` only)         |
| external_id?              | string                                                                   | External platform ID for this media (e.g. Spotify track ID) (`listened_media_extra` only)          |
| external_parent_id?       | string                                                                   | External platform ID for this media's parent (e.g. Spotify album ID) (`listened_media_extra` only) |
| media_assets_large_image? | string                                                                   | Image of this media (e.g. movie poster) (`watched_media_extra` only)                               |
| media_assets_small_image? | string                                                                   | Small image (e.g. platform logo) (`watched_media_extra` only)                                      |
| media_assets_large_text?  | string                                                                   | Large text (e.g. season and episode of a show) (`watched_media_extra` only)                        |
| media_assets_small_text?  | string                                                                   | Small text (e.g. platform name) (`watched_media_extra` only)                                       |
| media_title?              | string                                                                   | Title of this media (`watched_media_extra` only)                                                   |
| media_subtitle?           | string                                                                   | Subtitle of this media (`watched_media_extra` only)                                                |
| url?                      | string                                                                   | URL of this media (`watched_media_extra` only)                                                     |
| activity_name?            | string                                                                   | Name of the activity (`launched_activity_extra` only)                                              |

###### Content Inventory Metadata Types
...
###### Content Inventory Platform Types
...
To upload a file with your message, you'll need to send your payload as `multipart/form-data` (rather than `application/json`) and include your file with a valid filename in your payload. Details and examples for uploading files can be found in the [API Reference](/reference#uploading-files).
