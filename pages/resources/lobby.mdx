...
Updates lobby message's moderation metadata. Accepts a moderation metadata mapping, up to 5 keys, max 2000 characters per value. Returns a 204 empty response. Fires a [Lobby Message Update](/topics/gateway-events#lobby-message-update) or [Message Update](/topics/gateway-events#message-update) Gateway event.
