...
| connected ^1^ | boolean              | Whether the member is connected to a call in lobby       |

^1^ Only included in guild objects returned [over the Gateway](/topics/gateway-events#lobbies).

###### Lobby Member Flags

| Value  | Name           | Description                                     |
| ------ | -------------- | ----------------------------------------------- |
| 1 << 0 | CAN_LINK_LOBBY | Lobby member can link a text channel to a lobby |
...
| poll?                         | [poll](/resources/message#poll-object) object                                                      | A poll!                                                                                                                                   |
...
^4^ See the [message reference types](/resources/message#message-reference-type) for more information on which message references have which fields.
...
| Field                 | Type                 | Description                                                                                |
| --------------------- | -------------------- | ------------------------------------------------------------------------------------------ |
| secret                | string               | The lobby secret (max 250)                                                                 |
...
If the user is already a member of the lobby, this will update the provided fields and fire a [Lobby Member Update](/topics/gateway-events#lobby-member-update) Gateway event instead.
...
Posts a message to a lobby. Returns a partial [lobby message](#partial-lobby-message-object) object on success. Fires a [Lobby Message Create](/topics/gateway-events#lobby-message-create) and [Message Create](/topics/gateway-events#message-create) Gateway event.
