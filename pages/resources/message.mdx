...
| referenced_message? ^4^ | ?[message object](#message-object)                                                                 | The message associated with the `message_reference`                                                                                       |
...
^4^ Only returned for messages with a `type` of `REPLY` or `THREAD_STARTER_MESSAGE`. If the message is a reply but the `referenced_message` field is not present, the backend did not attempt to fetch the message that was being replied to, so its state is unknown. If the field exists but is `null`, the referenced message was deleted.
...
| 40     | GUILD_DEADCHAT_REVIVE_PROMPT                 | A message sent when no one sends a message in the current channel for 1 hour | "{author} reported a false alarm in {guild}."                                                                                                                                           | true      |
| 41     | CUSTOM_GIFT                                  | A message sent when a user buys another user a gift                          | Special embed endered from {embeds[0].url} and {gift_info}                                                                                                                              | true      |
...
| 1 << 13 | VOICE_MESSAGE                          | This message's audio attachments are rendered as voice messages                   |
...
| session_id | string  | The session ID associated with this activity (send-only)                 |
...
^1^ Optional when creating a reply, but will always be present when receiving an event/response that includes this object.
...
| auto_moderation_message ^1^       | [AutoMod alert](/resoures/auto-moderation#automod-alert)                                 |
| auto_moderation_notification ^1^  | [AutoMod incident notification](/resoures/auto-moderation#automod-incident-notification) |
| gift                              | Gift embed                                                                               |
...
For the `attachments` array when sending/editing messages, only the `id` is required.
...
| uploaded_filename? ^2^        | string                                                                  | The name of the file pre-uploaded to Discord's GCP bucket                                                                                    |
...
| height?                       | ?integer                                                                | Height of image                                                                                                                              |
| width?                        | ?integer                                                                | Width of image                                                                                                                               |
| placeholder_version?          | integer                                                                 | The attachment placeholder protocol version (currently 1)                                                                                    |
| placeholder?                  | string                                                                  | A low-resolution [thumbhash](https://github.com/evanw/thumbhash) of the image, to display before it is loaded                                |
| ephemeral? ^1^                | boolean                                                                 | Whether this attachment is ephemeral                                                                                                         |
...
| flags? ^3^                    | integer                                                                 | The [attachment's flags](#attachment-flags)                                                                                                  |
| is_clip? ^3^ ^4^ ^5^          | boolean                                                                 | Whether the file being uploaded is a [clipped recording of a stream](https://support.discord.com/hc/en-us/articles/16861982215703-Clips)     |
| is_thumbnail? ^3^             | boolean                                                                 | Whether the file being uploaded is a thumbnail                                                                                               |
| is_remix? ^3^                 | boolean                                                                 | Whether this attachment is a [remixed](https://support.discord.com/hc/en-us/articles/15145601963031-Remix-FAQ) version of another attachment |
| is_spoiler? ^3^               | boolean                                                                 | Whether this attachment is a spoiler                                                                                                         |
| clip_created_at? ^4^          | ISO8601 timestamp                                                       | When the clip was created                                                                                                                    |
| clip_participant_ids? ^4^ ^5^ | array[snowflake]                                                        | The IDs of the participants in the clip (max 100)                                                                                            |
| clip_participants? ^5^        | array[partial [user](/resources/user#user-object) object]               | The participants in the clip (max 100)                                                                                                       |
| title?                        | string                                                                  | The title of the clip                                                                                                                        |
| application_id? ^5^           | snowflake                                                               | The ID of the application the clip was taken in                                                                                              |
| application? ^5^              | partial [application](/resources/application#application-object) object | The application the clip was taken in                                                                                                        |
...
^2^ This field is send-only. See [Uploading to Google Cloud](/reference#uploading-to-google-cloud) for more information.

^3^ The `flags` field is not sendable. To set flags, use the `is_clip`, `is_thumbnail`, and `is_remix` send-only fields.

^4^ When sending a clip, `is_clip`, `clip_created_at`, and `clip_participant_ids` are required. [See message types](#clips) for more information.

^5^ The `clip_participant_ids` and `application_id` fields are send-only. You will receive the `clip_participants` and `application` fields back when retrieving the [message](#message-object).
...
| embeds? ^2^                 | array[[embed](#embed-object) object]                                                 | Embedded `rich` content (max 10, 6000 characters)                                                                                                                           |
...
| poll?                       | [poll create request](#poll-create-request-object) object                            | A poll!                                                                                                                                                                     |
...
| Field         | Type   | Description       |
| ------------- | ------ | ----------------- |
| original_url  | string | The provided URL  |
| refreshed_url | string | The refreshed URL |
...
Sets the channel's latest acknowledged message (marks a message as read) for the current user. Fires a [Message Ack](/topics/gateway-events#message-ack) Gateway event.
...
| Field | Type                                            | Description                                            |
| ----- | ----------------------------------------------- | ------------------------------------------------------ |
| media | [media preview](#media-preview-stucture) object | The preview of the media in the thread's first message |
...
| context_errors | array[string]                                      | The errors that occurred while parsing the website |
...
| allow_multiselect | boolean                                                    | Whether a user can select multiple answers                                |
| layout_type?      | integer                                                    | The [layout type](#poll-layout-type) of the poll. Defaults to... DEFAULT! |
