...
However, if the guild has `CLAN` feature, the same is determined via dividing the message's `timestamp` with millisecond precision by modulo 10.

See the 2nd table below for system welcome messages that shows up in CLAN servers.
...
| 4     | "New recruit! Luminosity joined the Guild."                                     |
