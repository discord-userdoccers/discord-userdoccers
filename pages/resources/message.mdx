...
| link_hostname?        | array[string]    | Filter messages by link hostname (e.g. `discordapp.com`)                                         |
...
Pagination with this endpoint can be tricky, because it supports multiple different pagination methods. The table below outlines the relationships between pagination parameters and how they may be used. It does not restrict which parameters can be included in the search query. You may include any combination of these fields, whether or not you intend to paginate the results.

| Field           | Sort Order | Sort By   | Description                                             |
|-----------------|------------|-----------|---------------------------------------------------------|
| min_id (after)  | asc        | timestamp | Preferred when sorting by timestamp in ascending order  |
| max_id (before) | desc       | timestamp | Preferred when sorting by timestamp in descending order |
| offset ^1^      | -          | relevance | Required to paginate when sorting by relevance          |

^1^ Clients may paginate using `offset` when sorting by `timestamp`, but pagination via `min_id` and `max_id` should be preferred. This is because the maximum amount of messages that can be retrieved via the `offset` parameter is limited to 10,000, whereas there is no limit to the number of messages that can be retrieved when paginating via `min_id` and `max_id`.
