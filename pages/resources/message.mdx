...
| 41     | CUSTOM_GIFT                                  | A message sent when a user buys another user a gift                          | Special embed endered from `embeds[0].url` and [`gift_info`](#message-gift-info-object)                                                                                                     | true      |
...
| guild_product_purchase | ?[guild product purchase](#guild-product-purchase-object) | The guild product purchase that prompted this message       |
...
| Field                   | Type                                                          | Description                                                                                                                |
| ----------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| type? ^1^               | integer                                                       | The [type of message reference](#message-reference-type)                                                                   |
...
| forward_only? ^2^ ^3^   | [message forward only](#message-forward-only-stucture) object | What to include in the forwarded message                                                                                   |

^1^ Optional when creating a reply, but will always be present when receiving this object.
...
While message snapshots are able to be nested, the depth of nesting is currently limited to 1.
...
| guild_id? | snowflake                                              | The ID of the guild the message originated from        |
...
| type             | integer                                                                              | The [type of message](#message-type)                  |
| flags?           | integer                                                                              | The [message's flags](#message-flags)                 |
| components? ^1^  | array[[message component](/interactions/message-components#component-object) object] | The message's components (e.g. buttons, select menus) |
...
- These messages have an array of [`message_snapshots`](#message-object) field containing a copy of the original message. This copy follows the same structure as a message, but has only the minimal set of fields returned required for context/rendering.
...
- Forwards are created by including a []`message_reference`](#message-reference-object) with [`FORWARD`](#message-reference-type) type when sending a message. When sending, `type`, `message_id`, and `channel_id` are required, and the requester must have `VIEW_CHANNEL` permissions.
...
| title?                        | string                                                                  | The name of the file without the extension (automatically provided when the filename is normalized or randomly generated due to invalid characters) |
...
| proxy_url ^3^                 | string                                                                  | A proxied url of the file                                                                                                                           |
| height?                       | ?integer                                                                | Height of image                                                                                                                                     |
| width?                        | ?integer                                                                | Width of image                                                                                                                                      |
...
| flags? ^4^                    | integer                                                                 | The [attachment's flags](#attachment-flags)                                                                                                         |
| is_clip? ^4^ ^5^ ^6^          | boolean                                                                 | Whether the file being uploaded is a [clipped recording of a stream](https://support.discord.com/hc/en-us/articles/16861982215703-Clips)            |
| is_thumbnail? ^4^             | boolean                                                                 | Whether the file being uploaded is a thumbnail                                                                                                      |
| is_remix? ^4^                 | boolean                                                                 | Whether this attachment is a [remixed](https://support.discord.com/hc/en-us/articles/15145601963031-Remix-FAQ) version of another attachment        |
| is_spoiler? ^4^               | boolean                                                                 | Whether this attachment is a spoiler                                                                                                                |
| clip_created_at? ^5^          | ISO8601 timestamp                                                       | When the clip was created                                                                                                                           |
| clip_participant_ids? ^5^ ^6^ | array[snowflake]                                                        | The IDs of the participants in the clip (max 100)                                                                                                   |
| clip_participants? ^6^        | array[partial [user](/resources/user#user-object) object]               | The participants in the clip (max 100)                                                                                                              |
| title?                        | string                                                                  | The title of the clip                                                                                                                               |
| application_id? ^6^           | snowflake                                                               | The ID of the application the clip was taken in                                                                                                     |
| application? ^6^              | partial [application](/resources/application#application-object) object | The application the clip was taken in                                                                                                               |
...
^3^ The proxy URL only supports attachments with a defined `width` and `height`, such as images and videos. For all other attachments, the proxy returns a 415 unsupported media type error.

^4^ The `flags` field is not sendable. To set flags, use the `is_clip`, `is_thumbnail`, and `is_remix` send-only fields.

^5^ When sending a clip, `is_clip`, `clip_created_at`, and `clip_participant_ids` are required. [See message types](#clips) for more information.

^6^ The `clip_participant_ids` and `application_id` fields are send-only. You will receive the `clip_participants` and `application` fields back when retrieving the [message](#message-object).
...
| duration           | integer                                             | Number of hours the poll should be open for (max 7 days)             |
