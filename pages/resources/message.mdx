...
^4^ This field is only returned for messages with a `type` of `REPLY` or `THREAD_STARTER_MESSAGE`. If the message is a reply but the `referenced_message` field is not present, the backend did not attempt to fetch the message that was being replied to, so its state is unknown. If the field exists but is `null`, the referenced message was deleted.
...
^1^ This field requires `manual` to be `true`.
