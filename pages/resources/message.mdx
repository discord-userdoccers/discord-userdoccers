...
\*\* Users must configure (or, in the case of bots, be approved for) the [`MESSAGE_CONTENT` intent](/topics/gateway#message-content-intent) to receive non-empty values for these fields in most situations.
...
| 35     | PREMIUM_REFERRAL                             | A message sent when a user gifts a nitro referral                         | "{content}"                                                                                                                                                                      | true      |
...
| nonce?\*\*\*                | integer \| string                                                                    | The message's nonce, used for message deduplication (will be present in the returned object and accompanying [Message Create](/topics/gateway-events#message-create) event) |
