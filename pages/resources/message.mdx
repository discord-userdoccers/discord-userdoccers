...
| uploaded_filename? ^2^ ^4^ | string            | The name of the file pre-uploaded to Discord's GCP bucket                                                                                    |
...
^4^ When sending a pre-uploaded file and setting `is_clip` to `true`, the file's clip metadata must be the same as when it was uploaded.
