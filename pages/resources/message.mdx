...
| resolved?                     | [select menu resolved](/resources/components#select-menu-resolved-object) object                   | The resolved data for the select menu component's interaction                                                                             |
...
| description? ^1^           | string  | The description for the media                                                                                 |
| content_type? ^1^          | string  | The attachment's [media type](https://en.wikipedia.org/wiki/Media_type)                                       |
| content_scan_metadata? ^1^ | string  | The content scan metadata for the media                                                                       |
...
| embed_provider?          | array[string]    | Filter messages by embed provider (case-sensitive, e.g. `Tenor`)                |
| link_hostname?           | array[string]    | Filter messages by link hostname (case-sensitive, e.g. `discordapp.com`)        |
| attachment_filename? ^3^ | string           | Filter messages by attachment filename                                          |
| attachment_extension?    | array[string]    | Filter messages by attachment extension (e.g. `txt`)                            |
| command_id? ^3^          | snowflake        | Filter messages by application command ID                                       |
...
| score?     | [search cursor score](#search-cursor-score-structure) object | The relevance metadata for the last message in this page of results; used for pagination by relevance |
...
<RouteHeader method="DELETE" url="/users/@me/messages/{message.id}" supportsOAuth2="dm_channels.messages.write">
  Delete DM Message
</RouteHeader>
...
| after?  | ISO8601 timestamp | Get messages pinned after this timestamp        |
...
<RouteHeader method="PUT" url="/channels/{channel.id}/pins/{message.id}" supportsAuditReason deprecated>
  Pin Message
</RouteHeader>

<Alert type="warn">

This endpoint is deprecated. It is replaced by [Add Message Pin](#add-message-pin).
...
<RouteHeader method="DELETE" url="/channels/{channel.id}/pins/{message.id}" supportsAuditReason deprecated>
  Unpin Message
</RouteHeader>

<Alert type="warn">

This endpoint is deprecated. It is replaced by [Remove Message Pin](#remove-message-pin).
