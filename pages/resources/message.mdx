...
| 1 << 13 | IS_VOICE_MESSAGE                       | This message's audio attachment is rendered as a voice message                    |
| 1 << 14 | HAS_SNAPSHOT                           | This message has a forwarded message snapshot attached                            |
| 1 << 15 | IS_COMPONENTS_V2                       | This message contains components from version 2 of the UI kit                     |
...
| type                | integer                                                                              | The [type of message](#message-type)                  |
| flags               | integer                                                                              | The [message's flags](#message-flags)                 |
| components? ^1^ ^2^ | array[[message component](/interactions/message-components#component-object) object] | The message's components (e.g. buttons, select menus) |
| sticker_items?      | array[[sticker item](/resources/sticker#sticker-item-object) object]                 | The message's sticker items                           |
| soundboard_sounds?  | array[[soundboard sound](/resources/soundboard/#soundboard-sound-object) object]     | The message's soundboard sounds                       |
...
| thumbnail?    | [embed image](#embed-image-structure) object        | Embed thumbnail information                                                           |
...
| flags?        | integer                                             | The [embed's flags](#embed-flags)                                                     |
...
| image                             | Image embed                                                                               |
| link                              | Link embed                                                                                |
| poll_result ^2^                   | Poll result embed                                                                         |
| post_preview ^3^                  | Media channel post preview embed                                                          |
...
^2^ See the [poll result notifications](#poll-result-notifications) section for more information.

^3^ This embed type is used to signal to the client to render a [preview of the linked media channel post](#get-channel-media-preview). This embed is created for URLs that link to a media channel post if the channel is a paywalled role subscription benefit. The URL is always in the format `https://discord.com/channels/:guild_id/:parent_id/threads/:thread_id/:initial_message_id`.

###### Embed Flags

| Value  | Name                    | Description                                                                                                 |
| ------ | ----------------------- | ----------------------------------------------------------------------------------------------------------- |
| 1 << 4 | CONTAINS_EXPLICIT_MEDIA | This embed was flagged as [sensitive content](https://support.discord.com/hc/en-us/articles/18210995019671) |
| 1 << 5 | CONTENT_INVENTORY_ENTRY | This embed is a legacy content inventory reply                                                              |

###### Embed Image Structure

| Field                    | Type    | Description                                                                                                   |
| ------------------------ | ------- | ------------------------------------------------------------------------------------------------------------- |
| url                      | string  | Source URL of image (only supports http(s) and attachments) (max 2048 characters)                             |
| proxy_url? ^1^           | string  | A proxied URL of the image                                                                                    |
| height? ^1^              | integer | Height of image                                                                                               |
| width? ^1^               | integer | Width of image                                                                                                |
| flags? ^1^               | integer | The [image's attachment flags](#attachment-flags)                                                             |
| placeholder_version? ^1^ | integer | The attachment placeholder protocol version (currently 1)                                                     |
| placeholder? ^1^         | string  | A low-resolution [thumbhash](https://github.com/evanw/thumbhash) of the image, to display before it is loaded |
...
| flags?               | integer | The [video's attachment flags](#attachment-flags)                                                             |
| placeholder_version? | integer | The attachment placeholder protocol version (currently 1)                                                     |
| placeholder?         | string  | A low-resolution [thumbhash](https://github.com/evanw/thumbhash) of the video, to display before it is loaded |
...
^5^ The `flags` field is received only and cannot be set directly. To set flags, use the `is_clip`, `is_thumbnail`, `is_remix`, and `is_spoiler` send-only fields.
...
| Value  | Name                    | Description                                                                                                              |
| ------ | ----------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 1 << 0 | IS_CLIP                 | This attachment is a [clipped recording of a stream](https://support.discord.com/hc/en-us/articles/16861982215703-Clips) |
| 1 << 1 | IS_THUMBNAIL            | This attachment is a thumbnail                                                                                           |
| 1 << 2 | IS_REMIX                | This attachment has been [remixed](https://support.discord.com/hc/en-us/articles/15145601963031-Remix-FAQ)               |
| 1 << 3 | IS_SPOILER              | This attachment is a spoiler                                                                                             |
| 1 << 4 | CONTAINS_EXPLICIT_MEDIA | This attachment was flagged as [sensitive content](https://support.discord.com/hc/en-us/articles/18210995019671)         |
| 1 << 5 | IS_ANIMATED             | This attachment is an animated image                                                                                     |
...
Returns a list of [message](#message-object) objects without the `reactions` key that match a search query in the guild or channel. The messages that are direct results will have an extra `hit` key set to `true`. Requires the `READ_MESSAGE_HISTORY` permission if operating on a guild channel.
...
Due to speed optimizations, search may return slightly less results than the limit specified.
This should only happen on pages of messages that have not been accessed for a long time.
...
| sort_by? ^2^          | string           | The [sorting algorithm](#message-sort-type) to use                       |
...
<RouteHeader method="POST" url="/users/{user.id}/messages" supportsOAuth2="dm_channels.messages.write">
  Create DM Message
</RouteHeader>
...
<RouteHeader method="POST" url="/users/@me/messages/{message.id}/delete" supportsOAuth2="dm_channels.messages.write">
  Delete DM Message
</RouteHeader>
