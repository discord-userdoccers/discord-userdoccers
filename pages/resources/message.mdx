...
^5^ This field will be missing if the attachment has not yet been scanned. In this case, a scan can be triggered with the [Explicit Content Scan](#explicit-content-scan) endpoint. If the field is present and set to `0`, the attachment is not eligible for a scan.
