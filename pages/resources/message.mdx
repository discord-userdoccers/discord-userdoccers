...
^1^ See [Uploading Files](/reference#uploading-files) for details. The attachments must be uploaded through [Create Message Attachments](/reference#create-message-attachments) endpoint.
...
<RouteHeader method="GET" url="/channels/{channel.id}/polls/{message.id}/answers/{poll_answer.id}" supportsBot>
