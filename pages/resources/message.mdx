...
| poll? ^2^               | [poll](#poll-structure) object                                                                     | A poll!                                                                                                                                   |
...
| Field             | Type                                                | Description                                      |
| ----------------- | --------------------------------------------------- | ------------------------------------------------ |
| question ^1^      | [poll media](#poll-media-object-structure) object   | The question of the poll                         |
...
It is similar but not exactly identical to the main [poll object](#poll-structure).
The main difference is that the request has `duration` which eventually becomes `expiry`.

| Field             | Type                                                | Description                                                               |
| ----------------- | --------------------------------------------------- | ------------------------------------------------------------------------- |
| question ^1^      | [poll media](#poll-media-object-structure) object   | The question of the poll                                                  |
| answers           | array[[poll answer](#poll-answer-structure) object] | Each of the answers available in the poll, up to 10                       |
...
^1^ `text` should always be non-null for both questions and answers, but please do not depend on that in the future.

^2^ When creating a poll answer with an emoji, one only needs to send either the `id` (custom emoji) or `name` (default emoji) as the only field.
...
Submit [poll](#poll-structure) votes. Returns a 204 empty response on success. Fires multiple [Message Poll Vote Add](/topics/gateway-events#message-poll-vote-add)/[Message Poll Vote Remove](/topics/gateway-events#message-poll-vote-remove) Gateway events.
