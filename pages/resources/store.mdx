...
Returns store layout for the given application ID.
...
| otps               | array[[store listing](#store-listing-object) object]         | The listings the users can purchase for lifetime |
...
<RouteHeader method="POST" url="/applications/{application.id}/storefront/publish">
  Modify SKU Storefront Publish States
</RouteHeader
...
<RouteHeader method="POST" url="/partner-sdk/guilds/{guild.id}/application-storefront/skus/{sku.id}/eligibility">
  Create Social Layer SKU Purchase Eligibility Interaction
</RouteHeader>

Creates a [`SOCIAL_LAYER_SKU_PURCHASE_ELIGIBILITY`](/interactions/receiving-and-responding#interaction-type) interaction.
