...
| store_page_guild_products_default_sort? | integer                                                   | The default [sort order](#guild-product-sort-order-type) of guild's products on the store page            |
...
^1^ Store pages are accessible at `https://discord.com/servers/<slug>`.
...
| 4     | NEWEST_ARRIVALS | Sort by their lifetime duration   |

###### Example Role Subscriptions Settings Object
...
| attachments_count | integer                                                          | Number of the attachments on the product listing                     |
...
| attachments       | array[[attachment](/resources/message#attachment-object) object] | The attached attachments on the product listing                      |
| price             | [unit price](/resources/payment#unit-price-structure) object     | The price for the product listing                                    |

### Guild Role Subscription Group Listing Object

###### Guild Role Subscription Group Listing Structure
...
| subscription_listings?     | array[[guild role subscription listing](#guild-role-subscription-listing-object) object]                    | The subscription listings                         |
| benefit_channels?          | array[[guild role subscription benefit channel](#guild-role-subscription-benefit-channel-structure) object] | The benefits for the channel                      |

###### Guild Role Subscription Benefit Channel Structure
...
### Guild Role Subscription Listing Object

###### Guild Role Subscription Listing Structure
...
| role_benefits      | [guild role subscription benefits](#guild-role-subscription-benefits-structure) object | The role benefits                                |
...
###### Guild Role Subscription Benefits Structure

| Field    | Type                                                                                        | Description              |
| -------- | ------------------------------------------------------------------------------------------- | ------------------------ |
| sku_id   | snowflake                                                                                   | The ID of the SKU        |
| benefits | array[[guild role subscription benefit](#guild-role-subscription-benefit-structure) object] | The benefits of the role |

###### Guild Role Subscription Benefit Structure
...
| ref_type       | integer    | The [type of reference](#guild-role-subscription-benefit-type) |
| ref_id         | ?snowflake | The ID of the referenced entity                                |

^1^ At most one of `emoji_id` and `emoji_name` may be set to a non-null value.

###### Guild Role Subscription Benefit Type

| Value | Name       | Description           | Reference Type                                      |
| ----- | ---------- | --------------------- | --------------------------------------------------- |
| 1     | CHANNEL    | Access to the channel | [channel](/resources/channel#channel-object) object |
...
<RouteHeader method="GET" url="/guilds/{guild.id}/onboarding/allowed-applications">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/onboarding-responses">
...
<RouteHeader method="PUT" url="/guilds/{guild.id}/onboarding-responses">
...
<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/settings">
  Get Guild Role Subscriptions Settings
</RouteHeader>

Returns a [role subscription settings](#role-subscription-settings-object) object for the given guild ID.

<RouteHeader method="PATCH" url="/guilds/{guild.id}/role-subscriptions/settings">
...
| --------------------------------------- | -------- | --------------------------------------------------------------------------------------------------------- |
| full_server_gate?                       | boolean  | Whether the guild is fully gated behind a role subscription                                               |
| description?                            | ?string  | The description of the guild's role shop (max 1500 characters)                                            |
| server_shop_tab_order?                  | integer  | The [order of tabs in guild shop](#guild-product-shop-tabs-order-type)                                    |
| store_page_enabled?                     | boolean  | Whether the store page should be enabled                                                                  |
| store_page_primary_color?               | ?integer | The color of the promo page accent color encoded as an integer representation of a hexadecimal color code |
| store_page_trailer_url?                 | ?string  | The URL to the promo trailer (max 8192 characters)                                                        |
| store_page_show_subscriber_count?       | boolean  | Whether to show amount of role subscribers                                                                |
| store_page_guild_products_default_sort? | integer  | The default [sort order](#guild-product-sort-order-type) of guild's products                              |

<RouteHeader method="POST" url="/guilds/{guild.id}/role-subscriptions/group-listings">
  Create Guild Role Subscription Group Listing
</RouteHeader>

Creates a subscription group listing. Requires the `ADMINISTRATOR` permission. Accepts an empty object. Returns a [guild role subscription group listing](#guild-role-subscription-group-listing-object) object on success.

<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}">
  Get Guild Role Subscription Group Listing
</RouteHeader>

Returns a partial [guild role subscription group listing](#guild-role-subscription-group-listing-object) object without `subscription_listings`, `benefit_channels` for the given guild and subscription group listing ID.
...
<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/group-listings">
  List Guild Role Subscription Group Listings
</RouteHeader>

Returns an array of [guild role subscription group listing](#guild-role-subscription-group-listing-object) objects for the given guild ID.
...
<RouteHeader method="PATCH" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}">
  Modify Guild Role Subscription Group Listing
</RouteHeader>

Modifies the guild role subscription group listing.Requires the `ADMINISTRATOR` permission.  Returns an updated [guild role subscription group listing](#guild-role-subscription-group-listing-object) object on success.
...
<RouteHeader method="DELETE" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}/subscription-listings">
  Create Guild Role Subscription Listing
</RouteHeader>

Creates the guild role subscription listing. Requires the `ADMINISTRATOR` permission. Returns a [guild role subscription listing](#guild-role-subscription-listing-object) object on success.
...
<RouteHeader method="PATCH" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}/subscription-listings/{subscription_listing.id}">
  Modify Guild Role Subscription Listing
</RouteHeader>

Modifies the guild role subscription listing. Requires the `ADMINISTRATOR` permission. Returns an updated [guild role subscription listing](#guild-role-subscription-listing-object) object on success.
...
<RouteHeader method="DELETE" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}/subscription-listings/{subscription_listing.id}">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/role-subscriptions/group-listings/{subscription_group_listing.id}/subscription-listings/{subscription_listing.id}/archive">
  Archive Guild Role Subscription Group Listing
</RouteHeader>

Archives the subscription group listing. Requires the `ADMINISTRATOR` permission. Returns a 204 empty response on success.

<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/trials">
  List Guild Role Subscription Trials
</RouteHeader>

Returns an array of [guild role subscription trial](#guild-role-subscription-trial-structure) objects for the given guild ID.

###### Guild Role Subscription Trial Structure

| Field                      | Type                                                                           | Description                                                        |
| -------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------ |
| id                         | snowflake                                                                      | The ID of the role subscription listing                            |
| num_active_trial_users     | integer                                                                        | Number of active users of the trial                                |
| max_num_active_trial_users | ?integer                                                                       | Maximum number of possibly active users of the trial               |
...
<RouteHeader method="PATCH" url="/guilds/{guild.id}/role-subscriptions/subscription-listings/{subscription_listing.id}/trial">
...
<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/templates">
  List Guild Role Subscription Listing Templates
</RouteHeader>

Returns the guild role subscription listing templates.

###### Response Body

| Field     | Type                                                                                                          | Description   |
| --------- | ------------------------------------------------------------------------------------------------------------- | ------------- |
| templates | array[[guild role subscription listing template](#guild-role-subscription-listing-template-structure) object] | The templates |

###### Guild Role Subscription Listing Template Structure

| Field                                 | Type                                                                                                          | Description                        |
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| category                              | string                                                                                                        | The category                       |
| category_image                        | string                                                                                                        | The default category image         | 
| unselected_dark_theme_category_image  | string                                                                                                        | The category image for dark theme  |
| unselected_light_theme_category_image | string                                                                                                        | The category image for light theme |
| listings                              | array[[template guild role subscription listing](#template-guild-role-subscription-listing-structure) object] | The template listings              |

###### Template Guild Role Subscription Listing Structure
...
| channels         | array[[template guild role subscription listing channel](#template-guild-role-subscription-listing-channel-structure) object] | The channels available to the role                                                                 |
| additional_perks | array[[guild role subscription benefit](#guild-role-subscription-benefit-structure) object]                                   | The additional perks of the role                                                                   |

###### Template Guild Role Subscription Listing Channel Structure
...
###### Example Template Guild Role Subscription Listing
    ...
    "listings": [
        {
            ...
            "image": "data:image/png;base64,",
            "price_tier": 399,
            "channels": [
                {
...
<RouteHeader method="GET" url="/guilds/{guild.id}/role-subscriptions/subscription-listings/{subscription_listing.id}/trial/{role_subscription_trial.id}/eligibility">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/enable-requests">
  Create Creator Monetization Enable Request
</RouteHeader>

Creates a request to enable the access to guild's monetization features. Returns a 204 empty response on success.

<RouteHeader method="GET" url="/guilds/{guild.id}/creator-monetization/requirements">
  Get Creator Monetization Eligibility
</RouteHeader>

Returns the information about creator monetization eligibility for the guild.
...
| country?                            | boolean                                                                                                  | Whether the guild owner is in country eligible for creator monetization                                |
...
| latest_request?                     | [creator monetization application](#creator-monetization-application-structure) object                   | The latest request to enable creator monetization                                                      |
| rejection?                          | [creator monetization application rejection data](#creator-monetization-rejection-data-structure) object | The information about application rejection                                                            |

###### Creator Monetization Application Structure

| Field | Type    | Description                                                         |
| ----- | ------- | ------------------------------------------------------------------- |
| state | integer | The [state of application](#creator-monetization-application-state) |

###### Creator Monetization Rejection Data Structure

| Field           | Type               | Description                              |
| --------------- | ------------------ | ---------------------------------------- |
| can_reapply_at? | ?ISO8601 timestamp | When the application can be reapplied at |

###### Creator Monetization Application State
...
<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/enable-requests/{creator_monetization_request.id}/accept-terms" deprecated>
  Accept Creator Monetization Request Terms
</RouteHeader>

Accepts guild monetization request's terms of use. Returns a 204 empty response on success. Fires a [Guild Update](/gateway/gateway-events#guild-update) Gateway event.
...
<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/accept-terms">
  Accept Creator Monetization Terms
</RouteHeader>

Accepts guild monetization's terms of use. Returns a 204 empty response on success. Fires a [Guild Update](/gateway/gateway-events#guild-update) Gateway event.

<RouteHeader method="GET" url="/guilds/{guild.id}/creator-monetization/restrictions">
  Get Creator Monetization Restrictions
</RouteHeader>

Returns a list of restrictions to creator monetization.
...
<RouteHeader method="GET" url="/guilds/{guild.id}/creator-monetization/marketing/onboarding">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/ownership-transfer/onboarding">
  Update Creator Monetization Team
</RouteHeader>

Transfers ownership of a creator monetization application to a [team](/resources/team). User must be the owner of the application or owning team.

<Alert type="note">

Afterwards, you may be required to accept new guild monetization's terms of use using [Accept Creator Monetization New Terms](#accept-creator-monetization-new-terms) or [Accept Creator Monetization New Terms Demonetized](#accept-creator-monetization-new-terms-demonetized) endpoints.
...
<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/accept-new-terms" deprecated>
  Accept Creator Monetization New Terms
</RouteHeader>

Accepts guild monetization's terms of use. Returns a 204 empty response.

<Alert type="warn">

This endpoint is deprecated. It is replaced by [Accept Creator Monetization New Terms Demonetized](#accept-creator-monetization-new-terms-demonetized).

</Alert>

<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/accept-new-terms-demonetized">
  Accept Creator Monetization New Terms Demonetized
</RouteHeader>

Accepts guild monetization's terms of use. Returns a 204 empty response.

<RouteHeader method="POST" url="/guilds/{guild.id}/creator-monetization/remove-monetization">
  Remove Creator Monetization
</RouteHeader>

Removes the guild's access to monetization features. Accepts an empty object. Returns a 204 empty response on succses. Fires a [Guild Update](/gateway/gateway-events#guild-update) Gateway event.

<RouteHeader method="POST" url="/guilds/{guild.id}/products">
...
| description?     | string                                                 | The description of the product listing                               |
| price_tier?      | integer                                                | The [base price](/resources/store#list-store-price-tiers) of the SKU |
| create_new_role? | boolean                                                | Whether to create new role                                           |
| image?           | ?[image data](/reference#cdn-data)                     | The thumbnail of product listing                                     |
| image_name?      | string                                                 | The name of the thumbnail image                                      |
| unlink_role?     | boolean                                                | Whether to unlink the role                                           |
| attachments? ^1^ | array[partial [attachment](#attachment-object) object] | The attachments to upload                                            |

^1^ See [Uploading Files](/reference#uploading-files) for details. The attachments must be uploaded through [Create Guild Product Attachments](/reference#create-guild-product-attachments) endpoint.

<RouteHeader method="GET" url="/guilds/{guild.id}/products/listings">
  List Guild Product Listings
</RouteHeader>

Returns a list of product listings for the given guild ID.
...
<RouteHeader method="GET" url="/guilds/{guild.id}/products/listings/{product_listing.id}">
...
<RouteHeader method="PATCH" url="/guilds/{guild.id}/products/listings/{product_listing.id}">
...
<RouteHeader method="DELETE" url="/guilds/{guild.id}/products/listings/{product_listing.id}">
...
<RouteHeader method="POST" url="/guilds/{guild.id}/products/listings/{product_listing.id}/attachments/{attachment.id}/download">
  Get Guild Product Listing Attachment URL
</RouteHeader>

Returns an URL to the attachment for the given guild and product ID.
