...
Represents an embeddable widget for a guild.
...
Represents a participating user in a [guild](/resources/guild#guild-object).
...
Represents a ban for a [guild](#guild-object). Banned users can't rejoin a guild unless [unbanned](#delete-guild-ban).
...
Discord is making significant changes to the membership screening API specifically related to getting and editing the membership screening object. Long story short is that it can be improved. As such, we have removed this documentation. There will **not be** any changes to how pending members work, as outlined above. That behavior will stay the same.
...
| name                           | string                                                             | The name of the guild (2-100 characters, excluding trailing and leading whitespace)                                        |
| region? **(deprecated)**       | ?string                                                            | The main [voice region](/resources/voice#voice-region-object) ID of the guild                                              |
| icon?                          | ?[image data](/reference#image-data)                               | The guild's icon                                                                                                           |
| verification_level?            | integer                                                            | The [verification level](#verification-level) required for the guild                                                       |
| default_message_notifications? | integer                                                            | Default [message notification level](/resources/guild#message-notification-level) for the guild                            |
| explicit_content_filter?       | integer                                                            | [Whose messages](/resources/guild#explicit-content-filter-level) are scanned and deleted for explicit content in the guild |
...
| system_channel_id?             | ?snowflake                                                         | The ID of the channel where system event messages, such as member joins and premium subscriptions (boosts), are posted     |
| system_channel_flags?          | integer                                                            | The [flags](#system-channel-flags) that limit system event messages                                                        |
| guild_template_code?           | string                                                             | The [template](/resources/template#template-object) code that inspired this guild, used for analytics                      |
...
| ------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| name?                          | string                               | The name of the guild (2-100 characters, excluding trailing and leading whitespace)                                                                                  |
| icon?                          | ?[image data](/reference#image-data) | The guild's icon; animated icons are only shown when the guild has the `ANIMATED_ICON` feature                                                                       |
| banner?                        | ?[image data](/reference#image-data) | The guild's banner; banners are only shown when the guild has the `BANNER` feature, animated banners are only shown when the guild has the `ANIMATED_BANNER` feature |
| home_header?                   | ?[image data](/reference#image-data) | The guild's home header, also used in server guide; home headers are only shown when the guild has the `BANNER` feature                                              |
| splash?                        | ?[image data](/reference#image-data) | The guild's invite splash; splashes are only shown when the guild has the `INVITE_SPLASH` feature                                                                    |
| discovery_splash?              | ?[image data](/reference#image-data) | The guild's discovery splash                                                                                                                                         |
...
Modifies the guild's [2FA requirement](/resources/guild#mfa-level) for administrative actions within the guild. Requires the `MANAGE_GUILD` permission. Fires a [Guild Update](/topics/gateway-events#guild-update) Gateway event.
...
| avatar? | ?[image data](/reference#image-data) | The member's guild avatar; can only be changed for premium (Nitro) users |                 |
| bio?    | ?string                              | The member's guild bio; can only be changed for premium (Nitro) users    |                 |
| banner? | ?[image data](/reference#image-data) | The member's guild banner; can only be changed for premium (Nitro) users |                 |
...
Modifies the current user's profile in the guild. Returns a [profile metadata](#profile-metadata-object) object on success. Fires a [Guild Member Update](/topics/gateway-events#user-update) Gateway event.
...
| Field                           | Type                                 | Description                                                                                                                                             |
| ------------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| pronouns?                       | ?string                              | The members's guild pronouns (max 40 characters)                                                                                                        |
...
Creates a new role for the guild. Requires the `MANAGE_ROLES` permission. Returns the new [role](/topics/permissions#role-object) object on success. Fires a [Guild Role Create](/topics/gateway-events#guild-role-create) Gateway event. All JSON params are optional.
...
| icon?          | ?[image data](/reference#image-data) | The role's icon                                                           |
...
Begins a prune operation. Requires the `KICK_MEMBERS` permission. For large guilds, it's recommended to set the `compute_prune_count` option to `false`, allowing the request to return before all members are pruned. Fires multiple [Guild Member Remove](/topics/gateway-events#guild-member-remove) Gateway events.
...
Returns a [guild widget](#guild-widget-structure) object for the given guild ID. The guild must have the widget enabled. May fire a [Invite Create](/topics/gateway-events#invite-create) Gateway event.
...
| Field  | Type   | Description                                                                            |
| ------ | ------ | -------------------------------------------------------------------------------------- |
| style? | string | [Style of widget image](#guild-widget-image-style-options) returned (default `shield`) |

###### Guild Widget Style Image Options
...
| Field                          | Type                                                           | Description                                                                                          |
| ------------------------------ | -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| primary_category_id?           | ?snowflake                                                     | The ID of the primary [discovery category](#discovery-category-object) set for the guild (default 0) |
