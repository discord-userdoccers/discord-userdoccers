...
Migrates the guild's `MANAGE_MESSAGES` permission to the new `PIN_MESSAGES` permission. Requires the `ADMINISTRATOR` permission. Returns a 204 empty response on success. Fires a [Guild Update](/topics/gateway-events#guild-update) and multiple [Channel Update](/topics/gateway-events#channel-update) and [Role Update](/topics/gateway-events#role-update) Gateway events.
