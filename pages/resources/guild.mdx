...
| premium_features ^3^                | ?[guild premium features](#guild-premium-features-object) object                           | The guild's powerup information                                                                                            |
...
Adds the given powerup SKU to the guild. Requires the `MANAGE_GUILD` permission. Returns a 204 empty response on success. Fires a [Guild Update](/topics/gateway-events#guild-update) and [Guild Powerup Entitlements Create](/topics/gateway-events#guild-powerup-entitlements-create) Gateway event.

<RouteHeader method="DELETE" url="/guilds/{guild.id}/skus/{sku.id}">
  Remove Guild Powerup
</RouteHeader>

Removes the given powerup SKU from the guild. Requires the `MANAGE_GUILD` permission. Returns a 204 empty response on success. Fires a [Guild Update](/topics/gateway-events#guild-update) and [Guild Powerup Entitlements Delete](/topics/gateway-events#guild-powerup-entitlements-delete) Gateway event.
