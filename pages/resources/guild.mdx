...
^2^ If `allow_multiple_guilds` is set to `false`, the `guild_id` parameter is ignored and an email is sent for the first guild found. If `allow_multiple_guilds` is set to `true`, a second request must be made with a `guild_id` provided to send the email.
