...
| ENABLED_MODERATION_EXPERIENCE_FOR_NON_COMMUNITY        | Guild has access to community moderation features (e.g. automod) without being a community                                                                                                                                                                                                     |
...
| STAFF_LEVEL_COLLABORATOR_REQUIRED                      | Restricts guild joining to Discord Collaborators                                                                                                                                                                                                                                               |
| STAFF_LEVEL_RESTRICTED_COLLABORATOR_REQUIRED           | Restricts guild joining to Discord Restricted Collaborators                                                                                                                                                                                                                                    |
...
| pending? ^2^                      | boolean                                            | Whether the member has not yet passed the guild's [membership screening](#membership-screening-object) requirements                                                       |
...
^2^ Won't be included in contexts that are impossible for a pending member to exist in.
...
| 1 << 2 | BYPASSES_VERIFICATION ^1^ | This guild member bypasses guild verification requirements                                                                                                                                                                                                                      |
...
^1^ Allows a member who does not meet verification requirements to participate in the guild.
...
| Field       | Type                                                        | Description                                                   |
| ----------- | ----------------------------------------------------------- | ------------------------------------------------------------- |
| description | string                                                      | A description of what the guild is about (max 300 characters) |
| form_fields | array[[verification form fields](#verification-form-field)] | A list of questions for applicants to answer (max 5)          |
| version     | ?ISO8601 timestamp                                          | When the membership screening was last modified               |
| guild ^1^   | ?partial [guild](#guild-object) object                      | Guild associated with this membership screening               |

^1^ Only included when fetched from the [Get Membership Verification](#get-memebership-verification) endpoint with `with_guild` set to `true`.

###### Verification Form Field

| Field       | Type               | Description                                                                                   |
| ----------- | ------------------ | --------------------------------------------------------------------------------------------- |
| field_type  | string             | [Form field type](#form-field-type)                                                           |
| label       | string             | Label for the form field (max 300 characters)                                                 |
| choices     | ?array[string]     | Multiple choice answers (max 8, max string length 150)                                        |
| response    | ?string or integer | Response for this field. For multiple choice answers this is used as the index into `choices` |
| automations | ?unknown           | Automations associated with this form field, used by TERMS type?                              |
| required    | boolean            | Whether or not this field is required to be answered                                          |
| description | ?string            | Subtext of the field                                                                          |
...
| Value           | Description                                   |
| --------------- | --------------------------------------------- |
| TERMS           | Rules agreement, used by membership screening |
| TEXT_INPUT      | "Short Answer" (max 150 characters)           |
| PARAGRAPH       | "Paragraph" (max 1000 characters)             |
| MULTIPLE_CHOICE | "Multiple Choice"                             |
| VERIFICATION    | Unused email or phone verification option     |
...
| join_request_id      | snowflake                                   | ID of the join request                                           |
| id                   | snowflake                                   | ID of the join request                                           |
| rejection_reason     | ?string                                     | Reason why the join request was rejected                         |
| application_status   | [join request status](#join-request-status) | Status of the join request                                       |
| last_seen            | ?ISO8601 timestamp                          | When the request was last seen                                   |
| guild_id             | snowflake                                   | Guild ID associated with this join request                       |
| user_id              | snowflake                                   | User who created this join request                               |
| interview_channel_id | ?snowflake                                  | Channel where interview regarding this join request is conducted |

###### Join Request Status

| Value     | Description                              |
| --------- | ---------------------------------------- |
| STARTED   | The request is started but not sumbitted |
...
For guilds with [membership screening](#membership-screening-object) enabled, this endpoint will default to adding new members as `pending` in the [guild member](#guild-member-object) object. Members that are `pending` will have to complete membership screening before they become full members that can talk.
...
| roles? ^1^   | array[snowflake] | The role IDs assigned to this member                                                                                     | `MANAGE_ROLES`          |
...
| flags?                            | integer            | The [member's flags](#guild-member-flags) (only `BYPASSES_VERIFICATION` can be set)                                                                                                                   | `MODERATE_MEMBERS` |
...
<RouteHeader method="PATCH" url="/guilds/{guild.id}/vanity-url" supportsAuditReason>
...
<RouteHeader method="GET" url="/guilds/{guild.id}/membership-verification">
  Get Membership Verification
</RouteHeader>

Returns the [membership screening](#membership-screening-object) object for the guild if one is set.

<RouteHeader method="PUT" url="/guilds/{guild.id}/requests/@me">
  Request To Join
</RouteHeader>

Requests to join a guild. Returns a [guild join request](#guild-join-request-object) object.

###### JSON Params

| Field       | Type                                                         | Description                                     |
| ----------- | ------------------------------------------------------------ | ----------------------------------------------- |
| form_fields | array[[verification form fields](#verification-form-fields)] | List of answered questions                      |
| version     | ?ISO8601 timestamp                                           | When the membership screening was last modified |
