...
For applications, this endpoint is restricted according to whether the `GUILD_MEMBERS` [Privileged Intent](/topics/gateway#privileged-intents) is enabled for the application.
...
Updates the onboarding response for the current user in the guild. If responses are not empty, returns an [onboarding responses](#onboarding-responses-object) object on success. Otherwise, returns a 204 empty response. May fire a [Guild Member Update](/topics/gateway-events#guild-member-update) Gateway event.
