...
Returns a list of [join request guild](#join-request-guild-structure) objects representing [non-previewable guilds](#guild-previewing) the current user has pending join requests for.

###### Join Request Guild Structure
...
| home_header                     | ?string       | The guild's [home header hash](/reference#cdn-formatting), also used in server guide |
| features                        | array[string] | Enabled [guild features](#guild-features)                                            |
| approximate_member_count? ^1^   | integer       | Approximate number of total members in the guild                                     |
| approximate_presence_count? ^1^ | integer       | Approximate number of non-offline members in the guild                               |

^1^ Only included when fetched from the [Join Guild](#join-guild) endpoint.

###### Example Join Request Guild
...
Adds the current user to the guild. The guild must be discoverable. If the user is not a member of the guild, returns the [guild](#guild-object) object with an extra optional `show_verification_form` key representing whether the user should be shown the guild's [member verification](/resources/guild#member-verification-object) form on success. Otherwise, returns a 204 empty response.
...
For guilds with [previewing disabled](#guild-previewing), the return type will instead be a [join request guild](#join-request-guild-structure) object.
...
<RouteHeader method="PUT" url="/guilds/{guild.id}/requests/@me/cooldown">
