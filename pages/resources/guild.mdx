...
Returns a [guild](#guild-object) object for a given guild ID.
...
Returns a [guild preview](#guild-preview-object) object for the given guild ID. If the user is not in the guild, the guild must be lurkable.
...
| theme_colors? | ?array[integer]                      | The members's two guild theme colors encoded as an array of integers representing hexadecimal color codes; can only be changed for premium (Nitro) users |
