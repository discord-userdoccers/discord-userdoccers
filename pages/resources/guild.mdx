...
^1^ Allows a member who does not meet verification requirements to participate in the guild, and forces the [member's `pending` status](#guild-member-object) to be `false`. Removing this flag does not make the member [`pending`]#guild-member-flags again.
...
| response? ^1^ | ?string \| integer | Response for this field                                      |
...
^1^ Not present when fetched from the [Get Guild Member Verification](#get-guild-member-verification) endpoint. For `MULTIPLE_CHOICE` questions, the response is the index of the selected choice.
