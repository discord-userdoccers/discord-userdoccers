...
| requester_acked_at | ISO8601 timestamp  | Unknown                                                          |
...
<RouteHeader method="GET" url="/guilds/{guild.id}/prune" supportsBot deprecated>
  Get Guild Prune
</RouteHeader>
...
This endpoint is deprecated. It is replaced by [Get Guild Prune V2](#get-guild-prune-v2).
...
<RouteHeader method="GET" url="/guilds/{guild.id}/prune/v2" supportsBot>
  Get Guild Prune V2
</RouteHeader>

Computes the number of members that would be removed in a prune operation. Requires both the `MANAGE_GUILD` and `KICK_MEMBERS` permissions. Returns a 204 empty response. Asynchronously fires [Guild Prune Update](/gateway/gateway-events#guild-prune-update) Gateway event for the current user.
...
<RouteHeader
  method="GET"
  url="/guilds/{guild.id}/role-subscriptions/subscription-listings/{subscription_listing.id}/trial/{subscription_trial.id}/eligibility"
>
