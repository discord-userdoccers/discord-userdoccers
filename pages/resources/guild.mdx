...
^1^ Only available inside [user guild settings](#user-guild-settings-object) object.
...
| MORE_SOUNDBOARD                                        | Increased guild soundboard slots (+48)                                                                                                                                                                                                                                                             |
...
| RAPIDASH_TEST                                          | Access to [clan conversion](/resources/clan#create-clan) general availability                                                                                                                                                                                                                      |
...
| unusual_dm_activity_until?        | ?ISO8601 timestamp                                                              | When the member's unusual DM activity flag will expire                                                                                                                    |
...
| 1 << 5  | STARTED_SERVER_GUIDE             | This guild member has started the server guide                                                                                                                         |
| 1 << 6  | COMPLETED_SERVER_GUIDE           | This guild member has completed all of the server guide                                                                                                                |
| 1 << 7  | AUTOMOD_QUARANTINED_NAME ^2^     | This guild member has been indefinitely quarantined by [an AutoMod Rule](/resources/auto-moderation#automod-rule-object) for their username, display name, or nickname |
| 1 << 8  | AUTOMOD_QUARANTINED_BIO ^2^      | This guild member has been indefinitely quarantined by [an AutoMod Rule](/resources/auto-moderation#automod-rule-object) for their bio                                 |
| 1 << 9  | DM_SETTINGS_UPSELL_ACKNOWLEDGED  | This guild member has acknowledged the DM privacy settings upsell modal                                                                                                |
| 1 << 10 | AUTOMOD_QUARANTINED_CLAN_TAG ^2^ | This guild member has been indefinitely quarantined by [an AutoMod Rule](/resources/auto-moderation#automod-rule-object) for their clan tag                            |
...
To represent a member's progress towards becoming a full member of the guild, a [Guild Join Request Create](/topics/gateway-events#guild-join-request-create) Gateway event will be dispatched. Once the join request is approved, the member's `pending` status will be false and they will be able to interact with the guild.
...
Assigning a member the [`BYPASSES_VERIFICATION` flag](#guild-member-flags) will force their `pending` status to be false, regardless of the join request's status.
...
This means that a [Guild Member Add](/topics/gateway-events#guild-member-add) Gateway event will not be dispatched until the member has passed the verification gate, and `pending` will always be false in the [guild member](#guild-member-object) object for new joiners.
...
This includes most of the join request endpoints, such as [Get Guild Join Request](#get-guild-join-request) and [Action Guild Join Request](#modify-guild-join-request).
...
| form_responses? ^1^   | ?array[[member verification form field](#verification-form-field) object] | Responses to the guild's member verification questions                              |
...
| 1 << 12 | OPT_IN_ENABLED        | Unknown                          |
| 1 << 13 | NEW_FORUM_THREADS_OFF | Unknown                          |
| 1 << 14 | NEW_FORUM_THREADS_ON  | Unknown                          |
...
Modifies user guild's settings. Returns list of an outdated [user guild settings](#user-guild-settings-object) objects on success. Fires mulitple [User Guild Settings Update](/topics/gateway-events#user-guild-settings-update) Gateway events.

###### JSON params

| Field  | Type                                                                       | Description                                             |
| ------ | -------------------------------------------------------------------------- | ------------------------------------------------------- |
| guilds | map[snowflake, partial [user guild settings](#user-guild-settings) object] | The mapping of guild IDs to partial user guild settings |

<RouteHeader method="PATCH" url="/users/@me/guilds/{guild.id}/settings OR /users/@me/guilds/@me/settings">
  Modify User Guild Settings
</RouteHeader>

Modifies user guild's settings. Takes [partial user guild settings](#partial-user-guild-settings) object. Returns [user guild settings](#user-guild-settings-object) object on success. Fires [User Guild Settings Update](/topics/gateway-events#user-guild-settings-update) Gateway event.
...
Returns a list of [supplemental guild member](#supplemental-guild-member-object) objects including join source information for the given user IDs.
...
For guilds with [member verification](#member-verification-object) and [guild previewing](#guild-previewing) enabled, this endpoint will default to adding new members as `pending` in the [guild member](#guild-member-object) object. Members that are `pending` will have to complete member verification before they become full members that can talk.
...
Adds the `DM_SETTINGS_UPSELL_ACKNOWLEDGED` [member flag](#guild-member-flags) to the current user. Returns a 204 empty response on success. Fires a [Guild Member Update](/topics/gateway-events#guild-member-update) Gateway event.
...
Returns a [role](/topics/permissions#role-object) object for the given role.
...
Returns the [member verification](#member-verification-object) object for the guild if one is set. If the user is not a member of the guild, the guild must be discoverable or have [guild previewing](#guild-previewing) disabled.
...
Joins Wumpus Feedback Sqaud through the guild. Requires the `MANAGE_GUILD` permission and the [`CLAN` guild feature](#guild-features). Returns a 204 empty response on success. Fires a [Guild Create](/topics/gateway-events#guild-create), [Guild Member Add](/topics/gateway-events#guild-member-add), and optionally [Guild Join Request Create](/topics/gateway-events#guild-join-request-create) Gateway event.
