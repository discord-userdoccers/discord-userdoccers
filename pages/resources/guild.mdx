...
| PRUNE_REQUIRES_ADMIN  ^2^                       | Whether the ability to [prune guild members](#-prune-guild) should be restricted to administrators                                                            | `ADMINISTRATOR`      |
...
Returns the number of members that would be removed in a prune operation. If the `PRUNE_REQUIRES_ADMIN` feature has been enabled, then the `ADMINISTRATOR` permission is required. Otherwise, requires both the `MANAGE_GUILD` and `KICK_MEMBERS` permissions.
