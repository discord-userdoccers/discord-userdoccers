...
In guilds with [member verification](https://support.discord.com/hc/en-us/articles/1500000466882) enabled, when a member joins, a [Guild Member Add](/topics/gateway-events#guild-member-add) Gateway event will be dispatched but they will initially be restricted from doing any actions in the guild, and `pending` will be true in the [member object](#guild-member-object).
...
When joining a guild with member verification enabled, a [Guild Join Request Create](/topics/gateway-events#guild-join-request-create) Gateway event will be dispatched, and the member will be restricted from doing any actions in the guild until they pass the verification gate.
...
This means that a [Guild Member Add](/topics/gateway-events#guild-member-add) Gateway event will not be dispatched until the member has passed the verification gate, and `pending` will immediately be true in the [member object](#guild-member-object) once it is.
...
Returns a list of join requests for the guild. Requires the `KICK_MEMBERS` permission.
...
If the guild has [previewing disabled](#guild-previewing), deletes the current user's join request. Else, functions the same as [Reset Guild Join Request](#reset-guild-join-request). Returns a 204 empty response on success. Fires a [Guild Join Request Delete](/topics/gateway-events#guild-join-request-delete) Gateway event.

<RouteHeader method="POST" url="/join-request/{guild_join_request.id}/interview">
  Create Guild Join Request Interview
</RouteHeader>

Creates or returns an existing private interview channel for the join request. Requires the `KICK_MEMBERS` permission. Returns a [DM channel](/resources/channel#channel-object) object on success. Fires a [Guild Join Request Update](/topics/gateway-events#guild-join-request-update) and [Channel Create](/topics/gateway-events#channel-create) Gateway event.

<RouteHeader method="PATCH" url="/guilds/{guild.id}/requests/{guild_join_request.id}">
  Action Guild Join Request
</RouteHeader>

Accepts or denies a join request for the guild. Requires the `KICK_MEMBERS` permission. Returns a [guild join request](#guild-join-request-object) object on success. Fires a [Guild Join Request Update](/topics/gateway-events#guild-join-request-update) and may fire a [Guild Member Add](/topics/gateway-events#guild-member-add) or [Guild Member Remove](/topics/gateway-events#guild-member-remove) Gateway event.
