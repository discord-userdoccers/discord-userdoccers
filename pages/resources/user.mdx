...
Modify the requester's user account settings. Returns a [user](#user-object) object with an extra `token` key representing the user's new authorization token on success. Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
Modify the current user's profile. Returns a [profile metadata](#profile-metadata-object) object on success. Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
Disable the current user's account. Invalidates all active tokens. Returns a 204 empty response on success.
...
Mark the current user's account for deletion. Invalidates all active tokens. Returns a 204 empty response on success.
...
Send a verification email to the current user's email address to initiate the email change process. Returns a 204 empty response on success.

<RouteHeader method="POST" url="/users/@me/email/verify-code">
  Verify User Email Change
</RouteHeader>

Verify a verification code sent to the current user's email address to initiate an email change.
...
Create a personal data harvest request for the current user. Returns a [harvest](#harvest-object) object on success.
...
Set the note for the given [user](#user-object). Returns a 204 empty response on success. Fires a [User Note Update](/topics/gateway-events#user-note-update) Gateway event.
...
Create a new connection for the current user. Returns a [connection](#connection-object) object on success. Fires a [User Connections Update](/topics/gateway-events#connection-create) Gateway event.
...
Refresh a connection. Returns a 204 empty response on success. Fires a [User Connections Update](/topics/gateway-events#user-connections-update) Gateway event.

<RouteHeader method="PATCH" url="/users/@me/connections/{connection.type}/{connection.id}">
  Modify User Connection
</RouteHeader>

Modify a connection. Returns a [connection](#connection-object) object on success. Fires a [User Connections Update](/topics/gateway-events#user-connections-update) Gateway event.
...
Send a friend request to another user, which can be accepted by creating a new relationship of type friend.
...
Add a relationship to another user. Returns a 204 empty response on success. Fires a [Relationship Add](/topics/gateway-events#user-relationship-create) Gateway event.
...
Modify a relationship to another user. Returns a 204 empty response on success. Fires a [Relationship Update](/topics/gateway-events#user-relationship-update) Gateway event.
...
Remove a relationship with another user. Returns a 204 empty response on success. Fires a [Relationship Remove](/topics/gateway-events#user-relationship-delete) Gateway event.
...
Create a new friend invite. Accepts an empty body. Returns a friend [invite](#invite-object) object on success.

<RouteHeader method="DELETE" url="/users/@me/invites">
  Revoke User invites
</RouteHeader>

Revoke all of the current user's friend invites. Returns a list of revoked friend [invite](#invite-object) objects on success.
...
Update the application's role connection for the user. Returns the updated [application role connection](#application-role-connection-object) object on success.
