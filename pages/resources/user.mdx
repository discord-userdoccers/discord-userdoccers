...
^1^ Only included in [Get User](#get-user) and [Get User Profile](#get-user-profile) endpoints.
...
^4^ This value can be obtained by requesting a verification code with the [Modify User Email](#modify-user-email) endpoint, and then using the [Verify User Email Change](#verify-user-email) endpoint.
