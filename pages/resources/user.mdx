...
| avatar_decoration_data   | ?[avatar decoration data](#avatar-decoration-data-structure) | The user's [avatar decoration](https://support.discord.com/hc/en-us/articles/13410113109911-Avatar-Decorations) |
| clan?                    | ?[user clan](#user-clan-structure)                           | The primary clan the user is in                                                                                 |
...
| tag               | string    | The text of the user's clan tag (max 4 characters) |
| badge             | string    | The clan's [badge hash](/reference#cdn-formatting) |
...
| profile_effect?                 | ?[partial profile effect](#partial-profile-effect-structure) | The user's [profile effect](https://support.discord.com/hc/en-us/articles/17828465914263-Profile-Effects) |
...
| Field                         | Type                                                                          | Description                                                       |
| ----------------------------- | ----------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| suggested_user                | partial [user](#user-object) object                                           | The suggested user                                                |
...
| platform | string  | The [platform that the suggestion originated from](#connection-type) |
...
| avatar_decoration?        | ?[image data](/reference#cdn-data) | The user's avatar decoration (only available to Discord employees)                                                                                                        |
| avatar_decoration_id?     | ?snowflake                         | The ID of the user's collected avatar decoration                                                                                                                          |
| avatar_decoration_sku_id? | ?snowflake                         | The SKU ID of the user's collected avatar decoration                                                                                                                      |
...
<RouteHeader method="GET" url="/users/username/{user.username}">
  Get User by Username
</RouteHeader>

Returns a partial [user](#user-object) object for a given username.

<Alert type="warn">

Unless the target user is a bot, you must be able to add the user as a friend to resolve them by username. Due to this restriction, you are not able to resolve your own username.
...
| mutual_friends?               | array[partial [user](#user-object) object]                                                             | The mutual friends the user has with the current user                                                                  |
...
<RouteHeader method="GET" url="/users/{user.id}/profile/recent-games">
  Get User Recent Games
</RouteHeader>

Returns a list of games the user has played in the last week.

<Alert type="warn">

This endpoint has the same requirements as the above [Get User Profile](#get-user-profile) endpoint.
...
| Field        | Type                                                | Description             |
| ------------ | --------------------------------------------------- | ----------------------- |
| recent_games | array[[recent game](#recent-game-structure) object] | The user's recent games |

###### Recent Game Structure

| Field           | Type                                                                                    | Description                                                                            |
| --------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| application     | [integration application](/resources/integration#integration-application-object) object | The application representing the game                                                  |
| duration        | integer                                                                                 | Duration (in seconds) the user has played the game in the last week                    |
| last_session_id | snowflake                                                                               | A snowflake ID representing the timestamp of the last session the user played the game |
  ...
  "recent_games": [
    {
      "application": {
        ...
        "icon": "dfc652a5b75879148779dc2865270f87",
        "description": "The long-awaited sequel to the class-based first-person shooter Team Fortress Classic sports a unique cartoony visual style, more accessible gameplay, and a very large amount of updates and new features since its original release.",
        "type": 1,
        "summary": "",
        "splash": "a109376e705e71ab44e7d2d966cecc35",
        "third_party_skus": [
          {
            "id": "440",
            "sku": "440",
      ...
      "duration": 1337,
      "last_session_id": "1234563653992833025",
      "is_new": false
    }
    {
      "application": {
        "id": "1121201675240210523",
        "name": "Honkai: Star Rail",
        "icon": "444d067889922e42b0af99b13e5d5c72",
        "description": "",
        "type": 1,
        "bot": {
          "id": "1121201675240210523",
          "username": "Honkai Star Rail",
          "avatar": "444d067889922e42b0af99b13e5d5c72",
          "discriminator": "8967",
          "public_flags": 0,
          "bot": true,
          "banner": null,
      ...
      "duration": 2560,
      "last_session_id": "1234175874748911658",
      "is_new": false
    }
  ]
}
...
Deletes a connection. Returns a 204 empty response on success. Fires a [User Connections Update](/topics/gateway-events#user-connections-update) and may fire a [Guild Delete](/topics/gateway-events#guild-delete) Gateway event.
...
Removes a friend suggestion for the current user. Returns a 204 empty response on success. Fires a [Friend Suggestion Delete](/topics/gateway-events#friend-suggestion-remove) Gateway event.
...
Sets the current user's clan. Returns a partial [user](#user-object) object of before the update.

###### JSON Params

| Field             | Type       | Description                                                       |
| ----------------- | ---------- | ----------------------------------------------------------------- |
| identity_enabled  | boolean    | Whether or not the clan tag is enabled                            |
| identity_guild_id | ?snowflake | The guild ID of the clan to be associated with, or empty to clear |

<RouteHeader method="GET" url="/users/@me/join-request-guilds">
  Get Guild Join Requests
</RouteHeader>

Returns an array of partial [guild](/resources/guild#guild-object) objects representing the guilds the user or is waiting for a response on.
