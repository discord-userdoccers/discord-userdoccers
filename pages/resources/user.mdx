...
Pomelo will rollout to users over time rather than all at once. The value of a single zero (`0`) in the [`discriminator` field on the user object](#user-object) indicates that the user has ~~been pommelled~~ migrated to the new username system. Note that the discriminator for migrated users will _not_ be 4-digits like a standard discriminator (it is `0`, not `0000`). The value of the `username` field will become the migrated user's unique username.
...
To migrate, users should first [check if the username they want is available](#get-pomelo-eligibility), and then [migrate their account to that username](#pomelo-user). If the username they want is not available, they can [get a list of suggested usernames](#get-pomelo-suggestions) to choose from.
...
| username ^5^             | string    | The user's username, may be unique across the platform                                  |
| discriminator ^5^        | string    | The user's stringified 4-digit Discord tag                                              |
| global_name ^5^          | ?string   | The user's display name (2-32 characters); for bots, this is the application name       |
...
| phone? ^1^               | ?string   | The user's phone number                                                                 |
...
^3^ Not included in the user object returned over the Gateway.

^4^ Only included in the user object returned over the Gateway.
...
| ------------------ | --------- | --------------------------------------------------------------------------------------- |
| id                 | snowflake | The ID of the user                                                                      |
| username ^2^       | string    | The user's username, not unique across the platform                                     |
| discriminator ^2^  | string    | The user's stringified 4-digit Discord tag                                              |
| global_name? ^2^   | ?string   | The user's display name (2-32 characters); for bots, this is the application name       |
| avatar             | ?string   | The user's [avatar hash](/reference#cdn-formatting)                                     |
| avatar_decoration? | ?string   | The user's [avatar decoration hash](/reference#cdn-formatting)                          |
| bot?               | boolean   | Whether the user belongs to an OAuth2 application                                       |
| system?            | boolean   | Whether the user is an Official Discord System user (part of the urgent message system) |
...
^1^ Not included when [fetching a user's connections](#get-current-users) via OAuth2.
...
| application?          | [integration application](/resources/application#integration-application-object) object                           | The application that owns the role connection                                                                                                                                                                                      |
...
| Field                     | Type                                 | Description                                                                                                                                                               |
| ------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| username? ^5^             | string                               | The user's username; if changed may cause the user's discriminator to be randomized                                                                                       |
...
| global_name? ^5^          | ?string                              | The user's display name (2-32 characters)                                                                                                                                 |
| avatar?                   | ?[image data](/reference#image-data) | The user's avatar; can be animated when the user has an applicable premium (Nitro) plan                                                                                   |
...
| guild_member? ^1^             | [guild member](#guild-member-object) object                               | The guild member in the guild specified, with extra `bio` and `banner` keys denoting the guild member's bio and banner |
...
| legacy_username ^2^           | ?string                                                                   | The user's pre-migration `username#discriminator`, if applicable and shown                                             |
...
Returns a list of suggested unique username strings based on the current user's username.
...
Claims a unique username for the user. Returns a 204 empty response on success. Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
| Field         | Type   | Description                      |
| ------------- | ------ | -------------------------------- |
| nickname? ^1^ | string | The nickname of the relationship |
