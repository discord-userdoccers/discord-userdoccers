...
The data of the guild widget.

| Field           | Type                                       | Description                                                                                  |
|-----------------|--------------------------------------------|----------------------------------------------------------------------------------------------|
| type            | string | The [type of the game widget](#game-widget-type)         |
| games?          | array[[game object](#game-widget-game-structure) object]         | The widget's games. (not applicable for [`application` widgets](#game-widget-type)           |
...
|-------------------|------------------------|-----------|
| favorite_games     | Favourite game (max 1)     | true      |
| played_games      | Games I Like (max 20) | false     |
| current_games | Games in rotation (max 5)     | true      |
| want_to_play_games      | Want to play (max 20) | false     |
| application       | Specific game details        | false       |
...
| tags?    | array[string] | [Game Tags](#game-widget-game-tag) to be displayed below the game in detailed game widget |

### Game Widget Game Tag
...
^1^ Max 1 of one of these variants can be present in a game widget game's tags at a time.
...
| Field   | Type                                                         | Description                                            |
| ------- | ------------------------------------------------------------ | ------------------------------------------------------ |
| widgets | array[[partial game widget object](#game-widget-object)] ^1^ |  (max 1 of each [game widget type](#game-widget-type)) |

^1^ Field `id` is optional, and `updated_at` is not present on the partial.

<RouteHeader method="GET" url="/users/@me/widgets/suggested-games">
  Get Suggested Game Widget Games
</RouteHeader>

Returns a list of suggested application (game application) IDs to be put in the profile game widgets for the current user, based on some internal criteria.

##### Response Body

| Field          | Type             | Description                 |
| -------------- | ---------------- | --------------------------- |
| suggsted_games | array[snowflake] | The list of suggested games |
