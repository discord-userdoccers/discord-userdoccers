...
<RouteHeader method="POST" url="/users/@me/relationships" risky>
  Send Friend Request
</RouteHeader>
...
<RouteHeader method="PUT" url="/users/@me/relationships/{user.id}" risky>
...
This endpoint will only count as a [risky](/reference#risky-request) endpoint if the [relationship type](#relationship-type) is set to `FRIEND`.
