...
| avatar                 | ?string   | The user's [avatar hash](/reference#image-formatting)                                   |
| avatar_decoration      | ?string   | The user's [avatar decoration hash](/reference#image-formatting)                        |
...
| banner?                | ?string   | The user's [banner hash](/reference#image-formatting)                                   |
...
| visibility          | integer                                                                 | [Visibility](#visibility-type) of this connection                       |
...
| user     | [user](#user-object) | The partial [user](#user-object) object of the target user |
...
Modify the requester's user account settings. Returns a [user](#user-object) object on success. Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
| Field | Type   | Description                          |
| ----- | ------ | ------------------------------------ |
| token | string | The new token for the user's account |
...
Modify the current user's profile. Returns a [profile metadata](#profile-metadata-object) object on success. Fires a [User Update](#DOCS_TOPICS_GATEWAY/user-update) Gateway event.
...
Returns a list of partial [user](#DOCS_RESOURCES_USER/user-object) objects that are friends with the user and current user.
...
Returns a list of active [private channel](/resources/channel#channel-object) objects the user is participating in.

<RouteHeader method="POST" url="/users/@me/channels">
  Create Private Channel
</RouteHeader>

One recipient creates or returns an existing [DM channel](/resources/channel#channel-object), no or multiple recipients create a [group DM channel](/resources/channel#channel-object). Returns a [private channel](/resources/channel#channel-object) object. Fires a [Channel Create](/topics/gateway-events#channel-create) Gateway event.
...
<RouteHeader method="POST" url="/connections/{connection.type}/authorize" unauthenticated>
...
<RouteHeader method="GET" url="/users/@me/connections/{connection.type}/{connection.id}/subreddits">
...
Adds a relationship to another user. Returns a 204 empty response on success. Fires a [Relationship Add](/topics/gateway-events#user-relationship-create) Gateway event.
...
Removes a relationship to another user. Returns a 204 empty response on success. Fires a [Relationship Remove](/topics/gateway-events#user-relationship-delete) Gateway event.
...
| Field                 | Type                                             | Description                                     |
| --------------------- | ------------------------------------------------ | ----------------------------------------------- |
| nitro_sticker_sends   | [Premium Usage Object](#premium-usage-structure) | The number of Nitro sticker the user has sent   |
| total_animated_emojis | [Premium Usage Object](#premium-usage-structure) | The number of animated emojis the user has sent |
| total_global_emojis   | [Premium Usage Object](#premium-usage-structure) | The number of global emojis the user has sent   |
| total_large_uploads   | [Premium Usage Object](#premium-usage-structure) | The number of large uploads the user has made   |
| total_hd_streams      | [Premium Usage Object](#premium-usage-structure) | The number of streams the user has made in HD   |
| hd_hours_streamed     | [Premium Usage Object](#premium-usage-structure) | The number of hours the user has streamed in HD |
...
Updates and returns the [application role connection](#application-role-connection-object) for the user.
