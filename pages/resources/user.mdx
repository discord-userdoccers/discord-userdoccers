...
| expires_at? | ?integer | Unix timestamp (in milliseconds) of when the current avatar decoration expires |
...
| expires_at | ?integer  | Unix timestamp (in milliseconds) of when the current profile effect expires |
