...
- Servers joined in the last week
- Servers the teen has sent messages to in the last week
...
| users          | array[[partial user](/resources/user#partial-user-structure) object]      | List of requestors the linked user is connected to           |
...
| range_start_id      | ?snowflake                                                                | The ID representing the start time of the 7-day track range?               |
| actions             | array[[action](#action-structure) object]                                 | List of [actions](#action-structure) the linked user has done              |
| users               | array[[partial user](/resources/user#partial-user-structure) object]      | List of users the linked user has interacted with within 7 days            |
| guilds              | array[[guild](/resources/guild#guild-structure) object]                   | List of guilds the linked user has joined or interacted with within 7 days |
| totals              | object                                                                    | Object keyed by [display types](#display-type), with their values          |
...
| entity_id       | snowflake    | The ID of the entity the action relates to (user, guild, or group DM) based off the [display type](#display-type)   |
| display_type    | integer      | The [display type](#display-type) of the action, detailing what this action involved                                |

###### Display Type

The Display Type represents what kind of action the linked user engaged in.

| Value     | Description                                                         |
| --------- | ------------------------------------------------------------------- |
| 1         | Users added within the last 7 days                                  |
| 2         | Server joined within the last 7 days                                |
| 3         | Users messaged within the last 7 days                               |
| 4         | Servers the linked user has sent messages in within the last 7 days |
| 5         | Users called                                                        |
...
| -------------- | ------------------------------------------------------ | ------------------------------------------------------------ |
| linked_users   | array[[linked user]((#linked-user-object)) object      | List of linked users                                         |
| users          | array[[user](/resources/user#user-structure) object]   | List of requestors the linked user is connected to           |

## Endpoints

<RouteHeader method="GET" url="/family-center/@me">
  Get Family Center Data
</RouteHeader>
...
| code                 | string                             | The link code from the linked users device                   |
