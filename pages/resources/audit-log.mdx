...
| threads ^1^            | array[[channel](/resources/channel#channel-object) object]                                           | Threads referenced in the audit log                |
...
^1^ Threads referenced in `THREAD_CREATE` and `THREAD_UPDATE` events are included in the threads map because archived threads might not be kept in memory by clients.
...
| 166     | ONBOARDING_CREATE                           | Onboarding was created                                    | [Onboarding](/resources/guild#onboarding-object)                                                                |
