...
Connections are links of third party accounts to Discord account.
...
| Value                  | Description                                 |
| ---------------------- | ------------------------------------------- |
| CREATE_NEW_CALL        | Create a new call on console device         |
| TRANSFER_EXISTING_CALL | Transfer an existing call to console device |
...
Returns a nonce for connecting to voice on PlayStation console.
...
Cancels a connect request. Returns a 204 empty response on success.

<RouteHeader method="GET" url="/consoles/{connection.type}/devices">
  Get Console Devices
</RouteHeader>

Returns a list of [console device](#console-device-object) objects.
...
<RouteHeader method="POST" url="/consoles/{connection.type}/devices/{device.id}/commands">
  Send Connect Voice Command
</RouteHeader>

Sends a command to connect to a voice call on console device.
...
| nonce?     | string    | The nonce obtained from [Get Connect Nonce](#get-connect-nonce) endpoint |
...
Cancels a command. Returns a 204 empty response on success.
