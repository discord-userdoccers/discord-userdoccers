...
| verify_key                                   | string                                                                  | The hex encoded client public key for verification in interactions and the GameSDK's [GetTicket](/game-sdk/applications#getticket)                                                   |
...
| ---------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| ~~1 << 1~~ | ~~EMBEDDED_RELEASED~~              | ~~This embedded application is released to the public~~ (See [release phases](/resources/application#embedded-activity-release-phase))                     | ~~Yes~~ |
...
| 1 << 5     | RPC_PRIVATE_BETA                   | This application can access the client RPC server                                                                                                          | No      |
| 1 << 6     | AUTO_MODERATION_RULE_CREATE_BADGE  | This application has created 100+ AutoMod rules                                                                                                            | Yes     |
| 1 << 8     | ALLOW_ASSETS                       | This application can use activity assets                                                                                                                   | No      |
| 1 << 9     | ALLOW_ACTIVITY_ACTION_SPECTATE     | This application can enable spectating activities                                                                                                          | No      |
| 1 << 10    | ALLOW_ACTIVITY_ACTION_JOIN_REQUEST | This application can enable activity join requests                                                                                                         | No      |
| 1 << 11    | RPC_HAS_CONNECTED                  | This application has accessed the client RPC server before                                                                                                 | No      |
...
| 1 << 24    | ACTIVE                             | This application has had at least one global application command used in the last 30 days                                                                  | Yes     |
...
| Field  | Type                                                                            | Description                  |
| ------ | ------------------------------------------------------------------------------- | ---------------------------- |
| urlMap | array[[application proxy mapping](#application-proxy-mapping-structure) object] | The URLs mapped to the proxy |
...
Modifies an application. User must be the owner of the application or member of the current team. Returns the updated [application](#application-object) object on success.
...
Resets the application's client secret. This revokes all previous secrets and returns a new secret. User must be the owner of the application or member of the current team.
...
Creates and attaches a bot to the given application ID. User must be the owner of the application or member of the current team.
...
Modifies the application's bot. User must be the owner of the application or member of the current team. Returns the updated [user](/resources/user#user-object) object on success.
...
Resets the application's bot token. This revokes all previous tokens and returns a new token. User must be the owner of the application or member of the current team.
...
Modifies the embedded activity config for the given application ID. User must be the owner of the application or member of the current team. Returns the updated [embedded activity config](#embedded-activity-config-object) object on success.
...
Returns the [application proxy config](#application-proxy-config) object for the given application ID. User must be the owner of the application or member of the current team.

<RouteHeader method="POST" url="/applications/{application.id}/proxy-config">
  Modify Application Proxy Config
</RouteHeader>

Replaces the proxy config for the given application ID. User must be the owner of the application or member of the current team. Returns the updated [application proxy config](#application-proxy-config-object) object on success.

<RouteHeader method="GET" url="/oauth2/applications/{application.id}/assets" unauthenticated>
  Get Application Assets
</RouteHeader>

Returns a list of [application assets](/resources/application#application-asset-object) for the given application ID.

<RouteHeader method="POST" url="/oauth2/applications/{application.id}/assets">
  Create Application Asset
</RouteHeader>

Creates a new application asset for the given application ID. User must be the owner of the application or member of the current team. Returns an [application asset](/resources/application#application-asset-object) object on success.
...
| name  | string                              | The name of the asset                            |
| type  | integer                             | The [type of the asset](#application-asset-type) |
| image | [image data](/reference#image-data) | The asset's image                                |

<RouteHeader method="DELETE" url="/oauth2/applications/{application.id}/assets/{asset.id}">
  Delete Application Asset
</RouteHeader>

Deletes an application asset permanently. User must be the owner of the application or member of the current team. Returns a 204 empty response on success.

<RouteHeader method="GET" url="/applications/detectable">
  Get Detectable Applications
</RouteHeader>

Returns a list of partial [application](#application-object) objects representing games that can be detected by Discord for rich presence.

<Alert type="info">

Because of the size of this list, this endpoint's response headers will include an [`ETag` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/ETag). This can be used in conjunction with the [`If-None-Match` header](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/If-None-Match) to check if the list has changed since the last request, avoiding unnecessary large data transfer.
