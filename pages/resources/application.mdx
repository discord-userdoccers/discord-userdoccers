...
| 1 << 8      | PUBLIC_OAUTH2_CLIENT                   | Application's OAuth2 credentials are considered public and a client secret is not required                                                                 | Yes     |
...
| soft_launch           | The activity is available to all guilds                                                                             |
| soft_launch_multi_geo | The activity is available to all guilds                                                                             |
...
| artifacts ^1^     | array[[social SDK release artifact](#social-sdk-release-artifact-object) object] | The SDK artifacts             |

^1^ Only provided in [Get Social SDK Release](#get-social-sdk-release) endpoint.

###### Social SDK Release Example
...
### Social SDK Release Artifact Object
...
| size_bytes   | integer | The size of file (in bytes) |
...
| UNITY                | [Unity-based authentication](https://docs.unity.com/ugs/manual/authentication/manual/unity-player-accounts)   |

### Application External Identity Provider Client Object
...
### Application External Identity Provider Configuration Object
...
###### Application External Identity Provider Client Example
...
Returns a list of [whitelisted user](#whitelisted-user-structure) objects representing the invited testers for the given application ID. User must be the owner of the application or member of the current team.
...
Adds a user to the application's list of testers. User must be the owner of the application or developer of the current team. Returns a [whitelisted user](#whitelisted-user-structure) object on success.
...
Returns a list of social SDK releases.
...
Enables social SDK for the given application and applies the [`SOCIAL_LAYER_INTEGRATION_LIMITED` application flag](#application-flags) to the application. Returns a 204 empty response on success.
...
| business_email       | string | The business email (max 320)      |
| game_or_studio_name? | string | The name of the company (max 100) |
| game_or_studio_url?  | string | The URL of the company (max 512)  |
...
Returns a list of [application external identity provider configuration](#application-external-identity-provider-configuration-object) objects for the given application.
...
Adds or updates configuration for given external identity provider type.

###### JSON Params

| Field           | Type                                                                                                                 | Description                                  |
| --------------- | -------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| clients         | array[[application external identity provider client](#application-external-identity-provider-client-object) object] | The provider clients                         |
| oidc_issuer_url | ?string                                                                                                              | The primary URL of the OpenID Connect Issuer |
...
Remove configuration for given external identity provider. Returns a 204 empty response on success.
