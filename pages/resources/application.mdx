...
Returns a list of partial [application identity](#partial-application-identity-structure) objects for the user IDs.
...
| Field        | Type             | Description                                     |
| ------------ | ---------------- | ----------------------------------------------- |
| user_ids ^1^ | array[snowflake] | The IDs of the users to retrieve identities for |
...
<RouteHeader method="GET" url="/applications/{application.id}/users/{user.id}/identities/{external_user_id}/profile">
  Get User Application Profile
</RouteHeader>

Returns an [user application profile](/topics/gateway-events#user-application-profile-structure) object for the given application, user, and external user IDs.

<RouteHeader method="PATCH" url="/applications/{application.id}/users/{user.id}/identities/{external_user_id}/profile">
  Modify User Application Profile
</RouteHeader>

Modifies the user's application profile. Returns an [user application profile](/topics/gateway-events#user-application-profile-structure) object on success.
