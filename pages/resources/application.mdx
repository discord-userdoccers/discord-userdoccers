...
| ------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1 << 1  | EMBEDDED_RELEASED                             | Indicates if the embedded application is released to the public |
| 1 << 2  | MANAGED_EMOJI                                 | Indicates if the application has the ability to create Twitch-style emotes |
| 1 << 3  | EMBEDDED_IAP                                  | Indicates if the application has the ability to use embedded in-app purchases |
| 1 << 4  | GROUP_DM_CREATE                               | Indicates if the application has the ability to create group DMs |
| 1 << 5  | RPC_PRIVATE_BETA                              | Indicates if the application has the ability to access the client RPC server |
| 1 << 6  | APPLICATION_AUTO_MODERATION_RULE_CREATE_BADGE | Indicates if an app has created over 100 AutoMod rules |
| 1 << 8  | ALLOW_ASSETS                                  | Indicates if the application has the ability to use activity assets |
| 1 << 9  | ALLOW_ACTIVITY_ACTION_SPECTATE                | Indicates f the application has the ability to enable spectating activities |
| 1 << 10 | ALLOW_ACTIVITY_ACTION_JOIN_REQUEST            | Indicates if the application has the ability to enable activity join requests |
| 1 << 11 | RPC_HAS_CONNECTED                             | Indicates if the application has accessed the client RPC server before |
...
| 1 << 15 | GATEWAY_GUILD_MEMBERS_LIMITED                 | Intent required for bots in under 100 servers to receive member-related events like `guild_member_add`, found on the **Bot** page in your app's settings. |
...
| 1 << 24 | ACTIVE                                        | Indicates if an app has atleast one global application command used in the last 30 days |
