...
| 6     | HANG ^1^  | {state} or {emoji} {details} ^2^ | "Chilling tbh"                       |
...
| ~~1 << 3~~ | ~~JOIN_REQUEST~~ ^1^        | ~~This activity requires a request to join~~                           |
...
^1^ Activities no longer need to be explicitly flagged as join requestable.
