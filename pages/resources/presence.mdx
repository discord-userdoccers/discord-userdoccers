...
A user's presence is their current status and activity. Presences are usually per-guild, but user accounts receive overall user presences for [friends and implicit relationships](/resources/user#relationship-object).
...
| last_modified?\* | integer                                            | Unix time (in milliseconds) of when the user's presence was last modified |
...
\* This field is only sent for overall user presences.
...
| web?\*      | string | The user's [status](#status-type) on an active web (browser) application session                 |
