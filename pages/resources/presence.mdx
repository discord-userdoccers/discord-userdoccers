...
To use an external image via media proxy, use the result of the [Proxy Application Assets](/resources/application#proxy-application-assets) endpoint prefixed with `mp:`.

<Alert type="info">

Activities sent through the Social Layer SDK, [headless sessions](#create-headless-session), or RPC automatically handle proxying external images.
