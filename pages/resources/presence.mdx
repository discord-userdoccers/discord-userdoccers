...
| secrets? ^5^         | [activity secrets](#activity-secrets-structure) object       | Secrets for rich presence joining and spectating (send-only)                          |
| metadata? ^5^        | [activity metadata](#activity-metadata-object) object        | Additional metadata for the activity (send-only)                                      |
...
^3^ This field is not supported for traditional presences (i.e. presences sent by regular clients over the Gateway) and is instead used to differentiate between various headless and embedded activities.
