...
| Value | Name                           | Description                                                                                                        |
| ----- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------ |
| 0     | OTHER                          | Purchase is disqualified from a refund due to another reasons                                                      |
...
Returns a list of [payment](#payment-object) objects the current user ever made.

{/* Unsure on `limit` default */}

| Field   | Type      | Description                                           |
| ------- | --------- | ----------------------------------------------------- |
| limit   | integer   | Max number of payments to return (1-100, default 100) |
...
Voids the payment. Returns a 204 empty response on success. Fires [Payment Update](/topics/gateway-events#payment-update) Gateway event. Only applicable for payments whose status is [`PENDING`](#payment-status).
...
Fires [Payment Update](/topics/gateway-events#payment-update), [Payment Update](/topics/gateway-events#payment-update), [Virtual Currency Balance Update](/topics/gateway-events#virtual-currency-balance-update), [Payment Update](/topics/gateway-events#payment-update), [Entitlement Create](/topics/gateway-events#entitlement-create), and [Entitlement Update](/topics/gateway-events#entitlement-update) Gateway events.
