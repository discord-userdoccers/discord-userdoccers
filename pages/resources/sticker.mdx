...
| banner_asset_id?  | snowflake                                | The ID of the sticker pack's [banner image](/reference#image-formatting) |
...
| Field       | Type                                       | Description                                                                    |
| ----------- | ------------------------------------------ | ------------------------------------------------------------------------------ |
| id          | snowflake                                  | The [ID of the sticker](/reference#image-formatting)                           |
...
Returns the list of [sticker packs](#sticker-pack-object) available to Nitro subscribers.
...
Returns an [expression guild](/resources/guild#expression-guild-object) object for the guild that owns the given sticker. This endpoint requires the guild to be discoverable and have emoji/sticker discoverability enabled.
...
Create a new sticker for the guild. Send a `multipart/form-data` body. Requires the `MANAGE_EMOJIS_AND_STICKERS` permission. Returns the new [sticker](#sticker-object) object on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
...
Modify the given sticker. Requires the `MANAGE_EMOJIS_AND_STICKERS` permission. Returns the updated [sticker](#sticker-object) object on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
...
Delete the given sticker. Requires the `MANAGE_EMOJIS_AND_STICKERS` permission. Returns `204 No Content` on success. Fires a [Guild Stickers Update](/topics/gateway-events#guild-stickers-update) Gateway event.
