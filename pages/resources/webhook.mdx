...
| user? ^1^           | partial [user](/resources/user#user-object) object          | The user this webhook was created by                                                               |
...
| token? ^2^          | string                                                      | The secure token of the webhook (returned for `INCOMING` webhooks)                                 |
| application_id      | ?snowflake                                                  | The application that created this webhook                                                          |
| source_guild? ^3^   | partial [guild](/resources/guild#guild-object) object       | The guild of the channel that this webhook is following (returned for `CHANNEL_FOLLOWER` webhooks) |
| source_channel? ^3^ | partial [channel](/resources/channel#channel-object) object | The channel that this webhook is following (returned for `CHANNEL_FOLLOWER` webhooks)              |
| url? ^2^            | string                                                      | The URL used for executing the webhook (returned for `INCOMING` webhooks)                          |

^1^ Not included when using the [Get Webhook with Token](#get-webhook-with-token) endpoint.

^2^ For application-owned webhooks, this field is only returned to the application that created the webhook.

^3^ These fields will not be included if the webhook creator has since lost access to the followed channel's guild.
...
| ------- | ------------------------------------ | ------------------------------------------------- |
| name    | string                               | The default name of the webhook (1-80 characters) |
| avatar? | ?[image data](/reference#image-data) | The default avatar of the webhook                 |
