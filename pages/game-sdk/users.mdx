...
Before calling this function, you'll need to wait for the [OnCurrentUserUpdate](#oncurrentuserupdate) callback to fire after instantiating the User manager.
...
Get the [PremiumType](#premiumtype-enum) for the currently connected user.
...
See whether or not the current user has a certain [UserFlag](#userflag-enum) on their account.
...
| name | type                       | description                             |
| ---- | -------------------------- | --------------------------------------- |
| flag | [UserFlag](#userflag-enum) | the flag to check on the user's account |
