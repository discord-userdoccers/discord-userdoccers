...
^1^ This field is only provided via the above rehosted updates endpoint, and requires `install_id` and `platform_version` to be set and supported. If this field is `null`, clients are expected to generate it themselves.
