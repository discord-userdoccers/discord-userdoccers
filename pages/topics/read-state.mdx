...
^2^ `last_message_id` and `last_acked_id` are mutually exclusive; only one will be present depending on the `read_state_type`.

^3^ `mention_count` and `badge_count` are mutually exclusive; only one will be present depending on the `read_state_type`.
...
Updates the channel's read state for the current user. Fires a [Message Ack](/topics/gateway#message-ack) Gateway event.
...
| Field              | Type    | Description                                                        |
| ------------------ | ------- | ------------------------------------------------------------------ |
| token?             | ?string | The last received [ack token](#ack-token), or `null`               |
...
^2^ If omitted, the existing value is retained.

###### Response Body

| Field | Type    | Description                     |
| ----- | ------- | ------------------------------- |
| token | ?string | The new [ack token](#ack-token) |
