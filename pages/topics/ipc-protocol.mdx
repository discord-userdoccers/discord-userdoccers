...
| 1     | FRAME     | The packet includes a typical payload for communication with the client                   |
...
| nonce    | string | A unique identifier (UUID) given to a command that will be echoed back to you upon a successful send |
...
| nonce ^1^ | ?string | The unique identifier (UUID) of the requested command                                   |
