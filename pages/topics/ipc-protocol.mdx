...
By sending packets through the native Discord IPC pipe, you can programmatically control your local Discord client, like joining voice channels, retrieving guild or user information, and more. A Discord application from the [Developer Portal](https://discord.com/developers/applications) is required.
...
Once connected to the ICP socket, you can start sending the packets to begin the identification and authentication process.

<Alert type="warn">

Not all commands can be sent via the native IPC transport. However, authorization for the WebSocket transport is
extremely limited and mainly utilized by Discord itself.
...
| OPEN_INVITE_DIALOG                               | Opens the invite modal                                                                         |
| OPEN_SHARE_MOMENT_DIALOG                         | Used for activities, allowing sharing an image from the activity                               |
| SHARE_INTERACTION                                | (Unknown)                                                                                      |
| INITIATE_IMAGE_UPLOAD                            | Used by an activity to get a user-provided image from their PC, opens file dialog              |
| SHARE_LINK                                       | Opens the share message modal, where a user can pick a channel to send a predefined message to |
| DEEP_LINK                                        | (Unknown)                                                                                      |
| CONNECTIONS_CALLBACK                             | Used to send the next part of connection flow to the client, part of connections flow v2       |
| BILLING_POPUP_BRIDGE_CALLBACK                    | Callback for payment methods like PaySafeCard, Klarna, Przelewy24, etc                         |
| BRAINTREE_POPUP_BRIDGE_CALLBACK                  | Callback for payment via Braintree                                                             |
| GIFT_CODE_BROWSER ^2^                            | Opens a given gift code link in the client                                                     |
| GUILD_TEMPLATE_BROWSER ^2^                       | Opens a given template link in the client                                                      |
| OPEN_MESSAGE                                     | Used to open a specific message or DM in the client                                            |
| OVERLAY                                          | (Unknown)                                                                                      |
...
| OPEN_OVERLAY_GUILD_INVITE                        | Opens the invite modal in the locally running client for a provided code                       |
| OPEN_OVERLAY_VOICE_SETTINGS                      | Opens a unique voice settings window for the application in the client                         |
| VALIDATE_APPLICATION                             | (Unknown)                                                                                      |
| GET_ENTITLEMENT_TICKET                           | Old store integration                                                                          |
| GET_APPLICATION_TICKET                           | Old store integration                                                                          |
| START_PURCHASE                                   | Launches the purchase flow for a specific SKU, per the sku_id                                  |
| START_PREMIUM_PURCHASE                           | Initiates a premium subscription purchase                                                      |
| GET_SKUS                                         | Returns a list of your app's SKUs                                                              |
| GET_ENTITLEMENTS                                 | Returns a list of entitlements for the current user                                            |
...
| OPEN_EXTERNAL_LINK ^1^                           | Prompts to open a given URL in the default web browser from the client                         |
| CAPTURE_LOG ^1^                                  | Forward logs to your own logger                                                                |
| ENCOURAGE_HW_ACCELERATION ^1^                    | Presents a modal dialog to allow enabling of hardware acceleration                             |
...
| GET_PROVIDER_ACCESS_TOKEN ^1^                    | Used for the Amazon Music activity so it can get the access token from Amazon connection       |
| MAYBE_GET_PROVIDER_ACCESS_TOKEN ^1^              | Used for the Amazon Music activity so it can get the access token from Amazon connection       |
| NAVIGATE_TO_CONNECTIONS ^1^                      | Used for Amazon Music, opens the connections page in settings                                  |
| INVITE_USER_EMBEDDED                             | (Unknown)                                                                                      |
| INVITE_BROWSER ^2^                               | Used to open an invite modal for a guild invite in the client                                  |

^1^ Used by Activities, not available from native IPC transport

^2^ Used by the WebSocket transport
...
| THERMAL_STATE_UPDATE                       | An update of the thermal states of mobile devices, via Discord's abstraction over Apple/Android APIs     |
...
| ENTITLEMENT_CREATE                         | Sent when an entitlement is created when a user purchases or is otherwise granted one of your app's SKUs |
...
| VIDEO_STATE_UPDATE                         | Sent when a user's voice state changes in a subscribed voice channel (mute, volume, etc.)                |
