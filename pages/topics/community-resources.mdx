...
Using Discord's [Dispatch](/docs/dispatch/dispatch-and-you) tool for game developers publishing on Discord can sometimes involve using the same long commands multiple times. The following open-source tool helps shorten these commands for you. It will also provide webhook support for when you're pushing an update.
...
[Gateway Intents](/docs/topics/gateway/gateway-intents) are pretty confusing at first. If you're not sure what to send in your [identify payload](/docs/topics/gateway/identify), then these tools may be of help:
