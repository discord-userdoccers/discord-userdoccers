...
| 4003     | Not authenticated           | You sent us a payload prior to [identifying](/topics/gateway#identifying).                                                                                                                                             |
...
| 15     | Media Sink Wants           | Send/Receive | Indicate the media streams wanted.                                           |
| 16     | Voice Backend Version      | Send/Receive | Version information about the voice backend.                                 |
| ~~17~~ | ~~Channel Options Update~~ | ~~Receive~~  | ~~Indicate an update in the RTC log ephemeral key.~~                         |
| 18     | Client Flags               | Receive      | Indicate a client's flags.                                                   |
| ~~19~~ | ~~Speed Test~~             | ~~Send~~     | ~~Start a speed test.~~                                                      |
...
A special `40333` JSON error code is returned if your request is blocked by CloudFlare. This may be due to a malformed request or improper user agent. The response resembles a normal error structure:
