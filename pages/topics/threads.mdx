...
When a client loses access to a channel, the gateway does not send them a [Thread Delete](/topics/gateway#thread-delete) event or any equivalent. They will still receive _an_ event when this happens, it just will not be a thread-specific event. Usually the event will be [Guild Role Update](/topics/gateway#guild-role-update), [Guild Member Update](/topics/gateway#guild-member-update) or [Channel Update](/topics/gateway#channel-update). It will be some event that caused the permissions on a channel to change. So _if_ a user wanted to simulate a "lost access to thread" event, it is entirely possible, albeit quite complicated to handle all cases correctly. Under the hood, Discord's clients actually don't worry about this detail. Instead, when performing an action, the client checks permissions first (which implicitly checks if the client has access to the parent channel too, since threads inherit permissions), that way _even if_ the client has some stale data, it does not end up acting on it.
...
- The `last_message_id` field on the forum channel object tracks the id of the most recently created thread. It has the same behavior and requirements as it does for messages, namely that you will not receive a `CHANNEL_UPDATE` when it is changed. Instead clients should update the value when receiving [Thread Create](/topics/gateway#thread-create).
