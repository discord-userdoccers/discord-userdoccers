...
| Name                                            | Description                                |
| ----------------------------------------------- | ------------------------------------------ |
| [Identify](#identify-structure)                 | Starts a new voice connection              |
| [Resume](#resume-structure)                     | Resumes a connection                       |
| [Heartbeat](#example-heartbeat)                 | Maintains an active gateway connection     |
| [Select Protocol](#select-protocol-structure)   | Selects the voice protocol and mode        |
| [Voice Backend Version](#voice-backend-version) | Requests the current voice backend version |

###### Gateway Events

| Name                                                  | Description                                                                                              |
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------- |
| [Hello](#hello-structure)                             | Sent immediately after connecting, contains heartbeat information                                        |
| [Ready](#ready-structure)                             | Contains SSRC, IP/Port, experiment, and encryption mode information                                      |
| [Resumed](#resumed)                                   | Acknowledges a successful resume                                                                         |
...
| [Session Update](#session-update-structure)           | A change in the session description                                                                      |
| [Client Disconnect](#other-client-disconnection)      | A user has disconnected from voice                                                                       |
| [Voice Backend Version](#voice-backend-version)       | The current voice backend version requested by the client                                                |

## Connecting to Voice

### Retrieving Voice Server Information

The first step in connecting to a voice server (and in turn, a guild's voice channel or private channel) is formulating a request that can be sent to the [Gateway](/topics/gateway#gateways), which will return information about the voice server we will connect to. Because Discord's voice platform is widely distributed, users **should never** cache or save the results of this call. To inform the gateway of our intent to establish voice connectivity, we first send an [Update Voice State](/topics/gateway#update-voice-state) payload.

If our request succeeded, the gateway will respond with _two_ events—a [Voice State Update](/topics/gateway#voice-state-update) event and a [Voice Server Update](/topics/gateway#voice-server-update) event—meaning you must properly wait for both events before continuing. The first will contain a new key, `session_id`, and the second will provide voice server information we can use to establish a new voice connection.
...
Once we retrieve a `session_id`, `token`, and `endpoint` information, we can connect and handshake with the voice server over another secure WebSocket. Unlike the gateway endpoint we receive in an HTTP [Get Gateway](/topics/gateway#get-gateway) request, the endpoint received from our [Voice Server Update](/topics/gateway#voice-server-update) payload does not contain a URL protocol, so some libraries may require manually prepending it with "wss://" before connecting. Once connected to the voice WebSocket endpoint, we can send an Identify payload with our `server_id`, `user_id`, `session_id`, and `token`:
...
In order to maintain your WebSocket connection, you need to continuously send heartbeats at the interval determined in [Opcode 8 Hello](/topics/opcodes-and-status-codes#voice):

###### Hello Structure

| Field              | Type    | Description                                                           |
| ------------------ | ------- | --------------------------------------------------------------------- |
| v?                 | integer | The [voice gateway version](#voice-gateway-versions)                  |
...
Once we receive the properties of a voice server from our [Opcode 2 Ready](#ready-structure) payload, we can proceed to the final step of voice connections, which entails establishing and handshaking a connection for voice data. First, we connect to the IP and port provided in the Ready payload. We then send a [Opcode 1 Select Protocol](/topics/opcodes-and-status-codes#voice) with details about our connection:
...
| experiments?       | array[string]                                        | The [received voice experiments](#ready-payload) to enable |
...
| xsalsa20_poly1305_lite_rtpsize\*  | XSalsa20 Poly1305 (Lite, RTP Size) | The nonce bytes are 4 bytes appended to the payload of the RTP packet, with the size of the RTP packet included in the nonce | Incremental 4 bytes (32bit) int value |
| aead_aes256_gcm                   | AEAD AES256 GCM                    | The nonce bytes are the RTP header                                                                                           | Copy the RTP header                   |
| aead_aes256_gcm_rtpsize\*         | AEAD AES256 GCM (RTP Size)         | The nonce bytes are the RTP header, with the size of the RTP packet included in the nonce                                    | Copy the RTP header                   |
| aead_xchacha20_poly1305_rtpsize\* | AEAD XChaCha20 Poly1305 (RTP Size) | The nonce bytes are the RTP header, with the size of the RTP packet included in the nonce                                    | Copy the RTP header                   |

\* **\[Citation Needed\]**

Finally, the voice server will respond with a [Opcode 4 Session Description](#gateway-events) that includes the `mode` and `secret_key`, a 32 byte array used for [encrypting and sending](#encrypting-and-sending-voice) voice data:
...
UDP is the most likely protocol that clients will use. First, we open a UDP connection to the IP and port provided in the Ready payload. If required, we can now perform an [IP Discovery](#ip-discovery) using this connection. Once we've fully discovered our external IP and UDP port, we can then tell the voice WebSocket what it is by sending a [Select Protocol](#select-protocol-payload) as outlined above, and receive our [Session Description](#session-description-payload) to begin sending/receiving voice data.
...
Voice data sent to Discord should be encoded with [Opus](https://www.opus-codec.org/), using two channels (stereo) and a sample rate of 48kHz. Voice Data is sent using a [RTP Header](https://www.rfcreader.com/#rfc3550_line548), followed by encrypted Opus audio data. Voice encryption uses the key passed in [Session Description](/topics/opcodes-and-status-codes#voice) and the nonce formed with the 12 byte header appended with 12 `null` bytes, if required. Discord encrypts with the [libsodium](https://download.libsodium.org/doc/) encryption library.
...
You must send at least one Speaking payload before sending voice data, or you will be disconnected with an Invalid SSRC error.
...
If successful, the voice server will respond with an [Opcode 9 Resumed](#gateway-payloads) to signal that your client is now resumed:
...
When a user disconnects from voice, the voice server will send an [Opcode 13 Client Disconnect](#gateway-payloads) payload:
...
###### Example Voice Backend Version (Request)
...
In response, the voice server will send an [Opcode 16 Voice Backend Version](#gateway-payloads) with the version:
