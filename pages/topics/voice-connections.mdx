...
| [Media Sink Wants](#dynamic-media-quality)                | Indicate the desired media stream quality |
...
| [Media Sink Wants](#dynamic-media-quality)                          | Requested media stream quality updated                                                                               |
...
Voice data sent to and received from Discord should be encoded or decoded with [Opus](https://www.opus-codec.org/), using two channels (stereo) and a sample rate of 48kHz. Video data should be encoded or decoded using the RFCs relevant to the codec specified. RTC data is sent using a [RTP Header](https://www.rfcreader.com/#rfc3550_line548), followed by encrypted Opus audio data. Encryption uses the key passed in [Session Description](#session-description-structure) and the nonce formed with the 12 byte header appended with 12 `null` bytes, if required. Discord encrypts with the [libsodium](https://download.libsodium.org/doc/) encryption library.
...
| Payload Type    | Unsigned byte                 | The [type of payload](#codec-structure) (`0x78` with the default Opus configuration) | 1 byte  |
...
The voice server supports dynamic media quality, allowing clients to send multiple video streams of different qualities and adjust the quality of the video stream they receive.
This can be used to lower the quality of a video stream when the user is not in focus, or to disable the transmission of a voice or video stream entirely when a user is off-screen or a client has muted them.
