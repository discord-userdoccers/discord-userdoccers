...
| [Speaking](#speaking-structure)                 | Indicates the user's speaking state        |
...
| [Session Update](#session-update-structure)           | Informs of a change in the session description                                                           |
| [Speaking](#speaking-structure)                       | Indicates a user's speaking state                                                                        |
| [Client Disconnect](#other-client-disconnection)      | Indicates a user has disconnected from voice                                                             |
| [Voice Backend Version](#voice-backend-version)       | Indicates the current voice backend version, as requested by the client                                  |
...
Once we receive the properties of a voice server from our [Ready](#ready-structure) payload, we can proceed to the final step of voice connections, which entails establishing and handshaking a connection for voice data. First, we connect to the IP and port provided in the Ready payload. We then send a [Opcode 1 Select Protocol](#gateway-events) with details about our connection:
...
Finally, the voice server will respond with a [Opcode 4 Session Description](#gateway-events) that includes the `mode` and `secret_key`, a 32 byte array used for [sending and receiving](#sending-and-receiving-voice) voice data:
...
UDP is the most likely protocol that clients will use. First, we open a UDP connection to the IP and port provided in the Ready payload. If required, we can now perform an [IP Discovery](#ip-discovery) using this connection. Once we've fully discovered our external IP and UDP port, we can then tell the voice WebSocket what it is by sending a [Select Protocol](#select-protocol-structure) as outlined above, and receive our [Session Description](#session-description-payload) to begin sending/receiving voice data.
...
| user_id?   | snowflake | The user ID of the speaking user (receive only) |
| delay?     | integer   | The speaking packet delay (send only)           |

\* For v4 and below, this field is a boolean.

###### Example Speaking (Request)
...
In response, the voice server will send an [Opcode 16 Voice Backend Version](#gateway-payloads) payload with the versions:

###### Example Voice Backend Version (Response)
