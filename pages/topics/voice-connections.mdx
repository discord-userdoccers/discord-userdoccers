...
| aead_aes256_gcm                   | AEAD AES256 GCM                    | The nonce bytes are the RTP header                                                                                           | Incremental 4 bytes (32bit) int value |
| aead_aes256_gcm_rtpsize\*         | AEAD AES256 GCM (RTP Size)         | The nonce bytes are the RTP header, with the size of the RTP packet included in the nonce                                    | Incremental 4 bytes (32bit) int value |
| aead_xchacha20_poly1305_rtpsize\* | AEAD XChaCha20 Poly1305 (RTP Size) | The nonce bytes are the RTP header, with the size of the RTP packet included in the nonce                                    | Incremental 4 bytes (32bit) int value |
...
\*\* This field is only sent by the voice server.

\*\*\* This field is not sent by the voice server.
