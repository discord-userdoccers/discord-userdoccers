...
To ensure that you have the most up-to-date information, please use [version 8](#gateway-versions). Otherwise, we cannot guarantee that the events and commands documented here will reflect what you receive over the socket. Video is only fully supported on Gateway v5 and above.

###### Gateway Versions

<Alert type="info">

The Voice Gateway does not provide a default version. You must explicitly pass a version with the `?v=` query parameter.
...
###### Example Heartbeat ACK Before v8
...
###### Example Heartbeat ACK Since v8
...
First, we connect to the IP and port provided in the Ready payload. We then send an [Opcode 1 Select Protocol](#gateway-events) with details about our connection:
