...
To ensure that you have the most up-to-date information, please use [version 4](#gateway-versions). Otherwise, we cannot guarantee that the [Opcodes](/topics/opcodes-and-status-codes#voice) documented here will reflect what you receive over the socket.
...
If the resume is unsuccessful—for example, due to an invalid session—the WebSocket connection will close with the appropriate [close event code](/topics/opcodes-and-status-codes#voice-close-event-codes). You should then follow the [Connecting](#connecting-to-voice) flow to reconnect.
