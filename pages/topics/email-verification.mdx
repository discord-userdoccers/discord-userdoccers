...
## Reverifying an Email Address

Users may be prompted to reverify their email address for [anti-abuse purposes](/resources/user#required-action-type).

In this case, a verification link will be automatically sent to the email address on file. If the user has not received a link, clients can [choose to resend it](#resend-verification-code).
...
Users cannot remove the registered email address from their account. Instead, they must [change it to a new one](#changing-email-addresses) or [delete their account entirely](/resources/user#delete-user-account).
