...
For bots with [elevated permissions](/topics/permissions#bitwise-permission-flags) (permissions with a `*` next to them), we enforce two-factor authentication on the owner's account when added to guilds that have guild-wide MFA enabled.
