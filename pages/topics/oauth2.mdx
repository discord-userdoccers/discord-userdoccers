...
<RouteHeader method="POST" url="/partner-sdk/token">
  Get Provisional Account Token
</RouteHeader>

Retrieves an authentication token of provisional account for the given credentials.

If account wasn't found but provided external authentication token was valid, a new provisional account will be created.
...
^1^ Required if the application does not have [`PUBLIC_OAUTH2_CLIENT` application flag](/resources/application#application-flags).
...
| access_token   | string        | The token of the provisional account                |
| expires_in     | integer       | Duration (in seconds) after which the token expires |
| scope          | string        | The scopes for the token (separated by space)       |
| id_token       | string        | The ID token                                        |
| refresh_token? | string        | The separate token used to refresh access token     |
| scopes?        | array[string] | The scopes for the token                            |
| expires_at_s?  | integer       | When the token expires (in seconds)                 |
...
{
  "token_type": "Bearer",
  "access_token": "MTE2OTQyMTc2MTg1OTgzMzk5Nw.bqVojV8HtgGiA6eeujrAFYLQcAblm0",
  "expires_in": 3600,
  "scope": "sdk.social_layer account.global_name.update openid",
  "id_token": "..."
}
```
