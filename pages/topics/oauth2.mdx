...
Retrieves an authentication token for a provisional account with the given credentials. Returns an [access token](#oauth2-access-token-object) object on success.
...
<RouteHeader method="POST" url="/partner-sdk/token/bot">
  Get Provisional Account Token By External User ID
</RouteHeader>
...
Retrieves an authentication token for a provisional account by the external user ID. Returns an [access token](#oauth2-access-token-object) object on success.

###### JSON Params

| Field                  | Type    | Description                                                       |
| ---------------------- | ------- | ----------------------------------------------------------------- |
| external_user_id       | string  | The external ID of the user (max 1024 characters)                 |
| preferred_global_name? | ?string | The display name to use for provisional account (1-32 characters) |
