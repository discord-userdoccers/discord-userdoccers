...
Retrieves an authentication for a provisional account with the given credentials. Returns an [access token](#oauth2-access-token-object) object with a refresh token on success.
