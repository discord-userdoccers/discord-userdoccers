...
If you want your clients to be able to refresh their own tokens automatically, you will need to set your Application's OAuth2 to be a `Public Client` in the Developer Portal.

By using PKCE, your application can also utilize custom scheme redirect URIs, such as `myapp://auth`, when registered in the Developer Portal.
...
If you made your application public in the Developer Portal, you can now [refresh the token](#refresh-token-exchange-example) on the client, ommitting the `client_secret` field.
