...
We support the authorization code grant, implicit grant, device grant, client credentials, and some modified special-for-Discord flows for bots and webhooks.
...
These are a list of all the OAuth2 scopes that Discord supports. Some scopes require approval from Discord to use. Requesting them from a user without approval from Discord will lead to unexpected error behavior in the OAuth2 flow.
...
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| account.global_name.update               | Allows [updating the user's global name](/resources/user#modify-user-account)                                                                             | No     |
...
| applications.entitlements                | Allows [managing entitlements for the user's applications](/resources/entitlements)                                                                       | Yes    |
...
| guilds.members.read                      | Allows [retrieving a user's member information in a guild](/resources/guild#get-current-member)                                                           | Yes    |
...
| openid                                   | Allows [retrieving basic user information](#get-openid-user-information) and includes an ID token in the authorization response                           | Yes    |
...
| role_connections.write ^3^               | Allows [updating a user's connection and application-specific metadata](/resources/application#update-role-connection#application-role-connection-object) | Yes    |
...
| voice ^2^ ^6^                            | Allows connecting to voice on the user's behalf and seeing all voice members in a guild                                                                   | No     |
| webhook.incoming ^7^                     | Creates an application-owned webhook in a user-selected channel and returns it in the authorization response                                              | Yes    |
...
^4^ Unless the application is approved for general RPC access, the `rpc` scope is allowed for the application owner and [whitelisted users](/resources/application#get-application-whitelist) only and requires that the [`EMBEDDED` application flag](/resources/application#application-flags) is not set.
...
While Discord does not require the use of the `state` parameter, it supports it and we highly recommend that you implement it for the security of your own applications and data.
...
| scope?                   | string    | The scopes to request, separated by space; only applicable for `client_credentials` grants                                                                      |
...
Retrieves an authentication token for a provisional account with the given credentials.
