...
| 1 << 9  | USER_SETTINGS_PROTO                   | Removes the `user_settings` field from the [Ready](/topics/gateway-events#ready) event, and prevents [User Settings Update](/topics/gateway-events#user-settings-update) events; the `user_settings_proto` field and User Settings Proto Update event is used instead                                                                          |
...
| 1 << 14 | PASSIVE_GUILD_UPDATE_V2 ^2^           | Enables passive guild updates v2, allowing the client to receive Passive Update v2 events instead of Channel Unreads Updateevents for guilds it is not subscribed to                                                                                                                                                                           |
