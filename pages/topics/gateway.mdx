...
| 1 << 12 | AUTO_CALL_CONNECT                     | Connects the client to all pre-existing calls upon connecting to the Gateway; this means clients will receive [Call Connect](/topics/gateway-events#call-connect) events for all calls created before the Gateway connection was established without needing to send a [Request Call Connect](/topics/gateway-events#request-call-connect) first |
...
| 1 << 14 | PASSIVE_GUILD_UPDATE_V2 ^2^           | Enables passive guild updates v2, allowing the client to receive [Passive Update v2](/topics/gateway-events#passive-update-v2) events instead of [Channel Unreads Update](/topics/gateway-events#channel-unreads-update) events for guilds it is not subscribed to                                                                               |
