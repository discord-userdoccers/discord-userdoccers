...
Packets sent from the client to the Gateway API are encapsulated within a [gateway payload object](#example-gateway-dispatch) and must have the proper opcode and data object set. The payload object can then be serialized in the format of choice (see [ETF/JSON](#etf/json)), and sent over the websocket. Payloads to the gateway are limited to a maximum of 4096 bytes sent, going over this will cause a connection termination with error code 4002.
...
| --------- | ------- | --------------------------------------------- | ----------------------------------------- |
| v         | integer | Gateway Version to use                        | see [Gateway versions](#gateway-versions) |
...
Any time the client sends a heartbeat, the gateway will respond with [Opcode 11 Heartbeat ACK](#example-gateway-heartbeat-ack), a successful _acknowledgement_ of their last heartbeat:
...
If you specify an `intent` value in your `IDENTIFY` payload that is _invalid_, the socket will close with a [`4013` close code](/topics/opcodes-and-status-codes#gateway-close-event-codes). An invalid intent is one that is not meaningful and not documented above.

If you specify an `intent` value in your `IDENTIFY` payload that is _disallowed_, the socket will close with a [`4014` close code](/topics/opcodes-and-status-codes#gateway-close-event-codes). A disallowed intent is a privileged intent that has not been approved for your bot.
...
| status     | string                                        | the user's new [status](#status-types)                                                      |
...
| Field       | Type                                                                            | Description                                                                               |
| ----------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| v           | integer                                                                         | [gateway version](#gateway-versions)                                                      |
...
Sent when a message is created. The inner payload is a [message](/resources/message#message-object) object.

#### Message Update

Sent when a message is updated. The inner payload is a [message](/resources/message#message-object) object.
...
Sent when a user in a guild uses an [Application Command](/interactions/application-commands/). Inner payload is an [Interaction](/interactions/receiving-and-responding#interaction-structure).
...
<RouteHeader method="GET" url="/gateway" requestDoesNotRequireAuthorizationHeader>
  Get Gateway
</RouteHeader>
