...
Unlike persistent messages, ephemeral messages are sent directly to the user and the app who sent the message rather than through the guild channel. Because of this, ephemeral messages are tied to the [`DIRECT_MESSAGES` intent](#list-of-intents), and the message object won't include `guild_id` or `member`.
