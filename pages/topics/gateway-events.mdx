...
Used to request the last messages (indicated by the `last_message_id` field) from channels. Responds with a single [Last Messages](#last-messages) event with up to 100 messages.
...
| channel_ids ^1^ | array[snowflake] | The IDs of the channels to request last messages for |

^1^ You can only provide up to 100 channel IDs. If you send more than that, the excessive ones will be ignored.
