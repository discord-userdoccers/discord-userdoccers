...
| presences ^6^                | array[[presence](/resources/presence#presence-object)]                     | The presences of the user's non-offline [friends and implicit relationships](/resources/user#relationship-object)                                                                                                                                                |
| merged_presences ^6^         | [merged presences](#merged-presences-structure) object                     | The presences of the user's non-offline [friends and implicit relationships](/resources/user#relationship-object), and any guild presences sent at startup                                                                                                       |
| merged_members ^6^ ^7^       | array[array[[guild member](/resources/guild#guild-member-object) object]]  | The members of the user's guilds                                                                                                                                                                                                                                 |
...
| presences ^2^              | array[[presence update](#presence-update) object]                                                    | Presences of included guild members; will only include non-offline members for large guilds    |
...
^1^ If a bot does not have the `GUILD_PRESENCES` [Gateway intent](/topics/gateway#gateway-intents), or if the guild has over 75k members, members and presences returned in this event will only contain your bot and users in voice channels.

^1^ For bots without the `GUILD_PRESENCES` [Gateway intent](/topics/gateway#gateway-intents), user accounts, or guilds with over 75k members, this will only include the client's member and users in voice channels.

^2^ User accounts only receive presences for non-offline [friends and implicit relationships](/resources/user#relationship-object). Bots with the `GUILD_PRESENCES` [Gateway intent](/topics/gateway#gateway-intents) receive all presences.
