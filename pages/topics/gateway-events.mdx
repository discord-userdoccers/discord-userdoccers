...
Used to search the recently joined members in a guild. Fires a [Guild Members Chunk](#guild-members-chunk) Gateway event with up to 1000 members that match the request.
...
| query ^1^               | string     | String that the username/nickname starts with, or an empty string to return all members |
| continuation_token ^2^  | ?snowflake | The calculated token to continue pagination from |

^1^ When a non-empty string the resultant `GUILD_MEMBERS_CHUNK` contains only one member.
^2^ The `continuation_token` is a member id calculated by the following Python pseudocode:
...
def calculate_new_continuation_token(guild_id: str, sorted_member_ids: list[str], state: State) -> str | None:
  ...
  # the array is empty until a `GUILD_MEMBERS_CHUNK` event received after a Search Recent Members call,
  # sorting can be performed by converting the `joined_at` ISO8601 timestamp in the guild member object
  # to a UNIX timestamp and sorting the result in descending numerical order
