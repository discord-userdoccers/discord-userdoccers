...
Requires `activities.write` or `presences.write` scope if operating in OAuth2 context.
...
Requires `voice` scope if operating in OAuth2 context.
...
Requires `lobbies.write` scope if operating in OAuth2 context.
...
Sent when the client user settings are modified. Inner payload is a partial [user settings](/resources/user-settings#user-settings-object) object with modified fields.
