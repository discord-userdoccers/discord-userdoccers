...
| guild_join_requests ^1^      | array[[guild join request](/resources/guild#guild-join-request-object)]   | Active join requests the user has to non-previewable guilds                                                                                                                                                                                                    |
...
Sent when a user creates a guild join request. Requires the `MANAGE_GUILD` permission for users other than the current user.

<Alert type="info">

Guild join requests created with a status of [`STARTED`](/resources/guild#guild-join-request-status) are only sent for the current user. For other users, this event is only sent for submitted join requests. If a join request is created and then submitted, a [Guild Join Request Update](#guild-join-request-update) event will be received instead.
...
Sent when a guild join request is updated. Requires the `MANAGE_GUILD` permission for users other than the current user.
...
Sent when a guild join request is deleted. Requires the `MANAGE_GUILD` permission for users other than the current user.
