...
Sent when the current user receives an activity invite but the session doesn't have `dm_channels.messages.read` scope.

<Alert type="warn">

This event is received only in OAuth2 context.
...
Sent when a private channel is partially updated. Unlike [Channel Update](#channel-update), this is sent when the field `last_message_id` is altered.

###### Channel Update Partial Update

| Field           | Type      | Description                                                                             |
| --------------- | --------- | --------------------------------------------------------------------------------------- |
| id              | snowflake | The ID of the channel                                                                   |
| last_message_id | snowflake | The ID of the last message sent in this channel (may not point to an existing resource) |
...
| moderation_metadata? | map[string, string]                                                 | Custom moderation metadata for the message (max 25 keys, 1024 characters per key and value)                                                             |
