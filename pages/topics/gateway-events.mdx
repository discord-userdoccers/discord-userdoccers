...
| [QoS Heartbeat](#qos-heartbeat)                               | Send a QOS (Quality of Service) heartbeat                         |
...
Requires `activities.write` or `presences.write` scope when operating in OAuth2 context.
...
Requires `voice` scope when operating in OAuth2 context.
...
Requires `lobbies.write` scope when operating in OAuth2 context.
...
Sent along with the normal [heartbeat](#heartbeat) to track [Quality of Service (QoS)](https://en.wikipedia.org/wiki/Quality_of_service) statistics.
...
| Field   | Type          | Description                                                    |
| ------- | ------------- | -------------------------------------------------------------- |
| ver     | integer       | The client heartbeat version (currently `25`)                  |
| active  | boolean       | If the session is currently active (when reasons has elements) |
...
| Name          | Description                                  |
| ------------- | -------------------------------------------- |
| foregrounded  | When the window is focused                   |
| rtc_connected | When the user is connected to the voice call |
...
| Field                    | Type    | Description                                                                                                         |
| ------------------------ | ------- | ------------------------------------------------------------------------------------------------------------------- |
| initialization_timestamp | fixed64 | Unix timestamp (in milliseconds) of when the client was initialized                                                 |
...
| application?                           | [gateway application](#gateway-application-structure) object                                                               | The application of the connected user or OAuth2 application                                                                                                                                                                                                            |
...
This object will be given only in OAuth2 context. As such, certain fields will be missing or empty if the user does not have required scope.

| Field                  | Type                                                                          | Description                                                                                                                                                                                                                                                            | Required Scopes                                                  |
| ---------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| \_trace                | array[string]                                                                 | An array of stringified JSON values representing the connection trace, used for debugging                                                                                                                                                                              |                                                                  |
| users ^1^              | array[partial [user](/resources/user#user-object) object]                     | The deduped users across all objects in the event                                                                                                                                                                                                                      |                                                                  |
| user_settings          | [user settings](/resources/user-settings#user-settings-object) object         | The client settings for the user                                                                                                                                                                                                                                       |                                                                  |
| user                   | [user](/resources/user#user-object) object                                    | The connected user                                                                                                                                                                                                                                                     |                                                                  |
| sessions               | array[[session](/resources/presence#session-object) object]                   | The sessions that are currently active for the user                                                                                                                                                                                                                    |                                                                  |
| session_id             | string                                                                        | Unique session ID, used for resuming connections                                                                                                                                                                                                                       |                                                                  |
| scopes                 | array[string]                                                                 | The [OAuth2 scopes](/topics/oauth2#oauth2-scopes) the user has authorized for the application                                                                                                                                                                          |                                                                  |
| resume_gateway_url     | string                                                                        | WebSocket URL for resuming connections                                                                                                                                                                                                                                 |                                                                  |
| relationships          | array[[relationship](/resources/relationships#relationship-object)]           | The relationships the user has with other users                                                                                                                                                                                                                        | `relalationships.read`                                           |
| private_channels       | array[[channel](/resources/channel#channel-object) object]                    | The DMs and group DMs the user is participating in                                                                                                                                                                                                                     | `dm_channels.read`                                               |
| presences? ^1^         | array[[presence](/resources/presence#presence-object) object]                 | The presences of the user's non-offline [friends and implicit relationships](/resources/relationships#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)).                                         | (`activities.read` or `presences.read`) and `relationships.read` |
| merged_presences ^1^   | [merged presences](#merged-presences-structure) object                        | The presences of the user's non-offline [friends and implicit relationships](/resources/relationships#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)), and any guild presences sent at startup | (`activities.read` or `presences.read`) and `relationships.read` |
| merged_members ^1^ ^2^ | array[array[[guild member](/resources/guild#guild-member-object) object]]     | The members of the user's guilds, in the same order as the `guilds` array                                                                                                                                                                                              | `guilds` and `guilds.members.read`                               |
| guilds? ^1^            | array[[gateway guild](#gateway-guild-object) object]                          | The guilds the user is in                                                                                                                                                                                                                                              | `guilds`                                                         |
| game_relationships     | array[[game relationship](/resources/relationships#game-relationship-object)] | The game relationships the user has with other users                                                                                                                                                                                                                   | `relationships.read`                                             |
| feature_flags          | [gateway feature flags](#gateway-feature-flags-structure) object              | Social layer SDK feature flags                                                                                                                                                                                                                                         |                                                                  |
| av_sf_protocol_floor   | integer                                                                       | The minimum supported version of the DAVE protocol in eligible voice connection                                                                                                                                                                                        |                                                                  |
| application            | [gateway application](#gateway-application-structure) object                  | The application of the connected user or OAuth2 application                                                                                                                                                                                                            |                                                                  |
| analytics_token        | string                                                                        | The token used for analytical tracking requests                                                                                                                                                                                                                        |                                                                  |
| lobbies? ^3^           | array[[gateway lobby](#gateway-lobby-object) object]                          | The lobbies the connected user is in                                                                                                                                                                                                                                   | `lobbies.write`                                                  |

^1^ When using the `DEDUPE_USER_OBJECTS` [Gateway capability](/topics/gateway#gateway-capabilities), `presences`, as well as each guild's `presences` array, is replaced by `merged_presences`. In addition, each guild's `members` array will be collapsed into `merged_members`. Finally, the `users` array will contain the user objects for every user in the event. Any user object in the event will be omitted, with an ID left in its place (e.g. `user_id` in [member](/resources/guild#guild-member-object) objects, `recipient_ids` in [private channel](/resources/channel#channel-object) objects, etc.).

^2^ When using the `PRIORITIZED_READY_PAYLOAD` [Gateway capability](/topics/gateway#gateway-capabilities), `merged_members` in Ready will only include the client's [member](/resources/guild#guild-member-object) object for each guild. The rest will be sent in the [Ready Supplemental](#ready-supplemental) event. See the [gateway guild](#gateway-guild-object) object documentation for more information about included data.

^3^ Requires the `AUTO_LOBBY_CONNECT` [Gateway capability](/topics/gateway#gateway-capabilities).
