...
Sent by the client to update user's voice states in multiple lobbies. Fires a [Lobby Voice State Update](#lobby-voice-state-update) and optionally [Lobby Voice Server Update](#lobby-voice-server-update) Gateway event.

The inner `d` key is array of following objects:
...
{/* TODO: it seems gateway completely ignores preferred_region and preferred_regions, but sent by SDK??? Maybe remove? */}
...
Used to request the number of members that can view a guild channel, as well as how many are online at the given time. Fires a [Channel Member Count Update](#channel-member-count-update) Gateway event with the requested count.
...
| [Lobby Voice State Update](#lobby-voice-state-update)                               | User joined, left, or moved from a lobby                                                        |
...
^10^ The field is only available in OAuth2 context.
...
###### Gateway Lobby Structure

| Field        | Type                                                                  | Description                                                                          |
| ------------ | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| voice_states | array[[lobby voice state](/resources/voice#lobby-voice-state) object] | The voice states of the users already in the lobby                                   |
| region       | string                                                                | The [voice region](/resources/voice#voice-region-object) ID the lobby is hosted from |
| metadata     | ?string \| map[string, string]                                        | The metadata of the lobby, might be serialized as JSON string                        |
...
###### Lobby Message Object Extra Fields
...
Sent when a lobby is deleted, or user was removed from lobby.
...
Sent when a lobby message is created. The inner payload is a [lobby message](/resources/message#message-object) object with the [extra structure](#lobby-message-object-extra-fields) above.

#### Lobby Message Update

Sent when a lobby message is updated. The inner payload is a [lobby message](/resources/message#message-object) object with the [extra structure](#lobby-message-object-extra-fields) above.
...
Sent when someone joins/leaves/moves lobbies. Inner payload is a [lobby voice state](/resources/voice#lobby-voice-state-object) object.
