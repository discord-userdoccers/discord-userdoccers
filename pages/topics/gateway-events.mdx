...
| [Remote Command](#remote-command)                         | Send a remote command to an embedded voice session (e.g. Xbox integration) |
...
Sent when a client wants to create a stream in a given voice channel. Requires the `STREAM` permission. Fires a [Stream Create](#stream-create) and [Stream Server Update](#stream-server-update) Gateway event.
...
Sent when a client wants to watch a user's stream. Fires a [Stream Create](#stream-create) and [Stream Server Update](#stream-server-update) Gateway event.
...
Sent when a client wants to pause or resume their stream. Fires a [Stream Server Update](#stream-server-update) Gateway event.
...
Sent when a client wants to disconnect from a stream. If the client is the owner of the stream, the stream will be deleted. Fires a [Stream Delete](#stream-delete) Gateway event.
...
| type              | integer                                               | The [type of reaction](/resources/channel#reaction-type) |
...
| Field             | Type      | Description                                                            |
| ----------------- | --------- | ---------------------------------------------------------------------- |
| stream_key        | string    | The [stream key](#stream-key) for the stream                           |
| rtc_server_id ^1^ | snowflake | The ID of the RTC server for the stream, used when connecting to voice |
| region            | string    | The region of the stream                                               |
...
Sent when the client settings are modified. Inner payload is a [user settings](/resources/user-settings#legacy-user-settings-object) object.
