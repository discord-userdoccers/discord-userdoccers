...
| [Request Forum Unreads](#request-forum-unreads)           | Request thread-only channel unread counts                              |
...
| [Search recent Members](#search-recent-members)           | Similar to OP 8, used for Member Safety.                               |
...
Used to search members for Member Safety. You can request up-to 1000 members per call

The `continuation_token` is a member id calculated by the following algorithm
...
search_chunk_size = 1000 # Max per call

def calculate_new_continuation_token(guild_id: str, sorted_member_ids: list[str], state: State) -> str | None:
  # `sorted_member_ids` is an array of member IDs sorted by their `joined_at` field (descending).
  # You do not need to calculate a `continuation_token` if you haven't performed an initial Search Recent Members call,
  # the array is empty until a `GUILD_MEMBERS_CHUNK` event recieved after a Search Recent Members call,
  # sorting can be performed by converting the `joined_at` ISO 8601 timestamp in the guild member object
  # to a UNIX timestamp and sorting the result in an descending numerical order.

  member_count = get_guild_member_count(guild_id) or len(sorted_member_ids)
  
  sorted_member_size = len(sorted_member_ids) - 1
  last_member_id = sorted_member_ids[sorted_member_size]

  minimum_chunk_size = min(search_chunk_size, sorted_member_size)

  current_index = state["current_page"] * state["page_size"]
  new_continuation_token = sorted_member_ids[current_index - 1] or last_member_id

  if not state["continuation_token"]:
    return state["continuation_token"] if current_index < minimum_chunk_size else new_continuation_token

  try:
    previous_continuation_token_index = sorted_member_ids.index(state["continuation_token"])

    if current_index > member_count:
      return last_member_id

    if (previous_continuation_token_index - current_index) >= 0:
      return state["continuation_token"]

    if ceil(current_index / minimum_chunk_size) > ceil(previous_continuation_token_index / minimum_chunk_size):
      return new_continuation_token
  except ValueError:
    return new_continuation_token
...
| Field              | Type       | Description                                 |
| ------------------ | ---------- | ------------------------------------------- |
| guild_id           | snowflake  | ID of guild to search                       |
| query              | string     | A username of ID to search for              |
| continuation_token | ?snowflake | Token/member ID to continue pagination from |
...
| user_settings ^1^ ^4^        | [user settings](/resources/user-settings#user-settings-object) object      | The client settings for the user                                                                                                                                                                                                                                 |
...
| tutorial ^1^                 | ?[tutorial](#tutorial-structure) object                                    | The tutorial state of the user, if any                                                                                                                                                                                                                           |
...
| experiments ^1^              | array[[user experiment](/resources/experiments#user-experiments) object]   | User experiment rollouts for the user                                                                                                                                                                                                                            |
| guild_experiments ^1^        | array[[guild experiment](/resources/experiments#guild-experiments) object] | Guild experiment rollouts for the user                                                                                                                                                                                                                           |
...
| merged_members ^1^    | array[array[[member](/resources/guild#member-object) object]]     | The members of the user's guilds, in the same order as the `guilds` array                                                                                                                                                                                     |
...
Sent when a channel is updated. The inner payload is a [channel](/resources/channel#channel-object) object. This is not sent when the field `last_message_id` is altered. To keep track of the `last_message_id` changes, you must listen for [Message Create](#message-create) events (or [Thread Create](#thread-create) events for thread-only channels).
...
| pending? ^1^                     | boolean                                            | Whether the member has not yet passed the guild's [membership screening](#membership-screening-object) requirements                                                                                                                  |
...
| presences?  | array[[presence](/resources/presence#presence) object]             | The presences of the returned members, if requested                                         |
| nonce?      | string                                                             | The nonce used in [Request Guild Members](#request-guild-members), if any                   |
...
Presence Update events are never received for the user's own presence. You _must_ track your own presence locally, or via the [Sessions Replace](#sessions-replace) event (if applicable).
