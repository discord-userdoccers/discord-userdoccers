...
| user_guild_settings ^1^ ^9^            | [versioned](#versioned-structure) object ([user guild settings](/resources/user-settings#user-guild-settings-object) object) | The guild settings for the user                                                                                                                                                                                                                               |
...
| relationships ^1^                      | array[[relationship](/resources/user#relationship-object)]                                                                   | The relationships the user has with other users                                                                                                                                                                                                               |
...
| presences ^6^                          | array[[presence](/resources/presence#presence-object) object]                                                                | The presences of the user's non-offline [friends and implicit relationships](/resources/user#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)).                                         |
| read_state ^10^                        | [versioned](#versioned-structure) object ([read state](/resources/channel#read-state-object) object)                         | The read states for the channels and guilds                                                                                                                                                                                                                   |
| merged_presences ^6^                   | [merged presences](#merged-presences-structure) object                                                                       | The presences of the user's non-offline [friends and implicit relationships](/resources/user#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)), and any guild presences sent at startup |
...
^9^ The field will be an array of [user guild settings](/resources/user-settings#user-guild-settings-object) objects when using `VERSIONED_USER_GUILD_SETTINGS` [Gateway capability](/topics/gateway#gateway-capabilities).

^10^ The field will be an array of [read state](/topics/read-state#read-state-object) objects when using `VERSIONED_READ_STATES` [Gateway capability](/topics/gateway#gateway-capabilities).
...
| Field      | Type                                                                 | Description                                                                                                                                                                                          |
| ---------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| friends    | array[[presence](/resources/presence#presence-object) object]        | Presences of the user's [friends and implicit relationships](/resources/user#relationship-object) (depending on the [`NO_AFFINE_USER_IDS` Gateway capability](/topics/gateway#gateway-capabilities)) |
...
| Field   | Type                                                                                                                                                  | Description                                       |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- |
| entries | array[[read state](/topics/read-state#read-state-object) object \| [user guild settings](/resources/user-settings#user-guild-settings-object) object] | The entries                                       |
| partial | boolean                                                                                                                                               | Whether the `entries` field is partial            |
| version | integer                                                                                                                                               | The version of read states or user guild settings |
...
Sent when a friend suggestion is created. The inner payload is a [friend suggestion](/resources/user#friend-suggestion-object) object.
...
^1^ For bots without the `GUILD_PRESENCES` [Gateway intent](/topics/gateway#gateway-intents), user accounts, or guilds with over 75k members, this will only include the client's member and users in voice channels. User accounts additionally receive [friends and implicit relationships](/resources/user#relationship-object) (depending on the [`NO_AFFINE_USER_IDS` Gateway capability](/topics/gateway#gateway-capabilities)), as well as users they have an open DM with.

^2^ User accounts only receive presences for non-offline [friends and implicit relationships](/resources/user#relationship-object) (depending on the [`NO_AFFINE_USER_IDS` Gateway capability](/topics/gateway#gateway-capabilities)), as well as users they have an open DM with. Bots with the `GUILD_PRESENCES` [Gateway intent](/topics/gateway#gateway-intents) receive all presences.
...
For user accounts, this event is only sent for themselves. Users are not automatically subscribed to [friends and implicit relationships](/resources/user#relationship-object) or users they have an open DM with upon them joining.
...
For user accounts, this event is only sent for themselves, members they are subscribed to, and in response to actions committed by the user. Users are automatically subscribed to [friends and implicit relationships](/resources/user#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)), as well as users they have an open DM with.
...
| -------- | ---------------------------------------------- | ------------------- |
| guild_id | snowflake                                      | The ID of the guild |
| role     | [role](/topics/permissions#role-object) object | The role created    |
...
| -------- | ---------------------------------------------- | ------------------- |
| guild_id | snowflake                                      | The ID of the guild |
| role     | [role](/topics/permissions#role-object) object | The role updated    |
...
For user accounts, this event is only sent for presences they are subscribed to. Users are automatically subscribed to the overall user presence and every per-guild presence of [friends and implicit relationships](/resources/user#relationship-object) (depending on the `NO_AFFINE_USER_IDS` [Gateway capability](/topics/gateway#gateway-capabilities)), as well as every per-guild presences of users they have an open DM with.
...
Presence Update events are never received for the user's own presence. You _must_ track your own presence locally, or via the Sessions Replace event (if applicable). {/* todo: link this here */}
...
Sent when a relationship is created, relevant to the current user. Inner payload is a [relationship](/resources/user#relationship-object) object.
...
This is not sent when the type of a relationship changes; see [Relationship Remove](#relationship-remove) and [Relationship Add](#relationship-add) for that.
...
| type                 | integer   | The [type](/resources/user#relationship-type) of relationship   |
...
Sent when the user guild settings are updated. Inner payload is [user guild settings](/resources/user-settings#user-guild-settings-object) object.
...
Returns an object with a single valid WebSocket URL, which the client can use for [Connecting](/topics/gateway#connecting). Clients **should** cache this value and only call this endpoint to retrieve a new URL if they are unable to properly establish a connection using the cached version of the URL.
...
Returns an object based on the information in [Get Gateway](#get-gateway), plus additional metadata that can help during the operation of large or [sharded](/topics/gateway#sharding) bots. Unlike [Get Gateway](#get-gateway), this route should not be cached for extended periods of time as the value is not guaranteed to be the same per-call, and changes as the user joins/leaves guilds.
