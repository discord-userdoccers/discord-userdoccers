...
| GET_CHANNEL                                      | Used to retrieve channel information from the client                                            | Any of `rpc`, `guilds`, or `guilds.channel.read`                                    |                                    |
| GET_CHANNELS                                     | Used to retrieve a list of channels for a guild from the client                                 | `rpc`                                                                               |                                    |
| GET_CHANNEL_PERMISSIONS                          | Used to retrieve the permission settings for a given channel                                    | `guilds.members.read`, or `guilds.channels.read`                                    |                                    |
| CREATE_CHANNEL_INVITE                            | Used to create an invite link for a given channel                                               | `rpc`                                                                               |                                    |
| GET_RELATIONSHIPS                                | Used to retrieve the friend list and relationship statuses                                      | `relationships.read`                                                                |                                    |
| GET_USER                                         | Used to retrieve information about a specific user                                              |                                                                                     | HTTP, WebSocket, or `post_message` |
...
| GET_ACTIVITY_INSTANCE_CONNECTED_PARTICIPANTS ^1^ | Retrieves users connected to a specific activity session                                        |                                                                                     | `post_message`                     |
