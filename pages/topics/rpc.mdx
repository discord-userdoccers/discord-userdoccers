...
| Field   | Type    | Description                                            |
| ------- | ------- | ------------------------------------------------------ |
| code    | integer | The reason for the closure                             |
...
Due to Discord client bugs, in some incoming packets, some of the below objects may instead be serialized as their internal client representation, which may not match the documented API structure.
...
| [SET_USER_VOICE_SETTINGS_2](#set-user-voice-settings-2)                                       | Used to change basic voice settings of users in voice channels                                               | `rpc.local`                                     |
...
| [SET_VOICE_SETTINGS_2](#set-voice-settings-2)                                                 | Used to set the client's basic voice settings                                                                | `rpc.local`                                     |
...
| [OPEN_EXTERNAL_LINK](#open-external-link)                                                     | Used to prompt to open a given URL in the default web browser from the client                                | `rpc.authenticated` or `rpc.embedded_app`       |
...
This structure is a superset of the [Get Current Authorization Information](/topics/oauth2#get-current-authorization-information) response's body with the following additional fields:
...
| [VOICE_SETTINGS_UPDATE_2](#voice-settings-update-2)                             | Same when the client's basic voice settings update                                                                  | `rpc.local`                                |
...
| [OVERLAY_UPDATE](#overlay-update)                                               | Sent when the game overlay settings are changed for a process                                                       | `rpc.local`                                |
...
| [ENTITLEMENT_DELETE](#entitlement-delete)                                       | Sent when an [entitlement is deleted](/topics/gateway-events#entitlement-delete) for one of your application's SKUs | `rpc.authenticated`, `rpc.local`           |
