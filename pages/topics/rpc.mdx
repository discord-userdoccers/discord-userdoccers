...
| ----- | ------ | --------------------------------------------------- | -------------------------------------------------------- |
| cmd   | enum   | [payload command](#rpc-commands)                    | Always                                                   |
| nonce | string | unique string used once for replies from the server | In responses to commands (not subscribed events)         |
| evt   | enum   | [subscription event](#rpc-events)                   | In subscribed events, errors, and (un)subscribing events |
...
| [GET_SELECTED_VOICE_CHANNEL](#get_selected_voice_channel) | used to get the current voice channel the client is in          |
...
| [SET_CERTIFIED_DEVICES](#set_certified_devices)           | used to send info about certified hardware devices              |
| [SET_ACTIVITY](#set_activity)                             | used to update a user's Rich Presence                           |
| [SEND_ACTIVITY_JOIN_INVITE](#send_activity_join_invite)   | used to consent to a Rich Presence Ask to Join request          |
| [CLOSE_ACTIVITY_REQUEST](#close_activity_request)         | used to reject a Rich Presence Ask to Join request              |
...
| [VOICE_STATE_CREATE](#voice_state_create/voice_state_update/voice_state_delete) | sent when a user joins a subscribed voice channel                                          |
| [VOICE_STATE_UPDATE](#voice_state_create/voice_state_update/voice_state_delete) | sent when a user's voice state changes in a subscribed voice channel (mute, volume, etc.)  |
| [VOICE_STATE_DELETE](#voice_state_create/voice_state_update/voice_state_delete) | sent when a user parts a subscribed voice channel                                          |
...
| [SPEAKING_START](#speaking_start/speaking_stop)                                 | sent when a user in a subscribed voice channel speaks                                      |
| [SPEAKING_STOP](#speaking_start/speaking_stop)                                  | sent when a user in a subscribed voice channel stops speaking                              |
| [MESSAGE_CREATE](#message_create/message_update/message_delete)                 | sent when a message is created in a subscribed text channel                                |
| [MESSAGE_UPDATE](#message_create/message_update/message_delete)                 | sent when a message is updated in a subscribed text channel                                |
| [MESSAGE_DELETE](#message_create/message_update/message_delete)                 | sent when a message is deleted in a subscribed text channel                                |
...
| --------- | ------------------------------------------------------ | ------------------------------------------------------------------------- |
| scopes    | array of [OAuth2 scopes](/topics/oauth2#oauth2-scopes) | scopes to authorize                                                       |
...
| scopes      | array of [OAuth2 scopes](/topics/oauth2#oauth2-scopes)     | authorized scopes               |
| expires     | date                                                       | expiration date of OAuth2 token |
| application | [OAuth2 application](#oauth2-application-structure) object | application the user authorized |
...
| pan?    | [pan](#pan-object) object | set the pan of the user                                  |
...
Used to join and leave voice channels, group dms, or dms. Returns the [Get Channel](#get_channel) response, `null` if none.
...
Used to get the client's current voice channel. There are no arguments for this command. Returns the [Get Channel](#get_channel) response, or `null` if none.

#### SELECT_TEXT_CHANNEL

Used to join and leave text channels, group dms, or dms. Returns the [Get Channel](#get_channel) response, or `null` if none.
...
| Field                  | Type                                                          | Description                       |
| ---------------------- | ------------------------------------------------------------- | --------------------------------- |
| input                  | [voice settings input](#voice-settings-input-object) object   | input settings                    |
| output                 | [voice settings output](#voice-settings-output-object) object | output settings                   |
| mode                   | [voice settings mode](#voice-settings-mode-object) object     | voice mode settings               |
...
| Field | Type    | Description                 |
| ----- | ------- | --------------------------- |
| type  | integer | see [key types](#key-types) |
...
| Field   | Type                                                | Description                                                   |
| ------- | --------------------------------------------------- | ------------------------------------------------------------- |
| devices | array of [certified device](#device-object) objects | a list of devices for your manufacturer, in order of priority |

###### Device Object

| Field                     | Type                            | Description                                              |
| ------------------------- | ------------------------------- | -------------------------------------------------------- |
| type                      | [device type](#device-types)    | the type of device                                       |
| id                        | string                          | the device's Windows UUID                                |
| vendor                    | [vendor](#vendor-object) object | the hardware vendor                                      |
| model                     | [model](#model-object) object   | the model of the product                                 |
...
| config | [rpc server configuration](#rpc-server-configuration-object) object | server configuration               |
...
No arguments. Dispatches the [Get Voice Settings](#get_voice_settings) response.
...
No arguments. This event requires the `rpc.notifications.read` [OAuth2 scope](/topics/oauth2#oauth2-scopes).
...
| Field  | Type   | Description                                                                                    |
| ------ | ------ | ---------------------------------------------------------------------------------------------- |
| secret | string | the [`join_secret`](/rich-presence/how-to#update-presence-payload-fields) for the given invite |
...
| Field  | Type   | Description                                                                                        |
| ------ | ------ | -------------------------------------------------------------------------------------------------- |
| secret | string | the [`spectate_secret`](/rich-presence/how-to#update-presence-payload-fields) for the given invite |
