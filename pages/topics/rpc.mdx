...
| [OPEN_SHARE_MOMENT_DIALOG](#open-share-moment-dialog) ^1^                                         | Used for activities, allowing sharing an image from the activity                                |                                                                                     |                                    |
...
| [SEND_ANALYTICS_EVENT](#send-analytics-events)                                                    | Sends an analytics event to Discord via embedded activity                                       |                                                                                     | `post_message`                     |
...
| buttons?             | array[[rpc activity button](#rpc-activity-button) object]                       | Custom buttons shown in rich presence (1-2)                                                                      |
...
^1^ For retrieving rich presence metadata, see [Get Activity Metadata](#get-activity-metadata). For retrieving secrets, see [Get Activity Secret](#get-activity-secret).
...
| Field        | Type   | Description                                                                                |
| handoffToken | string | The handoff token received from the [Create Handoff Token](#create-handoff-token) endpoint |
| fingerprint  | string | The current [fingerprint](/topics/experiments#fingerprints)                                |

{/* TODO: SET_CERTIFIED_DEVICES */}
