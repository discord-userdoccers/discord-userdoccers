...
^2^ Erlpack encoding may not be supported by all clients.

#### Sending and Receiving Packets

Packets are sent as JSON objects containing the [command you want to send to the Discord client](#outgoing-packet-structure). [Incoming packets](#incoming-packet-structure) will need to be correctly parsed to obtain the JSON response.
...
The HTTP transport is a subset of the WebSocket transport meant for one-of commands that do not require a long-lived connection.
...
Clients should send a `POST` request to the connection URL with a JSON body containing the [command you want to send](#outgoing-packet-structure). The server will respond with a JSON object containing the response of the command.
...
| nonce    | string | A unique identifier given to a [command](#rpc-commands) that will be echoed back to you upon a successful send |
...
### RPC Internal User Object

###### RPC Internal User Structure
...
| username ^1^            | string                                                                          | The user's username, may be unique across the platform (2-32 characters)                                        |
| discriminator ^1^       | string                                                                          | The user's stringified 4-digit Discord tag                                                                      |
| globalName? ^1^         | ?string                                                                         | The user's display name (1-32 characters)                                                                       |
| global_name? ^1^        | ?string                                                                         | The user's display name (1-32 characters)                                                                       |
| avatar                  | ?string                                                                         | The user's [avatar hash](/reference#cdn-formatting)                                                             |
...
| avatar_decorationData?  | ?[RPC avatar decoration data](#rpc-avatar-decoration-data-object) object        | The user's [avatar decoration](https://support.discord.com/hc/en-us/articles/13410113109911-Avatar-Decorations) |
| displayNameStyles?      | ?[RPC display name style](#rpc-display-name-style-structure) object             | The user's display name style                                                                                   |
| display_name_styles?    | ?[display name style](/resources/user#display-name-style-structure) object      | The user's display name style                                                                                   |
| primaryGuild?           | ?[RPC primary guild](#rpc-primary-guild-structure) object                       | The primary guild of the user                                                                                   |
| primary_guild?          | ?[primary guild](/resources/user#primary-guild-structure) object                | The primary guild of the user                                                                                   |
| bot?                    | boolean                                                                         | Whether the user is a bot account                                                                               |
| system?                 | boolean                                                                         | Whether the user is an official Discord System user (part of the urgent message system)                         |
| publicFlags?            | integer                                                                         | The public [flags](/resources/user#user-flags) on a user's account                                              |
| public_flags?           | integer                                                                         | The public [flags](/resources/user#user-flags) on a user's account                                              |
| premiumType?            | integer                                                                         | The [type of premium (Nitro) subscription](/resources/user#premium-type) on a user's account                    |
| premium_type?           | integer                                                                         | The [type of premium (Nitro) subscription](/resources/user#premium-type) on a user's account                    |
| flags                   | integer                                                                         | The [flags](/resources/user#user-flags) on a user's account                                                     |

^1^ See the [section on Discord's new username system](/resources/user#unique-usernames) for more information.

###### RPC Primary Guild Structure

| Field               | Type       | Description                                           |
| ------------------- | ---------- | ----------------------------------------------------- |
| identityEnabled ^1^ | ?boolean   | Whether the user is displaying their guild tag        |
| identityGuildId ^2^ | ?snowflake | The ID of the guild                                   |
...
^2^ Only populated for users with `identityEnabled` not set to `false`.

###### RPC Display Name Style Structure

| Field    | Type           | Description                                                                                    |
| -------- | -------------- | ---------------------------------------------------------------------------------------------- |
| fontId   | integer        | The [font](/resources/user#display-name-font) to use                                           |
| effectId | integer        | The [effect](/resources/user#display-name-effect) to use                                       |
...
| blocked?         | boolean                                                          | Whether the author of message is blocked by the current user |
| bot?             | boolean                                                          | Whether the author of message is a bot account               |
...
| mentions         | array[[RPC internal user](#rpc-internal-user-object) object]     | Users specifically mentioned in this message                 |
| mention_everyone | boolean                                                          | Whether this message mentions everyone                       |
| mention_roles    | array[snowflake]                                                 | Roles specifically mentioned in this message                 |
| embeds           | array[[RPC embed](#rpc-embed-structure) object]                  | Content embedded in the message                              |
...
| Field               | Type                                                                 | Description                                                                                                                  |
| ------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| id                  | string                                                               | The local ID of the embed (unique across application session)                                                                |
| url?                | string                                                               | The URL of the embed (max 2048 characters)                                                                                   |
| type                | string                                                               | The [type of embed](/resources/message#embed-type) (always `rich` for sent embeds)                                           |
| rawTitle?           | string                                                               | The title of the embed (max 256 characters)                                                                                  |
| rawDescription?     | string                                                               | The description of the embed (max 4096 characters)                                                                           |
| referenceId?        | snowflake                                                            | The ID of the message this embed was generated from                                                                          |
| flags?              | integer                                                              | The [embed's flags](/resources/message#embed-flags)                                                                          |
| contentScanVersion? | integer                                                              | The version of the explicit content scan filter this embed was scanned with                                                  |
| footer?             | [RPC embed footer](#rpc-embed-footer-structure) object               | Embed footer information                                                                                                     |
| author?             | [RPC embed author](#rpc-embed-author-structure) object               | Embed author information                                                                                                     |
| provider?           | [embed provider](/resources/message#embed-provider-structure) object | Embed provider information                                                                                                   |
| timestamp?          | ISO8601 timestamp                                                    | Timestamp of embed content                                                                                                   |
| color?              | string                                                               | The color of the embed encoded as [CSS color](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/color_value) |
| image?              | [RPC embed media](#rpc-embed-media-structure) object                 | Embed image information                                                                                                      |
| thumbnail?          | [RPC embed media](#rpc-embed-media-structure) object                 | Embed thumbnail information                                                                                                  |
| video?              | [RPC embed media](#rpc-embed-media-structure) object                 | Embed video information                                                                                                      |
| fields              | array[[RPC embed field](#rpc-embed-field-structure) object]          | The fields of the embed (max 25)                                                                                             |

###### RPC Embed Footer Structure

| Field         | Type   | Description                                                                                 |
| ------------- | ------ | ------------------------------------------------------------------------------------------- |
| text          | string | The footer text (max 2048 characters)                                                       |
| iconURL?      | string | Source URL of the footer icon (only supports http(s) and attachments) (max 2048 characters) |
| iconProxyURL? | string | A proxied URL of the footer icon                                                            |

###### RPC Embed Author Structure
...
| iconURL?      | string | Source URL of the author's icon (only supports http(s) and attachments) (max 2048 characters) |
| iconProxyURL? | string | A proxied URL of the author's icon                                                            |

###### RPC Embed Media Structure

| Field               | Type    | Description                                                                                                   |
| ------------------- | ------- | ------------------------------------------------------------------------------------------------------------- |
| url                 | string  | Source URL of media (only supports http(s) and attachments) (max 2048 characters)                             |
| proxyURL?           | string  | A proxied URL of the media                                                                                    |
| width?              | integer | Width of media                                                                                                |
| height?             | integer | Height of media                                                                                               |
| placeholder?        | string  | A low-resolution [thumbhash](https://github.com/evanw/thumbhash) of the media, to display before it is loaded |
| placeholderVersion? | integer | The attachment placeholder protocol version (currently 1)                                                     |
| description?        | string  | Alt text for the media                                                                                        |
| srcIsAnimated       | boolean | Whether the media's attachment is animated                                                                    |
| flags               | integer | The [media's attachment flags](/resources/message#attachment-flags)                                           |

###### RPC Embed Field Structure

| Field    | Type    | Description                                              |
| -------- | ------- | -------------------------------------------------------- |
| rawName  | string  | The name of the field (max 256 characters)               |
| rawValue | string  | The value of the field (max 1024 characters)             |
...
| threshold      | integer                                                           | The threshold (in dB) for voice activity (min -100, max 0)       |
...
| [USER_SETTINGS_GET_LOCALE](#user-settings-get-locale)                                             | Retrieves the client’s locale, i.e. `en-US`                                                                  |                                                                                     | HTTP, or IPC                       |
...
| [INVITE_USER_EMBEDDED](#invite-user-embedded)                                                     | Invites a user to the current embedded activity                                                              | `relationships.read`                                                                |                                    |
| [INVITE_BROWSER](#invite-browser)                                                                 | Used to open an invite modal for a guild invite in the client                                                |                                                                                     | WebSocket                          |
| [REQUEST_PROXY_TICKET_REFRESH](#request-proxy-ticket-refresh) ^1^                                 | Used to refresh proxy tickets                                                                                |                                                                                     |                                    |
...
The application must have [`EMBEDDED`](/resources/application#application-flags) flag set.
...
Used to invite a user to the current embedded activity set via [`SET_ACTIVITY`](#set-activity) command.
...
| [VOICE_SETTINGS_UPDATE_2](#voice-settings-update-2)                                 | Same as `VOICE_SETTINGS_UPDATE`, but without authentication                                                      |                                                                                                | HTTP, or IPC |
...
| [SPEAKING_START](#speaking-start)                                                   | Sent when a user in a subscribed voice channel speaks                                                            | `rpc` or `rpc.voice.read` are required unless using HTTP/IPC transport                         |              |
| [SPEAKING_STOP](#speaking-stop)                                                     | Sent when a user in a subscribed voice channel stops speaking                                                    | `rpc` or `rpc.voice.read` are required unless using HTTP/IPC transport                         |              |
| [GAME_JOIN](#game-join) **(deprecated)**                                            | Sent when the user clicks a Rich Presence join invite in chat to join a game                                     | `rpc` is required unless using HTTP/IPC transport                                              |              |
| [GAME_SPECTATE](#game-spectate) **(deprecated)**                                    | Sent when the user clicks a Rich Presence spectate invite in chat to spectate a game                             | `rpc` is required unless using HTTP/IPC transport                                              |              |
| [ACTIVITY_JOIN](#activity-join) ^1^                                                 | Sent when the user clicks a Rich Presence join invite in chat to join an activity                                |                                                                                                |              |
| [ACTIVITY_JOIN_REQUEST](#activity-join-request)                                     | Sent when the user receives a Rich Presence Ask to Join request                                                  | `rpc` is required unless using HTTP/IPC transport                                              |              |
| [ACTIVITY_SPECTATE](#activity-spectate) ^1^                                         | Sent when the user clicks a Rich Presence spectate invite in chat to spectate a game                             |                                                                                                |              |
| [ACTIVITY_INVITE](#activity-invite)                                                 | Sent when the user receives a Rich Presence Join request                                                         | `rpc` is required unless using HTTP/IPC transport                                              |              |
...
| [THERMAL_STATE_UPDATE](#thermal-state-update) ^2^                                   | Sent when thermal state of mobile device is updated                                                              |                                                                                                |              |
| [ORIENTATION_UPDATE](#orientation-update) ^2^                                       | For mobile devices, indicates a change in orientation of the screen                                              |                                                                                                |              |
...
| [NOTIFICATION_CREATE](#notification-create)                                         | Sent when the client receives a notification (mention or new message in eligible channels)                       | `rpc` and `rpc.notifications.read` are reqiured                                                |              |
| [MESSAGE_CREATE](#message-create)                                                   | Sent when a message is created in a subscribed text channel                                                      | `rpc` is required, `messages.read` is required unless the guild belongs to current application |              |
| [MESSAGE_UPDATE](#message-update)                                                   | Sent when a message is updated in a subscribed text channel                                                      | `rpc` is required, `messages.read` is required unless the guild belongs to current application |              |
| [MESSAGE_DELETE](#message-delete)                                                   | Sent when a message is deleted in a subscribed text channel                                                      | `rpc` is required, `messages.read` is required unless the guild belongs to current application |              |
| OVERLAY                                                                             | (Unknown)                                                                                                        |                                                                                                | WebSocket    |
| OVERLAY_UPDATE                                                                      | (Unknown)                                                                                                        |                                                                                                | HTTP, or IPC |
| [ENTITLEMENT_CREATE](#entitlement-create) ^1^                                       | Sent when an entitlement is created when a user purchases or is otherwise granted one of your application's SKUs |                                                                                                |              |
| [ENTITLEMENT_DELETE](#entitlement-delete) ^1^                                       | Sent when an [entitlement is deleted](/topics/gateway-events#entitlement-delete)                                 |                                                                                                |              |
...
| [SCREENSHARE_STATE_UPDATE](#screenshare-state-update)                               | Sent when a user's screenshare state changes                                                                     | `rpc.screenshare.read`                                                                         | HTTP, or IPC |
| [VIDEO_STATE_UPDATE](#video-state-update)                                           | Sent when a user's video state changes                                                                           | `rpc.voice.read`                                                                               | HTTP, or IPC |
...
| Field                    | Type                                                  | Description                                                         |
| ------------------------ | ----------------------------------------------------- | ------------------------------------------------------------------- |
| guild                    | [partial RPC guild](#partial-rpc-guild-object) object | The guild                                                           |
| online? **(deprecated)** | integer                                               | Approximate number of non-offline members in the guild (always `0`) |

#### Guild Create

Sent when a guild is created/joined on the client. The inner payload is a [partial RPC guild](#partial-rpc-guild-object) object.
...
Sent when a user's relationship updates. The inner payload is a [RPC relationship](#rpc-relationship-object) object.
...
Sent when a user joins a subscribed voice channel. The inner payload is a [RPC voice state](#rpc-voice-state-object) object.
...
Sent when a user parts a subscribed voice channel. The inner payload is a [RPC voice state](#rpc-voice-state-object) object.
...
Sent when a user's voice state changes in a subscribed voice channel (mute, volume, etc.). The inner payload is a [RPC voice state](#rpc-voice-state-object) object.
...
Sent when the client's voice settings update. The inner payload is a [RPC voice settings](#rpc-voice-settings-object) object.
...
Sent when an [entitlement is deleted](/topics/gateway-events#entitlement-delete). The inner payload is an [entitlement](/resources/entitlement#entitlement-object) object.
