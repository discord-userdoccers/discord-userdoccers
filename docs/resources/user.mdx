...
| linked_users ^1^ ^3^          | array[[linked user](#linked-user-structure) object]                 | The linked users connected to the account via Family Center                                                     |
...
| identity_guild_id ^1^ | ?snowflake | The ID of the guild                                |
...
Partial user data of an underage user linked to the account via [Family Center](https://support.discord.com/hc/en-us/articles/14155043715735-Family-Center-for-Parents-and-Guardians).
...
| requestor_id | snowflake           | The ID of the account the linked user is connected to        |
...
The Link Status represents the current state of the link.
...
The Link Type represents what part each user played in the connection.

| Value     | Description                                                           |
| --------- | --------------------------------------------------------------------- |
| 1         | The current user accepted the request and is the "teen" of the link   |
| 2         | The current user sent the request and is the "parent" of the link     |
...
| hubspot      | Official Discord programs                    |
...
| is_spam_request? ^1^ | boolean                      | Whether the relationship was flagged as spam (default false)    |
...
Modifies the requester's user account settings. Returns a [user](#user-object) object (does not respect the `email` scope). Fires a [User Update](/topics/gateway-events#user-update) Gateway event.
...
| integration_types_config?        | map[int, ?[application integration type configuration](/resources/application#application-integration-type-configuration-structure) object] | The configuration for each [integration type](/resources/application#application-integration-type) supported by the application |
...
^1^ Providing a valid [friend token](#get-friend-token) will bypass the user's friend request settings.
...
<RouteHeader method="DELETE" url="/users/@me/relationships">
  Bulk Remove User Relationships
</RouteHeader>
...
Returns the current user's friend token. These can be used to [fetch the user's profile](#get-user-profile) without a mutual and [add the user as a friend](#create-user-relationship) regardless of their friend request settings.
...
If the application the user is joining the program with loses the `ACTIVE` application flag, the user will have their [`ACTIVE_DEVELOPER` user flag](#user-flags) removed after a 30 day grace period.
