...
async function main() {
  if (!["push", "pull_request_target"].includes(context.eventName)) {
    error('expected context event to be one of ("push", "pull_request_target")', {
      title: "Invalid Event",
    });
  ...
  const deploymentRef =
    context.eventName === "pull_request_target" ? context.payload.pull_request!.head.sha : context.sha;
