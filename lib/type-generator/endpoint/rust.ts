...
export class RustEndpointGenerator {
  ...
  public generateCode() {
    ...
    if (data.endpoint.includes("{") || data.hasQueryParams) {
      ...
      output += `pub fn ${data.name.toSnakeCase().toUpperCase()}(${data.hasQueryParams ? `query: &${data.name.toPascalCase()}QueryParams${params.length ? ", " : ""}` : ""}${params.map((s) => {return s + ": &str"}).join(", ")}) -> String {\n`;
