...
export class Tokenizer {
    public constructor(public readonly root: HTMLDivElement, public readonly info: RouteHeaderProps) {}
...
export class Name {
    ...
    public toPascalCase(): string {
        return this.name.toLowerCase().split(" ").map((word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join("");
    }

    public toCamelCase(): string {
        return this.name.toLowerCase().split(" ").map((word, i) => i === 0 ? word.toLowerCase() : word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join("");
    }
}
