...
export abstract class TypeGenerator {
    ...
    private static isBlockLevelElement(element: HTMLElement): boolean {
        const blockLevelTags = ['DIV', 'P', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'UL', 'OL', 'LI', 'TABLE', 'TR', 'TD', 'BLOCKQUOTE', 'PRE', 'HR', 'FORM'];
    ...
    public static parseDOM(rootElement: HTMLElement) {
        ...
        switch (headings[0]) {
            ...
            // Enum or Bitfield
            case "VALUE": {
                const isBitfield = rowCells.some((row) => row.some(
                    (cell, cellId) => headings[cellId] === "VALUE" && cell.innerText.includes("<<")
                ));
    ...
    */
    protected static parseType(
        ...
        const nullableStr = (inner: string) => isNullable && origNullableStr ? origNullableStr(inner, isUndefinable) : inner;
